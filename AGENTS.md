...
- For every COREX task, start with `docs/agent_maps/INDEX.md`, the relevant row in `docs/agent_maps/COVERAGE.md`, and one relevant subsystem, feature-route, or testing map, even when a file or symbol is already known. After implementation or architecture/feature changes, update affected agent maps and `docs/agent_maps/COVERAGE.md` in the same change; if no map update is needed, state that in the final response.
...
- After the maps-first pass, look up known files, symbols, UI labels, errors, settings, and tests with a bounded source search in the mapped area. Use `scripts/nav.py source <path>` only when owner or focused-test context is needed; do not send a known target through fuzzy `nav.py find`.
...
- For broad or uncertain ownership after the maps-first pass, query `docs/agent_route_index.md` and `docs/agent_route_index.json` through `scripts/nav.py` instead of reading large raw indexes when the compact query is sufficient.
...
- For ambiguous QML work, use `scripts/nav.py qml <term>` after the maps-first pass and before widening into `ea_node_editor/ui_qml`; search exact component/property/signal names within the mapped area. Use `route`, `qml`, or `source` when the index family is known.
- Use `docs/source_test_file_index.md` as a path inventory after identifying the relevant route, not as ownership authority. For requirements/proof questions, use `docs/specs/INDEX.md` as the authority after the maps-first routing pass; agent maps remain navigation references unless formally registered as proof/spec artifacts.
