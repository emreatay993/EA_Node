...
- Prefer the navigation query CLI before reading raw indexes: `./venv/Scripts/python.exe scripts/nav.py find <term>` (also `route`, `qml`, `source`). It returns the owner map, source/test/QML candidates, focused-verification commands, and map section anchors in a few hundred tokens instead of grepping the 0.6 MB/3.4 MB index files. Use its output as the navigation-audit record. `nav.py source <path>` answers "which map owns this file" for a file you already have. Fall back to the raw indexes below only when the CLI under-resolves.
...
- For QML-heavy exploration, check `docs/qml_navigation_index.md` or `docs/qml_navigation_index.json` for component names, properties, signals, functions, path aliases, local component refs, and dynamic constructs before searching `ea_node_editor\ui_qml`.
...
- When spawning an explorer, include map/index confirmation instructions unless the task is an exact narrow lookup. Explorer prompts should require the explorer to list route index entries checked, maps consulted, source/test/QML candidates used, weak or discarded candidates, and any justified broad searches.
...
- Exact-file or exact-symbol lookups in a known file family may skip route-index and map consultation, but the explorer prompt must explicitly say it is an exact lookup and why the map layer is unnecessary.
...
- Explorer model defaults:
  ...
  - `gpt-5.6-terra` with `xhigh`: light synthesis across a small map-bounded working set, usually a few files in one subsystem.
  - `gpt-5.6-sol` with `xhigh`: broad repository scans when the map layer returns many plausible owners, feature tracing, ownership or insertion-point decisions, cross-layer QML/Python/UI/graph/persistence work, ambiguous architecture tracing, or long-context synthesis.
- Spark explorer responses must be compact: direct answer, relevant files/symbols, then uncertainties or escalation note. Do not use Spark for ownership tracing, insertion-point choice, "find all uses", feature flow, context/action wiring, or cross-layer QML/Python work. Query/search first, sample snippets second, keep the working set tiny, return once supported, and stop/escalate to `gpt-5.6-sol` if the first route/index result fans out.
