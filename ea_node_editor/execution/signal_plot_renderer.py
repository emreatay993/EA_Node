# Purpose: Render evenly spaced signals with XY's native PNG backend.
...
from ea_node_editor.runtime_contracts import DataTree, ImageValue, Interval1D, TypedInlineValue
...
def _branches(value: object) -> tuple[np.ndarray, ...]:
    if isinstance(value, DataTree):
        raw = tuple(items for _path, items in value.branches)
    elif isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        raw = (
            (tuple(value),)
            if not value or not isinstance(value[0], Sequence)  # type: ignore[index]
            else tuple(value)
        )
    else:
        raise ValueError("Values must be a DataTree or numeric sequence")
    if not raw:
        raise ValueError("Values must contain at least one branch")
    branches: list[np.ndarray] = []
    for index, branch in enumerate(raw):
        try:
            array = np.asarray(branch, dtype=np.float64)
        except (TypeError, ValueError) as exc:
            raise ValueError(f"Values branch {index} must contain numeric datapoints") from exc
        if array.ndim != 1 or not len(array):
            raise ValueError(f"Values branch {index} must be a non-empty one-dimensional signal")
        branches.append(array)
...
def render_signal_plot(inputs: Mapping[str, Any]) -> tuple[ImageValue, tuple[str, ...]]:
    signals = _branches(inputs.get("values"))
    count = len(signals)
    ...
    if labels and len(labels) != count:
        raise ValueError("Labels must be empty or contain exactly one label per Values branch")
    ...
    components: list[Any] = [
        *marks,
        xy.x_axis(label=str(inputs.get("x_axis_label", "") or ""), bounds=x_bounds, style=axis_style),
        xy.y_axis(
    ...
    if labels and bool(inputs.get("show_legend", False)):
