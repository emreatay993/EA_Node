...
class DpfViewerSessionMaterializationBackend:
    ...
    def materialize(
        ...
        workspace_name = self._workspace_name(request)
        materialized = self._worker_services.dpf_runtime_service.materialize_viewer_dataset(
            ...
            node_workspace_id=request.workspace_id,
            node_workspace_name=workspace_name,
            node_id=request.node_id,
            node_title="DPF Viewer",
            node_type="DPF Viewer",
        )
    ...
    @staticmethod
    def _workspace_name(request: ViewerSessionMaterializationRequest) -> str:
        snapshots = [request.runtime_snapshot]
        context = request.runtime_snapshot_context
        if context is not None:
            snapshots.append(context.runtime_snapshot)
        for snapshot in snapshots:
            if snapshot is None:
                continue
            try:
                workspace = snapshot.workspace(request.workspace_id)
            except KeyError:
                continue
            return str(workspace.document_fields.get("name", "") or "").strip()
        return ""
