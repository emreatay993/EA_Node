...
def _dpf_operator_descriptor_specs() -> dict[str, NodeTypeSpec]:
    from ea_node_editor.addons.ansys_dpf.catalog import load_ansys_dpf_plugin_descriptors
...
class DpfRuntimeBase:
    ...
    def _lease_cached_ref_for_owner(
        self,
        ...
        *,
        ...
        if not str(run_id).strip() and not str(owner_scope).strip():
            return runtime_ref
        resolved_owner_scope = self._resolve_handle_owner_scope(
            run_id=run_id,
            owner_scope=owner_scope,
        )
        if runtime_ref.owner_scope == resolved_owner_scope:
            return runtime_ref
        if (
        ...
        return replace(runtime_ref, owner_scope=resolved_owner_scope)

    def _discard_cached_ref(
        self,
        cache: dict[str, RuntimeHandleRef],
        cache_key: str,
        *,
    ...
    def _resolve_runtime_handle_input(
        ...
        *,
        ...
        if (
            ...
            and runtime_ref.kind == DPF_MESH_HANDLE_KIND
        ):
        ...
        if (
            ...
            and runtime_ref.kind == DPF_MESH_SCOPING_HANDLE_KIND
        ):
        ...
        if (
            runtime_ref.data_type_id == DPF_SCOPING_DATA_TYPE
            and runtime_ref.kind == DPF_TIME_SCOPING_HANDLE_KIND
        ):
            if DPF_SCOPING_DATA_TYPE not in expected_type_set:
                return _UNMATERIALIZED_OPERATOR_INPUT
            return self._worker_services.resolve_handle(
        ...
        if (
            ...
            and runtime_ref.kind == DPF_RESULT_FILE_HANDLE_KIND
        ):
            if DPF_MODEL_DATA_TYPE not in expected_type_set:
                return _UNMATERIALIZED_OPERATOR_INPUT
        ...
        if (
            ...
            and runtime_ref.kind == DPF_OBJECT_HANDLE_KIND
        ):
            if (
                DPF_OBJECT_HANDLE_DATA_TYPE not in expected_type_set
                and runtime_ref.data_type_id not in expected_type_set
            ):
                return _UNMATERIALIZED_OPERATOR_INPUT
            return self._worker_services.resolve_handle(
                runtime_ref,
    ...
    def _time_scoping_location(self, model: Any | None) -> str:
        ...
        # Set ids across the app are cumulative set numbers (1..n_sets), so time
        # scopings must use the cumulative-set location. The model's own
        # time_frequencies scoping reports the step-based location
        # ("TimeFreq_steps"), which mis-reads set ids as load-step numbers for
        # any file with more than one substep per step (e.g. modal results).
        locations = getattr(self._dpf_module(), "locations", None)
        return str(getattr(locations, "time_freq", DEFAULT_TIME_SCOPING_LOCATION))
    ...
    def _resolve_model_handle_and_object(self, value: Any) -> tuple[RuntimeHandleRef, Any]:
        ...
        model = self._worker_services.resolve_handle(
            model_ref,
            expected_data_type=DPF_MODEL_DATA_TYPE,
            expected_kind=DPF_MODEL_HANDLE_KIND,
        )
    ...
    def _resolve_mesh_object(self, value: Any | None, *, model: Any) -> Any:
        ...
        if (
            runtime_ref is None
            or runtime_ref.data_type_id != DPF_MESH_DATA_TYPE
    ...
    def _build_fields_container_metadata(
        ...
        *,
        ...
        metadata.update(compact_sequence_metadata("set_ids", set_ids))
        if result_name:
        ...
        if model is not None:
            time_values = self._metadata_time_values_for_sets(
                model,
                list(set_ids),
                len(fields_container),
            )
            if time_values:
                metadata.update(
                    compact_sequence_metadata("time_values", time_values)
                )
        value_ranges = self._fields_container_value_ranges(fields_container)
        if value_ranges:
            metadata.update(
                compact_sequence_metadata("value_ranges", value_ranges)
            )
        return metadata

    @staticmethod
    def _metadata_time_values_for_sets(model: Any, set_ids: list[int], field_count: int) -> list[float]:
        try:
            frequencies = list(model.metadata.time_freq_support.time_frequencies.data)
        except Exception:  # noqa: BLE001
            return []
        resolved_set_ids = list(set_ids) if len(set_ids) == field_count else list(range(1, field_count + 1))
        ...
        for set_id in resolved_set_ids:
            ...
            if index < 0 or index >= len(frequencies):
                return []
            try:
                time_values.append(float(frequencies[index]))
            except (TypeError, ValueError):
                return []
        return time_values

    @staticmethod
    def _fields_container_value_ranges(fields_container: Any) -> list[dict[str, Any]]:
        import numpy as np
        ...
        for index in range(len(fields_container)):
            try:
                ...
                if data.size == 0:
                    return []
                component_count = max(1, int(getattr(field_value, "component_count", 1)))
                if component_count > 1:
                    ...
                    entry = {
                        ...
                        "component_min": [float(v) for v in data.min(axis=0)],
                        "component_max": [float(v) for v in data.max(axis=0)],
                    }
                else:
                    ...
                    entry = {
                        ...
                        "component_min": [float(flattened.min())],
                        "component_max": [float(flattened.max())],
                    }
            except Exception:  # noqa: BLE001
                return []
