...
def _resolve_runtime_ref(
    ...
    *,
    ...
    if allowed_data_types:
        if value.data_type_id not in allowed_data_types:
            raise TypeError(
...
def _execute_data_sources_constructor(ctx: ExecutionContext) -> NodeResult:
    ...
    data_sources_ref = _register_object_handle(
        ...
        metadata={"has_result_path": result_path is not None, "key": key},
    )
    return NodeResult(outputs={"data_sources": data_sources_ref})
...
def _execute_data_sources_set_result_file_path(ctx: ExecutionContext) -> NodeResult:
    ...
    return NodeResult(outputs={"updated_receiver": data_sources_ref})
...
def _execute_streams_container_constructor(ctx: ExecutionContext) -> NodeResult:
    ...
    return NodeResult(outputs={"streams_container": streams_ref})
...
def _execute_workflow_constructor(ctx: ExecutionContext) -> NodeResult:
    ...
    return NodeResult(outputs={"workflow": workflow_ref})
...
def _execute_over_time_freq_fields_container(ctx: ExecutionContext) -> NodeResult:
    ...
    return NodeResult(outputs={"fields_container": fields_container_ref})
...
def _execute_scoping_on_all_time_freqs(ctx: ExecutionContext) -> NodeResult:
    ...
    if (
        receiver_value.data_type_id == DPF_MODEL_DATA_TYPE
        and receiver_value.kind == DPF_MODEL_HANDLE_KIND
    ):
        ...
        receiver_value.data_type_id == DPF_DATA_SOURCES_DATA_TYPE
        and receiver_value.kind == DPF_OBJECT_HANDLE_KIND
    ):
        receiver = ctx.resolve_handle(
            receiver_value,
            expected_data_type=DPF_DATA_SOURCES_DATA_TYPE,
            expected_kind=DPF_OBJECT_HANDLE_KIND,
        )
                ...
                ports=(
                    PortSpec(
                        ...
                        source_metadata=_port_source(
                            ...
                            direction="in",
                            value_key="arr",
                            data_type='COREX.DataTypes.Json',
                            presence="required",
                            omission_semantics="disallowed",
