...
@builtin_node_type(
    ...
    category_path=DPF_ADVANCED_BUILDING_BLOCKS_CATEGORY_PATH,
    description="Normalizes a Mechanical result-file path and returns a worker-local DPF handle.",
    ports=(
        PortSpec("path", "in", "data", 'COREX.DataTypes.Path', required=True, uses_property_default=True),
...
class DpfResultFileNodePlugin:
    def execute(self, ctx) -> NodeResult:  # noqa: ANN001
        ...
        result_ref = require_dpf_runtime_service(
            ctx,
            node_name="DPF Result File",
        ).load_result_file(result_path, run_id=ctx.run_id)
        return NodeResult(
            outputs={
...
@builtin_node_type(
    ...
    ports=(
        ...
        PortSpec("path", "in", "data", 'COREX.DataTypes.Path', required=False, uses_property_default=True),
    ...
    readiness_requirements=(
        ReadinessRequirementSpec(any_of_ports=("result_file", "path")),
    ),
)
class DpfModelNodePlugin:
    def execute(self, ctx) -> NodeResult:  # noqa: ANN001
        normalize_dpf_output_mode(ctx.properties.get("output_mode"))
        source_value = source_value_from_ctx(
            ctx,
            result_file_key="result_file",
            path_key="path",
            node_name="DPF Model",
        )
        model_ref = require_dpf_runtime_service(
            ctx,
            node_name="DPF Model",
        ).load_model(source_value, run_id=ctx.run_id)
        return NodeResult(outputs={"model": model_ref})
...
@builtin_node_type(
    ...
    readiness_requirements=(
        ...
        ReadinessRequirementSpec(
            any_of_properties=("named_selection",),
            when_properties=(
                PropertyConditionSpec(
                    "selection_mode",
                    (DPF_MESH_SELECTION_NAMED_SELECTION,),
...
class DpfMeshScopingNodePlugin:
    def execute(self, ctx) -> NodeResult:  # noqa: ANN001
        ...
        if selection_mode == DPF_MESH_SELECTION_NAMED_SELECTION:
            ...
            return NodeResult(outputs={"scoping": scoping_ref})
...
@builtin_node_type(
    ...
    readiness_requirements=(
        ReadinessRequirementSpec(any_of_properties=("set_ids", "time_values")),
        ReadinessRequirementSpec(
            any_of_ports=("model",),
            when_properties=(PropertyConditionSpec("time_values"),),
        ),
    ),
)
...
class DpfResultFieldNodePlugin:
    def execute(self, ctx) -> NodeResult:  # noqa: ANN001
        ...
        model_ref, model = require_model_input(ctx, node_name="DPF Result Field")
        ...
        field_ref = clone_field_handle_with_metadata(
            ...
            source_metadata={
                **field_ref.metadata,
                ...
                "set_ids": [active_set.set_id],
                **(
                    {"time_value": active_set.time_value, "time_values": [active_set.time_value]}
                    if active_set.time_value is not None
                    else {}
                ),
                **(
                    {"mesh_scoping_handle_id": ctx.inputs["mesh_scoping"].handle_id}
                    if hasattr(ctx.inputs.get("mesh_scoping"), "handle_id")
                    else {}
                ),
                **(
                    {"time_scoping_handle_id": active_set.time_scoping_ref.handle_id}
                    if active_set.time_scoping_ref is not None
                    else {}
                ),
            },
            release_original=True,
        )
        return NodeResult(outputs={"field": field_ref})
...
@builtin_node_type(
    ...
    ports=(
        PortSpec("field", "in", "data", DPF_FIELD_DATA_TYPE, required=True),
        PortSpec("model", "in", "data", DPF_MODEL_DATA_TYPE, required=False),
    ...
    def execute(self, ctx) -> NodeResult:  # noqa: ANN001
        ...
        try:
            ...
            if operation == DPF_FIELD_OP_CONVERT_LOCATION:
                model_ref, _ = require_model_input(ctx, node_name="DPF Field Ops")
...
class DpfMeshExtractNodePlugin:
    def execute(self, ctx) -> NodeResult:  # noqa: ANN001
        normalize_dpf_output_mode(ctx.properties.get("output_mode"))
        model_ref, _ = require_model_input(ctx, node_name="DPF Mesh Extract")
        ...
        return NodeResult(outputs={"mesh": mesh_ref})
...
@builtin_node_type(
    ...
    ports=(
        ...
        PortSpec("dataset", "out", "data", 'COREX.DataTypes.Any', exposed=True),
        PortSpec("exports", "out", "data", 'COREX.DataTypes.Any', exposed=True),
        PortSpec("csv", "out", "data", 'COREX.DataTypes.Path', exposed=True),
        PortSpec("png", "out", "data", 'COREX.DataTypes.Path', exposed=True),
        PortSpec("vtu", "out", "data", 'COREX.DataTypes.Path', exposed=True),
        PortSpec("vtm", "out", "data", 'COREX.DataTypes.Path', exposed=True),
    ),
    properties=(
...
class DpfExportNodePlugin:
    def execute(self, ctx) -> NodeResult:  # noqa: ANN001
        ...
        model_ref, _ = require_model_input(ctx, node_name="DPF Export")
        ...
        try:
            ...
            if output_mode != DPF_OUTPUT_MODE_MEMORY:
                ...
                artifact_key = normalize_export_artifact_key(
                    ...
                    fallback=default_export_artifact_key(ctx, field_ref),
                )
