...
def _construct_inputs() -> tuple[PortSpec, ...]:
    rows = (
        ("camera_position", POINT_3D_DATA_TYPE_ID, "Camera Position"),
        ("camera_target", POINT_3D_DATA_TYPE_ID, "Camera Target"),
        ("camera_up_vector", VECTOR_3D_DATA_TYPE_ID, "Camera Up Vector"),
        ("display_mode", INTEGER_DATA_TYPE_ID, "Display Mode"),
        ("projection_mode", INTEGER_DATA_TYPE_ID, "Projection Mode"),
        ("show_mesh_edges", BOOLEAN_DATA_TYPE_ID, "Show Mesh Edges"),
        (
            "show_attribute_colors",
            BOOLEAN_DATA_TYPE_ID,
            "Show Attribute Colors",
        ),
        ("show_triad", BOOLEAN_DATA_TYPE_ID, "Show Triad"),
        ("show_view_cube", BOOLEAN_DATA_TYPE_ID, "Show View Cube"),
        ("show_axes", BOOLEAN_DATA_TYPE_ID, "Show Axes"),
    )
    return tuple(
        PortSpec(
            key,
        ...
        for key, data_type_id, label in rows
    )


def _construct_properties() -> tuple[PropertySpec, ...]:
    return (
        PropertySpec(
            "camera_position",
            "json",
            ...
            persistence_data_type_id=POINT_3D_DATA_TYPE_ID,
        ),
        PropertySpec(
            "camera_target",
            "json",
        ...
        PropertySpec(
            ...
            "Camera Up Vector",
            inspector_visible=False,
            persistence_data_type_id=VECTOR_3D_DATA_TYPE_ID,
        ),
        PropertySpec(
            "display_mode",
            "int",
...
@node_type(
    type_id=CONSTRUCT_VIEW_TYPE_ID,
    display_name="Construct View",
    category_path=("Utilities", "Viewer"),
    icon="visibility",
    ...
    properties=_construct_properties(),
)
class ConstructViewNodePlugin:
...
def _deconstruct_outputs() -> tuple[PortSpec, ...]:
    rows = (
        ("camera_position", POINT_3D_DATA_TYPE_ID, "Camera Position"),
        ("camera_target", POINT_3D_DATA_TYPE_ID, "Camera Target"),
        ("camera_up_vector", VECTOR_3D_DATA_TYPE_ID, "Camera Up Vector"),
...
@node_type(
    type_id=DECONSTRUCT_VIEW_TYPE_ID,
    display_name="Deconstruct View",
    category_path=("Utilities", "Viewer"),
    icon="visibility",
    ...
    ports=(
        ...
        *_deconstruct_outputs(),
    ),
    properties=(),
)
class DeconstructViewNodePlugin:
    def execute(self, ctx: ExecutionContext) -> NodeResult:
        payload = _require_viewport(ctx.inputs["viewport"])
        return NodeResult(
            outputs={
...
COREX_VIEWER_VIEWPORT_NODE_DESCRIPTORS = (
    plugin_descriptor(ConstructViewNodePlugin),
    plugin_descriptor(DeconstructViewNodePlugin),
)
...
__all__ = [
    ...
    "ConstructViewNodePlugin",
    "DECONSTRUCT_VIEW_TYPE_ID",
    "DEFAULT_CAMERA_POSITION",
    "DEFAULT_CAMERA_TARGET",
    "DEFAULT_CAMERA_UP_VECTOR",
    "DeconstructViewNodePlugin",
