...
DPF_ADVANCED_BUILDING_BLOCKS_FAMILY_PATH = (*DPF_ADVANCED_FAMILY_PATH, "Building Blocks")
...
DPF_ADVANCED_BUILDING_BLOCKS_CATEGORY_PATH = dpf_category_path(
    *DPF_ADVANCED_BUILDING_BLOCKS_FAMILY_PATH
)
...
def operator_family_path(family: str) -> tuple[str, ...]:
    # Every raw operator mirror lives under Advanced so the default library
    # surface stays workflow-first; the family only picks the leaf segment.
    display_name = operator_family_display_name(_normalized_token(family))
    return (*DPF_RAW_API_MIRROR_FAMILY_PATH, display_name)


def operator_family_category_path(family: str) -> CategoryPath:
    return dpf_category_path(*operator_family_path(family))
