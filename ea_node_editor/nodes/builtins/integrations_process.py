...
def _discard_stored_transcripts(
    ...
    try:
        store.discard_staged_entries(
            tuple(target.artifact_id for target in targets)
        )
    except BaseException as error:
        cleanup_error = error
    try:
        store.discard_staged_paths(
            tuple(target.relative_path for target in targets)
        )
    except BaseException as error:
    ...
    category_path=("Input / Output",),
            ...
            ports=(
                PortSpec(
                    ...
                    data_access="tree",
                    description="Executable or shell command; overrides the configured Command property.",
                ),
                PortSpec(
                    "args",
                    "in",
            ...
            properties=(
                ...
                PropertySpec("termination_grace_sec", "float", 0.6, "Termination Grace (sec)"),
                PropertySpec("core_hint", "int", 0, "Core Hint"),
                PropertySpec("thread_hint", "int", 0, "Thread Hint"),
                PropertySpec("memory_mb_hint", "int", 0, "Memory Hint (MB)"),
        ...
        try:
            ...
            while True:
                ...
                if elapsed > policy.timeout_sec:
                    _cancel()
                    raise TimeoutError(f"Process Run timed out after {policy.timeout_sec:.2f} seconds.")
