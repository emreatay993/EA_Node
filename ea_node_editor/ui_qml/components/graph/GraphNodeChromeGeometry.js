...
function normalized(width, height, radius, centers, holeRadius, inset) {
    ...
    ["left", "right"].forEach(function(side) {
        var x = side === "left" ? 0 : width;
        var values = centers[side] || [];
        for (var i = 0; i < values.length; ++i) {
            var y = Number(values[i]);
            if (!isFinite(y) || y + r <= box.top || y - r >= box.bottom)
                continue;
            var duplicate = box.holes.some(function(hole) {
                return Math.abs(hole.x - x) <= box.epsilon && Math.abs(hole.y - y) <= box.epsilon;
            });
            if (!duplicate)
...
function uniqueCuts(cuts) {
    ...
    return cuts.filter(function(value, index) { return index === 0 || value - cuts[index - 1] > PARAM_EPS; });
}
...
function retainedSegments(box) {
    ...
    var holes = box.holes.map(function(hole) { return arc(hole.x, hole.y, hole.radius, 0, -TAU); });
    var edges = outer.concat(holes);
    var boxes = edges.map(bounds);
    ...
    edges.forEach(function(edge, index) {
        ...
        for (var cut = 1; cut < cuts.length; ++cut) {
            ...
            if (index >= outer.length && !insideCard(box, middle))
                continue;
            if (box.holes.some(function(hole, holeIndex) {
                return holeIndex !== index - outer.length && insideHole(hole, middle, box.epsilon);
            }))
                continue;
...
function silhouette(box) {
    var remaining = retainedSegments(box);
    var paths = [];
    while (remaining.length) {
        ...
        while (!near(end, start, box.epsilon)) {
            var next = remaining.findIndex(function(candidate) { return near(candidate.start, end, box.epsilon); });
            ...
            segment = remaining.splice(next, 1)[0];
