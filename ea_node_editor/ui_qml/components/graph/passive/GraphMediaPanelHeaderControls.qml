...
Item {
    ...
    readonly property real badgeY: {
        if (!(root.surface && root.surface.host))
            return 6;
        if (root.surface.chromeToggleAvailable && !root.surface.chromeTitleVisible)
            return Math.max(6, Number(root.surface.contentTopMargin || 0));
        return Number(root.surface.host.surfaceMetrics.title_top || 0)
            + Math.max(0, (Number(root.surface.host.surfaceMetrics.title_height || 24) - issueBadge.height) * 0.5);
    }
