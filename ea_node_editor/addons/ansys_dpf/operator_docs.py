...
_CURATED_WORKFLOWS_DIR = "workflow_docs"
_DOC_INDEX_FILENAME = "doc_index.json"
_DPF_OPERATOR_TYPE_ID_PREFIX = "dpf.op."
_CURATED_WORKFLOW_DOCS = {
    "dpf.workflow.result_source": "result-source.md",
    "dpf.workflow.result_fields": "result-fields.md",
    "dpf.workflow.result_viewer": "result-viewer.md",
    "dpf.workflow.min_max_envelope": "min-max-envelope.md",
    "dpf.workflow.time_history_probe": "time-history-probe.md",
    "dpf.workflow.stress_invariants": "stress-invariants.md",
    "dpf.workflow.mode_shape_viewer": "mode-shape-viewer.md",
    "dpf.workflow.field_math": "field-math.md",
    "dpf.workflow.table_export": "table-export.md",
}
...
def is_dpf_workflow_type_id(type_id: str | None) -> bool:
    return bool(type_id) and str(type_id) in _CURATED_WORKFLOW_DOCS


def is_dpf_help_type_id(type_id: str | None) -> bool:
    return is_dpf_workflow_type_id(type_id) or is_dpf_operator_type_id(type_id)
...
def markdown_path_for_type_id(type_id: str, lookup: _SpecLookup) -> Path | None:
    curated_filename = _CURATED_WORKFLOW_DOCS.get(str(type_id))
    if curated_filename:
        return Path(__file__).resolve().with_name(_CURATED_WORKFLOWS_DIR) / curated_filename
...
def markdown_for_node(host: Any, node_id: str) -> tuple[str, str, str] | None:
    ...
    if not is_dpf_help_type_id(spec.type_id):
        return None
