...
class ProjectSessionController:
    ...
    def build_project_files_snapshot(
        ...
        *,
        ...
        resolved_project_path = self._normalize_project_path(
    ...
    def prompt_recover_autosave(self, recovered_project: ProjectData | None = None) -> QMessageBox.StandardButton:
        from PyQt6.QtWidgets import QMessageBox

        snapshot = self.build_project_files_snapshot(
            project=recovered_project,
            project_path=self._host.project_path,
        ) if recovered_project is not None else self.build_project_files_snapshot()
