...
class ProjectDocumentIOService:
    ...
    def _finalize_loaded_project(
        self, project: ProjectData, *, project_path: str
    ) -> None:
    ...
    def _reset_save_as_destination_sidecar(self, saved_path: Path) -> None:
        ...
        self._delete_save_as_sidecar_path(layout.workspaces_root)
        self._delete_save_as_sidecar_path(layout.nodes_root)
        ...
        *,
        ...
        for artifact_id in normalized_managed_ids:
            ...
            if managed_entry is None:
                continue
            destination_entry = source_store.workspace_scoped_managed_entry(
                managed_entry,
                workspaces=workspaces,
            )
            destination_metadata["artifacts"][artifact_id] = (
                destination_entry.to_metadata_entry()
            )
            ...
            if source_path is None or not source_path.exists():
                continue
            destination_path = destination_layout.absolute_path_for_relative(
                destination_entry.relative_path
            )
            self._copy_save_as_payload(source_path, destination_path)
        return destination_metadata

    def _copy_save_as_staged_payloads(
        self,
        *,
        saved_path: Path,
        ...
        destination_layout = ProjectArtifactLayout.from_project_path(saved_path)
        destination_staged = destination_metadata.setdefault("staged", {})
        if not isinstance(destination_staged, dict):
            destination_staged = {}
            destination_metadata["staged"] = destination_staged
        normalized_staged_ids = sorted(
            {
                str(artifact_id).strip()
                for artifact_id in referenced_staged_ids
                if str(artifact_id).strip()
            }
        )
        for artifact_id in normalized_staged_ids:
            staged_entry = source_store.staged_entry(artifact_id)
            if staged_entry is None:
                continue
            destination_entry = source_store.workspace_scoped_staged_entry(
                staged_entry,
                workspaces=workspaces,
            )
            destination_staged[artifact_id] = destination_entry.to_metadata_entry()
            if not staged_entry.relative_path:
                continue
            source_path = source_store.resolve_staged_path(artifact_id)
            if source_path is None or not source_path.exists():
                continue
            if not destination_entry.relative_path:
                continue
            destination_path = destination_layout.absolute_path_for_relative(
                destination_entry.relative_path
            )
    ...
    def save_project(self) -> None:
        ...
        if not path:
            path, _ = QFileDialog.getSaveFileName(
                self._dialog_parent(), "Save Project", "", "COREX Project (*.cxproj)"
            )
        if not path:
            return
        saved_path = Path(path).with_suffix(".cxproj")
        self._workspace_session.save_active_view_state()
        self.persist_script_editor_state()
        persistent_document = self._host.serializer.to_persistent_document(
            self._host.model.project
        )
        ...
        store.migrate_workspace_artifact_folders(
    ...
    def save_project_as(self) -> None:
        ...
        destination_store_metadata = self._build_save_as_artifact_store_metadata(
        ...
        self._copy_save_as_staged_payloads(
            saved_path=saved_path,
            source_store=source_store,
            destination_metadata=destination_store_metadata,
            workspaces=self._host.model.project.workspaces,
            referenced_staged_ids=artifact_refs.staged_ids,
        )
        destination_store = ProjectArtifactStore(
            project_path=saved_path,
            metadata=destination_store_metadata,
        )
        destination_store.migrate_workspace_artifact_folders(
            workspaces=self._host.model.project.workspaces
        )
        promotion = destination_store.commit_referenced_artifacts(
            referenced_managed_ids=artifact_refs.managed_ids,
            referenced_staged_ids=artifact_refs.staged_ids,
        )
        updated_document = rewrite_project_artifact_refs(
        ...
        document_metadata[PROJECT_ARTIFACT_STORE_METADATA_KEY] = (
            destination_store.metadata
        )
        updated_document["metadata"] = document_metadata
        ...
        self._host.model.project.replace_metadata(document_metadata)
        self._host.project_path = str(saved_path)
        try:
            self._session._session_state.last_manual_save_ts = (
                saved_path.stat().st_mtime
    ...
    def open_project(self) -> bool:
        ...
        if not path:
            return False
        return self.open_project_path(path, show_errors=True)
