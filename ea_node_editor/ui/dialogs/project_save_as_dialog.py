...
class ProjectSaveAsDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        header = QLabel("Choose how project files should be handled for the new project path.", self)
        ...
        project_only_details = QLabel(
            "Saved files are not copied. Existing saved-file links stay in the project and may be unresolved "
            "until their files are copied into the new sidecar later. Referenced temporary files are still saved "
            "into the new project's node folders. External file links stay external.",
            self,
        )
    ...
    def _copy_option_label(cls, managed_count: int) -> str:
        if managed_count == 1:
            return "Create a self-contained copy and copy 1 referenced saved file"
        return f"Create a self-contained copy and copy {managed_count} referenced saved files"

    @classmethod
    def _copy_option_details(cls, *, managed_count: int, staged_count: int) -> str:
        if managed_count:
            managed_clause = "Referenced saved files are copied into the new sibling .data sidecar."
        else:
            managed_clause = "No referenced saved files are currently recorded."
        if staged_count == 1:
            staging_clause = "1 referenced temporary file is saved into the new project's node folders."
        elif staged_count > 1:
            staging_clause = f"{staged_count} referenced temporary files are saved into the new project's node folders."
        else:
            staging_clause = "No temporary files are currently referenced."
