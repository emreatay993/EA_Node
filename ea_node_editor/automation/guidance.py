...
def guide_text() -> str:
    ...
    lines: list[str] = [
        ...
        "   (every lane keeps what it holds), `swimlane_remove_lane` (its neighbour takes its band and nodes) and",
        "   `swimlane_describe` (lanes and what they hold) manage lanes; rename a lane with node_update(title=...).",
...
def node_types_text() -> str:
    ...
    lines = [
        ...
        "- `passive.annotation.swimlane_pool`: `title`, `orientation` (horizontal|vertical); a pool stacks its",
        "  `passive.annotation.swimlane_lane` lanes (`title` = role name, no ports; each lane is a Group backdrop, so",
        "  a node inside a lane belongs to it and moves with it). Create both with `swimlane_create_pool`, not",
