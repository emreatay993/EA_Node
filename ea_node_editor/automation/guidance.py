...
def node_types_text() -> str:
    ...
    lines = [
        ...
        "- `body_fit` (not on timestamp): `clip` (default; clipped text ends in an ellipsis), `grow` (the shape",
        "  grows to fit `body` as one undoable resize; square shapes such as connector keep their aspect), or",
        "  `shrink` (the font steps down to fit, 6 px at the smallest). The canvas applies `grow` once the shape",
        "  is drawn, so frame the node and read its size back after it settles.",
