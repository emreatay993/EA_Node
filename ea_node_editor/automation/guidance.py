...
def server_instructions() -> str:
    return "\n".join(
        (
            ...
            "   Tidy with layout_tidy() first: it lays the scope (or node_ids) out from the wires with straight rows",
            "   and columns, keeps Group backdrops intact and lays swimlane pools out lane by lane.",
            "   Use layout_arrange / layout_straighten for fine control.",
...
def guide_text() -> str:
    ...
    lines: list[str] = [
        ...
        "   Swimlanes (who does what): `swimlane_create_pool(x, y, title, lanes=[\"Customer\", \"Sales\"],",
        "   orientation=\"horizontal\"|\"vertical\")` returns `pool_node_id` and `lane_node_ids` in stack order",
        "   (a pool holds two or more lanes; `swimlane_create_lane` makes one lane on its own, and",
        "   `swimlane_add_lane(lane_node_id=..., after=...)` next to it forms a pool around both);",
        "   put each step in its role's lane with `swimlane_assign(node_ids, lane_node_id)` (or node_add inside the",
        "   lane's rectangle), wire the steps, then `layout_tidy(node_ids=[pool_node_id])`: layers along the flow,",
        "   one row per lane (a column per lane in a vertical pool). `swimlane_add_lane`, `swimlane_move_lane`",
        "   (every lane keeps what it holds), `swimlane_remove_lane` (its neighbour takes its band and nodes; a pool",
        "   left with one lane dissolves) and `swimlane_describe` (pools, standalone lanes, what they hold) manage",
        "   lanes; rename a lane with node_update(title=...), colour it with node_update(properties={\"color\":",
        "   \"#rrggbb\"}).",
        "7. Tidy -- `layout_tidy()` first (no ids = the whole open scope; or `node_ids`): it rebuilds the layout from",
        "   the wires with rows and columns centered so right->left and bottom->top wires run straight, detects the",
        "   direction (or pass `direction=\"left_to_right\"|\"top_to_bottom\"`), keeps loop-back wires as elbows",
        "   (`loop_edge_ids`), and keeps Group backdrops intact (members stay inside, backdrops are refitted).",
        "   `mode=\"in_place\"` keeps your arrangement and only snaps rows and columns onto shared centers. NO_EFFECT",
        "   with `details.membership_conflict_node_ids` means the result would move nodes into or out of a Group:",
        "   add that Group's backdrop to `node_ids` so it moves as one block, or tidy its members on their own. For",
        "   fine control use `layout_arrange(node_ids, action=\"align_center_y\")` (one row center so side ports",
        "   meet; `align_center_x` for a column; also align_left|right|top|bottom, distribute_horizontal|vertical,",
        "   match_width|height) and `layout_straighten()` (node_ids / edge_ids), which only moves nodes so wires run",
        "   straight. Read `skipped_edges`: `mixed_port_sides` is an elbow such as right->top (reconnect to opposite",
        "   sides if it should be straight); `unresolved_offset` means the ends still differ by `offset` px (move a",
        "   node with node_update); `locked_node` / `hidden_in_collapsed_group` ends are never moved. `skipped_nodes`",
        "   lists nodes left alone (`locked_group`: blocked by a Group that is locked or holds a locked node). Check",
        "   `overlapping_node_pairs` and distribute or move nodes apart.",
...
def node_types_text() -> str:
    ...
    lines = [
        ...
        "- text fit (every shape except timestamp): `node_fit_text(node_id, mode=clip|grow|shrink)` stores",
        "  `body_fit` and reports how the body fits once the canvas has drawn the shape. `clip` (default) ends",
        "  clipped text in an ellipsis, `grow` enlarges the shape until the text fits in the same undo step",
        "  (square shapes such as connector keep their aspect), `shrink` lowers the font to 6 px at most. Check",
        "  `text_fit.overflowing`; omit `mode` to only check. Frame an off-screen shape with `view_set_camera`",
        "  first, or the call times out and changes nothing.",
        ...
        "- `passive.annotation.group_backdrop`: `title` only; expanded Groups own what lies inside them, collapsed Groups keep",
        "  the members they held. Prefer `group_wrap(node_ids, title)` so the backdrop is sized around the members.",
        "- `passive.annotation.swimlane_lane` (the Library's Swimlane): `title` = role name, `orientation`",
        "  (horizontal|vertical), `color` (#rrggbb, empty for the theme's); no ports. A lane is a Group backdrop, so a",
        "  node inside it belongs to it and moves with it. `passive.annotation.swimlane_pool` (`title`,",
        "  `orientation`) stacks two or more lanes; it forms around a lane when a lane is added next to it and",
        "  dissolves when one lane is left. Create them with `swimlane_create_pool` / `swimlane_create_lane`, not",
        "  node_add: lanes are sized, stacked and kept inside their pool for you.",
        "",
        ...
        "- `node_update(node_id, x=..., y=..., width=..., height=...)` moves and resizes. Moving a Group moves",
        "  everything inside it (returned as `carried_node_ids`; do not move those nodes yourself); pass",
        "  `move_contents=false` to move or reshape only its frame. Resizing a lane resizes its pool and the other",
        "  lanes with it (lanes never shrink past what they hold); moving a pool moves its lanes and their nodes;",
        "  reorder lanes with `swimlane_move_lane`, not by moving them. `layout_tidy` lays a whole",
        "  flow (or `node_ids`) out from its wires and keeps Groups intact; for fine control `layout_arrange` aligns,",
        "  distributes, and matches sizes and `layout_straighten` makes wires straight.",
        "- Default heights differ by shape (process 84, decision 128; read `default_size`). Nodes at the same `y`",
        "  align their top edges, so side ports sit at different heights and straight connectors jog.",
        "  `layout_tidy()` centers every row and column at once; for a single row use",
    ...
    if spec["name"] == "build_flowchart":
        ...
        return "\n".join(
            (
                ...
                "4. Tidy: layout_tidy() lays the flow out from its wires (straight rows and columns, loops kept as",
                "   elbows). Read skipped_edges and overlapping_node_pairs; use layout_arrange / layout_straighten",
                "   for fine control.",
