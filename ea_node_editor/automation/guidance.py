...
def server_instructions() -> str:
    return "\n".join(
        (
            ...
            "10. Retry APP_BUSY, APP_BUSY_MODAL, and TIMEOUT after a short pause; every other error carries a hint",
...
def guide_text() -> str:
    ...
    lines: list[str] = [
        ...
        "   `scope_navigate(target=\"root\")` to return); `layout_arrange` aligns or distributes; `selection_set`",
        "   drives the canvas selection.",
        "7. Annotate -- `node_add_text` for markdown blocks, `node_add_media` for images/PDF/video (staged into the",
        ...
        "8. `project_save(path=\"C:/.../name.cxproj\")` -- a new project needs a path once (reason_code",
        "   `save_path_required`); afterwards `project_save()` saves in place and publishes staged files.",
        "9. `capture_screenshot` -- mode=views renders the active view cropped to content and returns the PNG inline;",
        ...
        "- Mutations during a run return RUN_ACTIVE; wait or stop first.",
        "",
        "## Error codes",
        "",
    ...
    lines.extend(
        (
            ...
            "## Retry rules",
            "",
            "- `APP_BUSY` (project IO) and `TIMEOUT`: wait 1-2 s and resend the same request; raise `timeout_s` for",
            "  long waits.",
    ...
    if spec["name"] == "build_flowchart":
        ...
        return "\n".join(
            (
                ...
                "4. Optionally style with node_set_style (see corex://styles) and wrap phases with group_wrap.",
                "5. Finish with capture_screenshot and describe what the board shows.",
            )
        )
