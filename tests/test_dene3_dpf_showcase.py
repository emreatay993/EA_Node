...
class Dene3DpfShowcaseTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.project_path = cls.repo_root / "dene3.cxproj"
    ...
    def _workspace_node_id_by_type(self, workspace: Any, type_id: str) -> str:
        matches = [
            node.node_id for node in workspace.nodes.values() if node.type_id == type_id
        ]
        self.assertEqual(
            len(matches),
            1,
    ...
    def test_saved_dene3_showcase_workspaces_run_from_project_path(self) -> None:
        self.assertTrue(
            self.project_path.exists(), f"Missing showcase file: {self.project_path}"
        )
        project = self.serializer.load(str(self.project_path))

        workspace_by_name = {
            workspace.name: workspace for workspace in project.workspaces.values()
        }
        ...
        for workspace_name in expected_names:
            self.assertIn(
                workspace_name,
                workspace_by_name,
                f"Missing showcase workspace {workspace_name!r}",
            )
        ...
        for expectation in _SHOWCASE_WORKSPACES:
            ...
            final_node_id = self._workspace_node_id_by_type(
                workspace, expectation["final_type"]
            )
            ...
            with self.subTest(workspace=workspace.name):
                ...
                failed_settlements = [
                    ...
                    for node_id, event in settled_by_node_id.items()
                    if str(event.get("status", "")) in {"failed", "blocked"}
                ]
                self.assertFalse(
                    failed_settlements,
                    f"Workspace {workspace.name!r} emitted failed or blocked node results: "
                    f"{failed_settlements!r}",
                )
                ...
                final_settlement = settled_by_node_id[final_node_id]
                self.assertEqual(final_settlement.get("status"), "completed")
                outputs = dict(final_settlement.get("outputs", {}))
                self.assertIn(
                    expectation["output_key"],
                    outputs,
                    f"Workspace {workspace.name!r} final node outputs were {outputs!r}",
                )
                result = outputs[expectation["output_key"]]
                self.assertIsInstance(result, dict)
                self.assertEqual(result.get("status"), "value")
                tree = deserialize_runtime_value(
                    result["value"],
                    catalog=self.registry.data_types,
                )
                self.assertIsInstance(tree, DataTree)
                self.assertGreater(tree.item_count, 0)
