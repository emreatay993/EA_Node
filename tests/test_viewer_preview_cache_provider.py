...
def test_viewer_preview_cache_provider_serves_revisioned_qimages(qapp) -> None:  # noqa: ANN001
    ...
    assert source_v1.startswith("image://viewer-preview-cache/preview?")
    assert "revision=1" in source_v1

    original.fill(QColor("#ff0000"))
    cached_v1, size_v1 = provider.requestImage(source_v1.split("image://viewer-preview-cache/", 1)[1], QSize())
    assert size_v1 == QSize(5, 3)
    assert cached_v1.pixelColor(0, 0) == QColor("#3366ff")
    assert provider.preview_signature("workspace one", "node/a") == ("viewer", 1)
    ...
    cached_v2, _size_v2 = provider.requestImage(source_v2.split("image://viewer-preview-cache/", 1)[1], QSize())
...
def test_viewer_preview_cache_provider_does_not_double_decode_node_ids(qapp) -> None:  # noqa: ANN001
    provider = ViewerPreviewCacheImageProvider()
    ...
    assert provider.set_preview(
        ...
        "node/a",
        _image("#ff3333"),
        signature=("viewer", "slash"),
    )
    assert provider.set_preview(
        "workspace one",
        "node%2Fa",
        _image("#3366ff"),
        signature=("viewer", "percent"),
    )
