...
class IntegrationNodesTrackFTests(unittest.TestCase):
    ...
    def test_email_send_validation_and_error_wrapping(self) -> None:
        ...
        message = str(missing_error.exception).lower()
...
class IntegrationFlowSmokeTests(unittest.TestCase):
    ...
    def test_smoke_excel_input_python_transform_excel_output(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            end = model.add_node(ws.workspace_id, "core.end", "End", 480, 0)

            model.add_edge(ws.workspace_id, start.node_id, "exec_out", excel_read.node_id, "exec_in")
            model.add_edge(ws.workspace_id, excel_read.node_id, "rows", script.node_id, "payload")
            model.add_edge(ws.workspace_id, excel_read.node_id, "exec_out", script.node_id, "exec_in")
            model.add_edge(ws.workspace_id, script.node_id, "result", excel_write.node_id, "rows")
            model.add_edge(ws.workspace_id, script.node_id, "exec_out", excel_write.node_id, "exec_in")
            model.add_edge(ws.workspace_id, excel_write.node_id, "exec_out", end.node_id, "exec_in")

            events = self._run_model(model, ws.workspace_id)
    ...
    def test_smoke_file_input_python_transform_file_output(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            model.add_edge(ws.workspace_id, start.node_id, "exec_out", file_read.node_id, "exec_in")
            model.add_edge(ws.workspace_id, file_read.node_id, "text", script.node_id, "payload")
            model.add_edge(ws.workspace_id, file_read.node_id, "exec_out", script.node_id, "exec_in")
            model.add_edge(ws.workspace_id, script.node_id, "result", file_write.node_id, "text")
            model.add_edge(ws.workspace_id, script.node_id, "exec_out", file_write.node_id, "exec_in")
            model.add_edge(ws.workspace_id, file_write.node_id, "exec_out", end.node_id, "exec_in")
