...
class DpfWorkflowHelperTests(unittest.TestCase):
    ...
    def test_fixture_backed_helper_chain_reaches_model_scoping_and_materialization(self) -> None:
        ...
        self.assertEqual(data_sources_ref.kind, DPF_OBJECT_HANDLE_KIND)
        self.assertEqual(data_sources_ref.data_type_id, DPF_DATA_SOURCES_DATA_TYPE)
        self.assertNotIn("dpf_data_type", data_sources_ref.metadata)
        ...
        self.assertEqual(updated_data_sources_ref.kind, DPF_OBJECT_HANDLE_KIND)
        self.assertEqual(
            updated_data_sources_ref.data_type_id,
            DPF_DATA_SOURCES_DATA_TYPE,
        )
    ...
    def test_synthetic_helper_nodes_create_runtime_objects(self) -> None:
        ...
        self.assertEqual(workflow_ref.kind, DPF_OBJECT_HANDLE_KIND)
        self.assertEqual(workflow_ref.data_type_id, DPF_WORKFLOW_DATA_TYPE)
        workflow_value = services.resolve_handle(
            workflow_ref,
        ...
        self.assertEqual(streams_ref.kind, DPF_OBJECT_HANDLE_KIND)
        self.assertEqual(
            streams_ref.data_type_id,
            DPF_STREAMS_CONTAINER_DATA_TYPE,
        )
        streams_value = services.resolve_handle(
            streams_ref,
            expected_kind=DPF_OBJECT_HANDLE_KIND,
        )
        self.assertIsInstance(streams_value, dpf.StreamsContainer)

    def test_generic_helper_rejects_forged_duplicate_type_metadata(self) -> None:
        services = dpf_worker_services()
        workflow_outputs, _ = self._execute_node(
            "dpf.helper.workflow.workflow",
            services=services,
        ...
        with self.assertRaisesRegex(TypeError, "requires one of"):
            self._execute_node(
                "dpf.helper.data_sources.set_result_file_path",
                inputs={
                    "receiver": forged_ref,
                    "result_path": str(STATIC_ANALYSIS_RST),
                },
                services=services,
            )
