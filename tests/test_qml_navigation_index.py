...
class QmlNavigationIndexTests(unittest.TestCase):
    def test_live_repository_qml_routes_have_exact_bounded_owners(self) -> None:
        ...
        self.assertEqual(len(qml_entries), 172)
        self.assertEqual(len(qml_routes), 172)
        self.assertEqual(len({entry["path"] for entry in qml_entries}), 172)
        self.assertEqual(len({entry["component_name"] for entry in qml_entries}), 172)
