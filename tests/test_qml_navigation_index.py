...
class QmlNavigationIndexTests(unittest.TestCase):
    def test_live_repository_qml_routes_have_exact_bounded_owners(self) -> None:
        ...
        self.assertEqual(len(qml_entries), 206)
        self.assertEqual(len(qml_routes), 206)
        self.assertEqual(len({entry["path"] for entry in qml_entries}), 206)
        self.assertEqual(len({entry["component_name"] for entry in qml_entries}), 206)
