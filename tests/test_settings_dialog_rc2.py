...
class WorkflowSettingsRc2Tests(unittest.TestCase):
    ...
    def test_open_workflow_settings_updates_project_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            session_path = Path(temp_dir) / "last_session.json"
            autosave_path = Path(temp_dir) / "autosave.sfe"
            with patch("ea_node_editor.ui.main_window.recent_session_path", return_value=session_path), patch(
