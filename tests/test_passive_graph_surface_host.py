...
class ExcalidrawWebBoardPreviewPayloadTests(unittest.TestCase):
    ...
    def test_web_board_preview_viewport_renders_exported_artifact_refs(self) -> None:
        ...
        self.assertIn("ref.artifact_ref", viewport_source)
        self.assertIn('previewMode: exportedPreviewLoaded ? "image"', viewport_source)
        self.assertIn("webEngineView.destroy();", viewport_source)
        self.assertIn("enabled: false", viewport_source)
        self.assertIn("cache: false", viewport_source)
        self.assertIn("visible: root.webEngineAvailable && !root.exportedPreviewLoaded", viewport_source)
        self.assertIn("visible: root.exportedPreviewLoaded", viewport_source)
...
class WebPageGenericSurfaceQmlTests(PassiveGraphSurfaceHostTestBase):
    ...
    def test_graph_canvas_exposes_video_source_browse_toolbar_action(self) -> None:
        self._run_qml_probe(
            "video-source-browse-toolbar-action",
            """
            from PyQt6.QtTest import QTest
            ...
            scene = GraphSceneBridge()
            scene.set_workspace(model, registry, workspace_id)
            node_id = scene.add_node_from_type("passive.media.video_panel", 120.0, 90.0)
            scene.set_node_property(node_id, "source_path", "")
            scene.select_node(node_id, False)
            ...
            try:
                settle_events(10)
                card = node_card_for(canvas, node_id)
                surface = card.findChild(QObject, "graphNodeVideoSurface")
                source_button = named_item(canvas, "graphNodeFloatingToolbarAction_editSource")
                ...
                assert action_ids == ["editSource", "toggle_content_only", "toggle_title", "toggle_frame"], action_ids
                source_action = next(action for action in actions if action["id"] == "editSource")
                assert str(source_action["icon"]) == "search"
                assert bool(source_action["enabled"])

                # The popover is gone: editSource now routes straight to a native
                # file dialog via the canvas command bridge, so neither the popover
                # nor its overlay layer should exist in the QML tree.
                assert canvas.findChild(QObject, "graphNodeVideoSourcePopover") is None
                assert canvas.findChild(QObject, "graphNodeVideoSourceOverlayLayer") is None

                QTest.mouseMove(window, item_scene_point(card, 0.5, 0.5))
                settle_events(5)
                assert bool(source_button.property("visible")), source_button.property("visible")
            finally:
                dispose_host_window(canvas, window)
                engine.deleteLater()
                app.processEvents()
