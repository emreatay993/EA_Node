...
from ea_node_editor.nodes import ansys_dpf_data_types
...
class DpfContractTests(unittest.TestCase):
    def test_normalize_dpf_type_id_preserves_specialized_object_families(self) -> None:
        self.assertEqual(
            ansys_dpf_data_types.normalize_dpf_type_id("model"),
            ansys_dpf_data_types.DPF_MODEL_DATA_TYPE,
        )
        self.assertEqual(
            ansys_dpf_data_types.normalize_dpf_type_id(
                "ansys.dpf.core.meshed_region.MeshedRegion"
            ),
            ansys_dpf_data_types.DPF_MESH_DATA_TYPE,
        )
        self.assertEqual(
            ansys_dpf_data_types.normalize_dpf_type_id(
                "ansys.dpf.core.data_sources.DataSources"
            ),
            ansys_dpf_data_types.DPF_DATA_SOURCES_DATA_TYPE,
        )
        self.assertEqual(
            ansys_dpf_data_types.normalize_dpf_type_id(
                "ansys.dpf.core.workflow.Workflow"
            ),
            ansys_dpf_data_types.DPF_WORKFLOW_DATA_TYPE,
        )

    def test_normalize_dpf_type_id_falls_back_to_generic_object_handle(self) -> None:
        self.assertEqual(
            ansys_dpf_data_types.normalize_dpf_type_id(
                "ansys.dpf.core.custom.magic.CustomCarrier"
            ),
            ansys_dpf_data_types.DPF_OBJECT_HANDLE_DATA_TYPE,
        )
    ...
    def test_ports_compatible_accept_declared_target_accepted_data_types(self) -> None:
        ...
        self.assertEqual(
            effective_ports.port_accepted_data_types(target),
            (
                ansys_dpf_data_types.DPF_OBJECT_HANDLE_DATA_TYPE,
                ansys_dpf_data_types.DPF_MODEL_DATA_TYPE,
                ansys_dpf_data_types.DPF_DATA_SOURCES_DATA_TYPE,
        ...
        self.assertFalse(
            effective_ports.ports_compatible(
                PortSpec(
                    ...
                    "out",
                    "data",
                    ansys_dpf_data_types.DPF_FIELD_DATA_TYPE,
                ),
                target,
                data_types=data_types,
            )
    ...
    def test_dpf_pin_source_metadata_tracks_accepted_data_types_for_multi_type_inputs(self) -> None:
        source = DpfPinSourceSpec(
            ...
            value_origin="port",
            value_key="receiver",
            data_type=ansys_dpf_data_types.DPF_OBJECT_HANDLE_DATA_TYPE,
            accepted_data_types=(
                ansys_dpf_data_types.DPF_MODEL_DATA_TYPE,
                ansys_dpf_data_types.DPF_DATA_SOURCES_DATA_TYPE,
            ),
