...
TestCase {
        ...
        Item {
            ...
            Item {
                ...
                function dispatchSurfaceAction(actionId) {
                    ...
                    if (String(actionId || "") === "videoBookmarkAdd") {
                        floatingHost.videoBookmarkAddCount += 1
                        floatingHost._setVideoBookmarkActions()
                    }
                ...
                function useFontSizeSurfaceActionProbe() {
                    floatingHost.availableActions = [{
                ...
                function _videoBookmarkPopoverActions() {
                    var actions = [{
                        "id": "videoBookmarkAdd",
                        "label": "Add 0:13",
                    ...
                    for (var index = 0; index < floatingHost.videoBookmarkAddCount; ++index) {
                ...
                function _setVideoBookmarkActions() {
                    floatingHost.availableActions = [{
                        "id": "bookmarks",
                        "label": "Bookmarks",
                        ...
                        "popoverActions": floatingHost._videoBookmarkPopoverActions()
                    }]
                }

                function useVideoBookmarkActionProbe() {
                    floatingHost.videoBookmarkAddCount = 0
                    floatingHost._setVideoBookmarkActions()
                }
                ...
                function useFontFamilySurfaceActionProbe() {
                    floatingHost.availableActions = [{
    ...
    function test_floating_toolbar_video_bookmarks_popover_refreshes_after_repeated_adds() {
        var root = createFloatingToolbarProbe()
        var host = findChild(root, "floatingToolbarProbeHost")
        var toolbar = findChild(root, "floatingToolbarProbeToolbar")
        host.useVideoBookmarkActionProbe()
        wait(0)
        var events = []
        ...
        findChild(root, "graphNodeFloatingToolbarAction_bookmarks").clicked()
        var popover = findChild(root, "graphNodeFloatingToolbarActionPopover")
        tryCompare(popover, "visible", true, 1000)
        compare(toolbar.actionPopoverActions.length, 1)

        findChild(root, "graphNodeFloatingToolbarPopoverAction_videoBookmarkAdd").clicked()
        tryVerify(function() {
            return events.length === 1 && toolbar.actionPopoverActions.length === 2
        }, 1000)
        compare(events[0], "videoBookmarkAdd")
        verify(popover.visible)
        compare(findChild(root, "graphNodeFloatingToolbarVideoBookmarkTime"), null)
        var labelField = findChild(root, "graphNodeFloatingToolbarVideoBookmarkLabel_bookmark-1")
        compare(labelField.placeholderText, "0:13")
        compare(findChild(root, "graphNodeFloatingToolbarVideoBookmarkJump_bookmark-1").iconName, "video-bookmark-jump")

        findChild(root, "graphNodeFloatingToolbarPopoverAction_videoBookmarkAdd").clicked()
        tryVerify(function() {
            return events.length === 2 && toolbar.actionPopoverActions.length === 3
        }, 1000)
        compare(JSON.stringify(events), JSON.stringify(["videoBookmarkAdd", "videoBookmarkAdd"]))
        compare(toolbar.actionPopoverActions[0].role, "add")
        compare(toolbar.actionPopoverActions[1].role, "bookmark")
        compare(toolbar.actionPopoverActions[2].role, "bookmark")
    }

    function test_floating_toolbar_font_size_popover_uses_field_slider_and_stepper() {
        var root = createFloatingToolbarProbe()
        var host = findChild(root, "floatingToolbarProbeHost")
        var toolbar = findChild(root, "floatingToolbarProbeToolbar")
        host.useFontSizeSurfaceActionProbe()
        wait(0)
        var events = []
        host.surfaceActionRequested.connect(function(actionId) {
            events.push(String(actionId))
        ...
        var groupButton = findChild(root, "graphNodeFloatingToolbarAction_text_font_size_group")
        compare(groupButton.text, "24")
        verify(!groupButton.iconOnly)
        groupButton.clicked()
        var popover = findChild(root, "graphNodeFloatingToolbarActionPopover")
        tryCompare(popover, "visible", true, 1000)
        compare(toolbar.actionPopoverLayout, "font_size")

        var panel = findChild(root, "graphNodeFloatingToolbarFontSizePanel")
        var field = findChild(root, "graphNodeFloatingToolbarFontSizeField")
        var slider = findChild(root, "graphNodeFloatingToolbarFontSizeSlider")
        var stepper = findChild(root, "graphNodeFloatingToolbarFontSizeStepper")
        ...
        root.previewFontSize(48)
        root.previewFontSize(52)
        compare(JSON.stringify(events), JSON.stringify([
            "text_font_size_preview:48",
            "text_font_size_preview:52"
        ]))
        compare(field.text, "52")
        root.commitFontSize(52)
        compare(events[events.length - 1], "text_font_size_set:52")

        events = []
        root.previewFontSize(48)
        root.closeActionPopover(true)
        compare(JSON.stringify(events), JSON.stringify([
            "text_font_size_preview:48",
            "text_style_flush"
        ]))
        ...
        keyClick(Qt.Key_Return)
        compare(events[events.length - 1], "text_font_size_set:9")

        field.text = "999"
        field.forceActiveFocus()
        keyClick(Qt.Key_Return)
        compare(events[events.length - 1], "text_font_size_set:144")
        compare(field.text, "144")
        findChild(root, "graphNodeFloatingToolbarPopoverAction_text_font_size_increase").clicked()
        compare(events[events.length - 1], "text_font_size_increase")
    }

    function test_floating_toolbar_pdf_page_popover_uses_page_field_and_navigation_buttons() {
        var root = createFloatingToolbarProbe()
        var host = findChild(root, "floatingToolbarProbeHost")
        var toolbar = findChild(root, "floatingToolbarProbeToolbar")
        host.usePdfPageActionProbe()
        wait(0)
        var events = []
        host.surfaceActionRequested.connect(function(actionId) {
            events.push(String(actionId))
        ...
        var groupButton = findChild(root, "graphNodeFloatingToolbarAction_pdf_page_navigation")
        compare(groupButton.iconName, "navigate")
        verify(groupButton.iconOnly)
        groupButton.clicked()
        var popover = findChild(root, "graphNodeFloatingToolbarActionPopover")
        tryCompare(popover, "visible", true, 1000)
        compare(toolbar.actionPopoverLayout, "pdf_page")

        var panel = findChild(root, "graphNodeFloatingToolbarPdfPagePanel")
        var field = findChild(root, "graphNodeFloatingToolbarPdfPageField")
        var total = findChild(root, "graphNodeFloatingToolbarPdfPageTotalLabel")
        var previousButton = findChild(root, "graphNodeFloatingToolbarPopoverAction_pdf_page_previous")
        var nextButton = findChild(root, "graphNodeFloatingToolbarPopoverAction_pdf_page_next")
        verify(panel.visible)
        ...
        field.forceActiveFocus()
        keyClick(Qt.Key_Return)
        compare(events[events.length - 1], "pdf_page_set:4")
        compare(field.text, "4")
        nextButton.clicked()
        compare(events[events.length - 1], "pdf_page_set:5")
    ...
    function test_floating_toolbar_source_storage_popover_uses_external_default_combo() {
        var root = createFloatingToolbarProbe()
        var host = findChild(root, "floatingToolbarProbeHost")
        var toolbar = findChild(root, "floatingToolbarProbeToolbar")
        host.useSourceStorageActionProbe()
        wait(0)
        var events = []
        host.surfaceActionRequested.connect(function(actionId) {
            events.push(String(actionId))
        ...
        findChild(root, "graphNodeFloatingToolbarAction_editSource").clicked()
        var popover = findChild(root, "graphNodeFloatingToolbarActionPopover")
        tryCompare(popover, "visible", true, 1000)
        compare(toolbar.actionPopoverLayout, "source_storage")
        var panel = findChild(root, "graphNodeFloatingToolbarSourceStoragePanel")
        var combo = findChild(root, "graphNodeFloatingToolbarSourceStorageCombo")
        var browseButton = findChild(root, "graphNodeFloatingToolbarSourceBrowseButton")
        verify(panel.visible)
        compare(combo.currentText, "External")
        compare(Math.round(combo.height), toolbar._popoverControlHeight)
        compare(Math.round(combo.controlHeight), toolbar._popoverControlHeight)
        compare(Math.round(browseButton.height), toolbar._popoverControlHeight)
        combo.currentIndex = 1
        browseButton.clicked()
        compare(JSON.stringify(events), JSON.stringify(["editSourceManagedCopy"]))
        tryCompare(popover, "visible", false, 1000)
    }

    function test_floating_toolbar_font_family_popover_filters_and_dispatches_selection() {
        var root = createFloatingToolbarProbe()
        var host = findChild(root, "floatingToolbarProbeHost")
        var toolbar = findChild(root, "floatingToolbarProbeToolbar")
        host.useFontFamilySurfaceActionProbe()
        wait(0)
        var events = []
        host.surfaceActionRequested.connect(function(actionId) {
            events.push(String(actionId))
        ...
        var groupButton = findChild(root, "graphNodeFloatingToolbarAction_text_font_family_group")
        compare(groupButton.text, "Segoe UI")
        verify(!groupButton.iconOnly)
        groupButton.clicked()
        var popover = findChild(root, "graphNodeFloatingToolbarActionPopover")
        tryCompare(popover, "visible", true, 1000)
        compare(toolbar.actionPopoverLayout, "font_family")
        verify(findChild(root, "graphNodeFloatingToolbarFontFamilyPanel").visible)
        verify(findChild(root, "graphNodeFloatingToolbarFontFamilyList").visible)
        compare(findChild(root, "graphNodeFloatingToolbarFontFamilyField").placeholderText, "Search fonts")
        tryVerify(function() {
            return findChild(root, "graphNodeFloatingToolbarPopoverAction_text_font_family_set:Segoe%20UI") !== null
        }, 1000)

        toolbar.actionPopoverFilterText = "geo"
        tryVerify(function() {
            return findChild(root, "graphNodeFloatingToolbarPopoverAction_text_font_family_set:Georgia") !== null
        }, 1000)
        findChild(root, "graphNodeFloatingToolbarPopoverAction_text_font_family_set:Georgia").clicked()
        compare(JSON.stringify(events), JSON.stringify([
        ...
        groupButton.clicked()
        tryCompare(popover, "visible", true, 1000)
        findChild(root, "graphNodeFloatingToolbarPopoverAction_text_font_family_clear").clicked()
        compare(events[events.length - 2], "text_font_family_clear")
        compare(events[events.length - 1], "text_style_flush")
    }
