...
_MISSING_ADDON_REASON = "ansys.dpf.core is not installed; the DPF node family remains unavailable."
_MISSING_ADDON_PAYLOAD = {
    "addon_id": "ansys.dpf",
...
def _missing_dpf_surface_payload() -> dict[str, object]:
    result_node = {
        ...
        "parent_node_id": None,
        "_missing_addon_placeholder": dict(_MISSING_ADDON_PAYLOAD),
    }
    model_node = {
        "node_id": "node_model",
        "type_id": "dpf.model",
    ...
    return {
        ...
        "workspaces": [{
            ...
            "views": [{"view_id": "view_dpf", "name": "V1", "zoom": 1.0, "pan_x": 0.0, "pan_y": 0.0}],
            "nodes": [result_node, model_node],
            "edges": [{
...
class GraphSurfaceLockedPortContractTests(GraphSurfaceInputContractTestBase):
    def test_graph_node_host_emits_locked_input_port_double_click_contract(self) -> None:
        self._run_qml_probe(
            ...
            events = []
            host.portDoubleClicked.connect(lambda node_id, port_key, direction, locked: events.append((node_id, port_key, direction, locked)))
    ...
    def test_graph_node_host_emits_unlocked_lockable_input_double_click_contract(self) -> None:
        self._run_qml_probe(
            ...
            """
            payload = node_payload()
            payload["ports"] = [{"key": "message", "label": "Message", "direction": "in", "kind": "data", "data_type": "str", "lockable": True, "locked": False}]
            host = create_component(graph_node_host_qml_path, {"nodeData": payload})
