...
def inspect_rst(rst_path):
    ...
    ``times`` (list[float]), ``unit`` (str), ``n_nodes`` (int).
    """
    dpf = _require_dpf()
...
def load_rst_surface_mesh(rst_path, named_selection=None):
...
def load_rst_strain(rst_path, set_ids, named_selection=None, rotate_to_global=True):
