...
- The shipped DPF catalog remains intentionally narrow: the existing handwritten compute nodes plus `dpf.viewer` are the only in-repo DPF nodes on this branch, and broad autogenerated operator exposure is deferred.
- `ea_node_editor/nodes/builtins/ansys_dpf_catalog.py` and `ea_node_editor/nodes/builtins/ansys_dpf_common.py` normalize operator metadata into stable node, port, and pin-source descriptors that distinguish required inputs, optional inputs, omitted defaults, literal values, and mutually exclusive groups.
- `ea_node_editor/execution/dpf_runtime/base.py`, `contracts.py`, and `operations.py` are the generic operator-backed runtime seam for the shipped handwritten DPF compute nodes.
...
- Broad autogenerated operator rollout and non-operator `ansys.dpf.core` reflection remain deferred to a later packet set; that work should reuse the backend contract published in [the DPF operator backend review](docs/DPF_OPERATOR_PLUGIN_BACKEND_REVIEW_2026-04-12.md) and [the DPF operator backend QA matrix](docs/specs/perf/DPF_OPERATOR_PLUGIN_BACKEND_REFACTOR_QA_MATRIX.md).
...
- The public closeout snapshot now spans `ARCHITECTURE.md`, `README.md`, `docs/DPF_OPERATOR_PLUGIN_BACKEND_REVIEW_2026-04-12.md`, `docs/specs/INDEX.md`, `docs/PACKAGING_WINDOWS.md`, `docs/PILOT_RUNBOOK.md`, `docs/specs/perf/ARCHITECTURE_MAINTAINABILITY_REFACTOR_QA_MATRIX.md`, `docs/specs/perf/ARCHITECTURE_FOLLOWUP_REFACTOR_QA_MATRIX.md`, `docs/specs/perf/DPF_OPERATOR_PLUGIN_BACKEND_REFACTOR_QA_MATRIX.md`, `docs/specs/perf/VERIFICATION_SPEED_QA_MATRIX.md`, and `docs/specs/requirements/TRACEABILITY_MATRIX.md`.
