...
- Membership: an expanded Group owns what lies fully inside it (the smallest fully containing Group in the same scope and parent chain), which drives nesting, descendant drag/resize propagation and the expanded clipboard/delete rules. A collapsed Group — and every Group hidden inside it — owns exactly what it held when it was collapsed, persisted as `held_member_ids`; its hidden area claims nothing, and it belongs to the Group that contains its collapsed body's top-left corner. Anything that grows inside a Group makes room level by level and grows the Group.
- A Group has only its hidden title property: its default title is empty, an untitled Group shows `Double click to edit title` only while selected, and double-click opens the shared title editor. Groups have no body/rich-text content or Inspector-editable properties. Serializer and clipboard fragments persist authored title, collapsed state, and geometry; derived containment metadata is stripped and recomputed on paste and load (stored member lists are kept and remapped). Old external type IDs are not compatible, while tracked fixtures use the new type.
