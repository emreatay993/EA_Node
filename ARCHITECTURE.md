...
- `GraphActionController` does not own the behavior implementation. It selects the established owner for each verb: workspace graph-edit controllers for selection/subgraph operations, `GraphCanvasHostPresenter` for scope entry plus graph cursor/style/label host actions, `GraphSceneBridge` for Group Peek state, and the help or add-on bridges for their shell-owned surfaces.
...
- `ea_node_editor/addons/ansys_dpf/catalog.py`, `ea_node_editor/addons/tabular_data/catalog.py`, `ea_node_editor/addons/hot_apply.py`, `ea_node_editor/execution/worker_services.py`, and `ea_node_editor/ui_qml/viewer_host_service.py` make ANSYS DPF and Tabular Data shipped repo-local `hot_apply` add-ons. Availability changes rebuild registry/runtime state, invalidate relevant viewer/worker/cache state, and project unavailable live nodes back into locked unavailable-add-on surfaces when needed.
...
- `ea_node_editor/addons/tabular_data/loader_cache_service.py` scans and opens CSV, TSV, TXT, XLSX, XLSM, Parquet, HDF5, NPY, and NPZ sources, selects lazy or memory-mapped policies where available, writes managed Parquet cache artifacts only when the source/options require it, and can reopen refs from saved `source_uri` plus load-option metadata for downstream consumers.
