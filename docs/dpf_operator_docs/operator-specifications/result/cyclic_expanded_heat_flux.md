...
This operator is deprecated: use the operator heat flux with the read_cyclic pin instead. Read heat flux from a result file and expand it with cyclic symmetry.
...
ansys::dpf::Operator op("TF_cyclic"); // operator instantiation
...
op = dpf.operators.result.cyclic_expanded_heat_flux() # operator instantiation
