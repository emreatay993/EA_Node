...
This operator is deprecated: use the operator element nodal forces with the read_cyclic pin instead. Read element nodal forces from a result file and expand it with cyclic symmetry.
...
ansys::dpf::Operator op("ENF_cyclic"); // operator instantiation
...
op = dpf.operators.result.cyclic_expanded_enf() # operator instantiation
