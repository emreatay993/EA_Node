...
| <strong>Pin 100</strong>|  down_stream_info |[`int32`](../../core-concepts/dpf-types.md#standard-types) | Chunk size of modes (number of modes) that are allowed to be loaded. When a value smaller than the number of modes is entered, the incremental chunking is activated and caching mechanisms are disabled to avoid storing loaded modes. Default: No chunking, cache active. |
...
ansys::dpf::Operator op("UTOT_msup"); // operator instantiation
