...
Read state variables results from the RST file. On demand expansion and cyclic expansion procedures are not supported. If no index is specified, every state variable will be returned in a separate field with the label SVAR.
 ...
 **Full name**: result.esv

 **Internal name**: mapdl::rth::ESV
...
ansys::dpf::Operator op("mapdl::rth::ESV"); // operator instantiation
...
op = dpf.operators.result.esv() # operator instantiation
