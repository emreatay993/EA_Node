...
This operator is deprecated: use the operator temperature field with the read_cyclic pin instead. Read temperature field from a result file and expand it with cyclic symmetry.
...
ansys::dpf::Operator op("TEMP_cyclic"); // operator instantiation
...
op = dpf.operators.result.cyclic_expanded_temperature() # operator instantiation
