...
Read ms fluid velocity results from result streams.
...
|-------|------|------------------|-------------|
|  **Pin 0**| fluid_velocity |[`fields_container`](../../core-concepts/dpf-types.md#fields-container) | results by specifying name. |
...
ansys::dpf::Operator op("lsdyna::ms::FV"); // operator instantiation
op.connect(0, my_time_scoping);
op.connect(3, my_streams_container);
op.connect(4, my_data_sources);
ansys::dpf::FieldsContainer my_fluid_velocity = op.getOutput<ansys::dpf::FieldsContainer>(0);
```
</details>
...
my_fluid_velocity = op.outputs.fluid_velocity()
```
</details>
...
op.inputs.streams_container.Connect(my_streams_container)
op.inputs.data_sources.Connect(my_data_sources)
my_fluid_velocity = op.outputs.fluid_velocity.GetData()
```
</details>
<br>
