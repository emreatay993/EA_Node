...
Read ms average pressure results from result streams.
...
|-------|------|------------------|-------------|
|  **Pin 0**| average_pressure |[`fields_container`](../../core-concepts/dpf-types.md#fields-container) | results by specifying name. |
...
ansys::dpf::Operator op("lsdyna::ms::AP"); // operator instantiation
op.connect(0, my_time_scoping);
op.connect(3, my_streams_container);
op.connect(4, my_data_sources);
ansys::dpf::FieldsContainer my_average_pressure = op.getOutput<ansys::dpf::FieldsContainer>(0);
```
</details>
...
my_average_pressure = op.outputs.average_pressure()
```
</details>
...
op.inputs.streams_container.Connect(my_streams_container)
op.inputs.data_sources.Connect(my_data_sources)
my_average_pressure = op.outputs.average_pressure.GetData()
```
</details>
<br>
