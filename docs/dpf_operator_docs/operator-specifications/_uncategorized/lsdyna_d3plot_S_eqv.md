...
Read Von Mises Stress from result streams.
...
|-------|------|------------------|-------------|
|  **Pin 0**| von_mises_stress |[`fields_container`](../../core-concepts/dpf-types.md#fields-container) | von mises stress |
 ...
 **Internal name**: lsdyna::d3plot::S_eqv
...
ansys::dpf::Operator op("lsdyna::d3plot::S_eqv"); // operator instantiation
...
ansys::dpf::FieldsContainer my_von_mises_stress = op.getOutput<ansys::dpf::FieldsContainer>(0);
```
</details>
...
my_von_mises_stress = op.outputs.von_mises_stress()
```
</details>
...
op.inputs.part_id.Connect(my_part_id)
op.inputs.unit_system.Connect(my_unit_system)
my_von_mises_stress = op.outputs.von_mises_stress.GetData()
```
</details>
<br>
