...
Read ms randle i results from result streams.
...
|-------|------|------------------|-------------|
|  **Pin 0**| randle_i |[`fields_container`](../../core-concepts/dpf-types.md#fields-container) | results by specifying name. |
 ...
 **Internal name**: lsdyna::ms::randle_I
...
ansys::dpf::Operator op("lsdyna::ms::randle_I"); // operator instantiation
op.connect(0, my_time_scoping);
op.connect(3, my_streams_container);
op.connect(4, my_data_sources);
ansys::dpf::FieldsContainer my_randle_i = op.getOutput<ansys::dpf::FieldsContainer>(0);
```
</details>
...
my_randle_i = op.outputs.randle_i()
```
</details>
...
op.inputs.streams_container.Connect(my_streams_container)
op.inputs.data_sources.Connect(my_data_sources)
my_randle_i = op.outputs.randle_i.GetData()
```
</details>
<br>
