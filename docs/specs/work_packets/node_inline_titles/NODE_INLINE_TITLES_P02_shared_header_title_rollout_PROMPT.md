Implement NODE_INLINE_TITLES_P02_shared_header_title_rollout.md exactly. Before editing, read NODE_INLINE_TITLES_MANIFEST.md, NODE_INLINE_TITLES_STATUS.md, and NODE_INLINE_TITLES_P02_shared_header_title_rollout.md. Implement only P02. Stay inside the packet write scope, preserve locked defaults and public graph-title/scope-entry contracts unless the packet explicitly changes them, run the full Verification Commands, run the Review Gate before marking the packet done when it is not `none`, create the required packet wrap-up artifact, update NODE_INLINE_TITLES_STATUS.md with branch label, accepted commit sha, commands, tests, artifacts, and residual risks, commit packet-local changes on the packet branch when you own the final substantive state, and stop after P02; do not start P03.
...
- Keep this packet on the shared header/host seam only. Do not solve subnode scope-entry or collapsed-node behavior here.

Notes:
- Target branch: `codex/node-inline-titles/p02-shared-header-title-rollout`
- Review Gate: `QT_QPA_PLATFORM=offscreen ./venv/Scripts/python.exe -m pytest tests/test_passive_graph_surface_host.py --ignore=venv -k "title" -q`
- Expected artifacts:
