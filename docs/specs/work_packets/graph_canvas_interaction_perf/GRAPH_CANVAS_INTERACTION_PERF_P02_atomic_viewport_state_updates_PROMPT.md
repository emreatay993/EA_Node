Implement GRAPH_CANVAS_INTERACTION_PERF_P02_atomic_viewport_state_updates.md exactly. Before editing, read GRAPH_CANVAS_INTERACTION_PERF_MANIFEST.md, GRAPH_CANVAS_INTERACTION_PERF_STATUS.md, and GRAPH_CANVAS_INTERACTION_PERF_P02_atomic_viewport_state_updates.md. Implement only P02. Stay inside the packet write scope, preserve locked defaults and current full-fidelity visuals unless the packet explicitly changes an internal optimization seam, run the full Verification Commands, run the Review Gate before marking the packet done when it is not `none`, create the required packet wrap-up artifact, update GRAPH_CANVAS_INTERACTION_PERF_STATUS.md with branch label, accepted commit sha, commands, tests, artifacts, and residual risks, commit packet-local changes on the packet branch when you own the final substantive state, and stop after P02; do not start P03.
...
- Keep this packet on atomic viewport mutation, raw viewport-state hot-path binding, cached visible-scene-rect plumbing, and dead per-node zoom fan-out removal only. Do not start redraw coalescing or viewport-aware node activation here.

Notes:
- Target branch: `codex/graph-canvas-interaction-perf/p02-atomic-viewport-state-updates`
- Review Gate: `./venv/Scripts/python.exe -m pytest tests/graph_track_b/viewport.py --ignore=venv -q`
- Expected artifacts:
