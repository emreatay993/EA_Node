...
- Added the packet's Mockup B missing-add-on presentation across the host and canvas stack: muted locked header treatment, dashed accent stripe, `LOCKED` chip, collapsed `Requires add-on` placeholder ribbon with add-on details, lighter `Load...` node CTA copy, aggregate locked-node canvas ribbon, and locked input and output port glyphs while preserving node silhouette and port positions.
- Routed the locked placeholder escape hatch through the existing P02 seam by sending node-level common-action and placeholder-button requests plus the new aggregate canvas action to `addonManagerBridge.requestOpen(focus_addon_id)` when the missing add-on exposes a single focus id, otherwise falling back to the general manager entry point.
...
- Refreshed packet-owned tests to cover the locked placeholder contract, lighter `Load...` host copy, manager-only node context menus, aggregate locked-node canvas ribbon/action routing, read-only bridge rejection, passive-host visual probes for the locked chip, stripe, ribbon, locked ports, and the inherited graph-canvas performance and live-resize geometry expectations that now pass on the packet branch.

## Verification

- PASS: `.\venv\Scripts\python.exe -m pytest tests/test_graph_surface_input_contract.py tests/test_graph_surface_input_controls.py tests/test_graph_surface_input_inline.py tests/test_passive_graph_surface_host.py --ignore=venv -q` -> `119 passed, 4 warnings, 18 subtests passed`
...
Expected result: the node shows the muted Mockup B locked treatment, `LOCKED` badge, add-on requirement ribbon, lighter `Load...` affordance on the node, locked port glyphs, and the aggregate locked-node canvas ribbon with `Load missing add-ons`.
