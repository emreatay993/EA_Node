# COREX_CLEAN_ARCHITECTURE_RESTRUCTURE P04 Current-Schema Persistence Prompt

Read the manifest, status ledger, and `COREX_CLEAN_ARCHITECTURE_RESTRUCTURE_P04_current_schema_persistence.md` before editing. Implement exactly `P04_current_schema_persistence`.
...
Create `docs/specs/work_packets/corex_clean_architecture_restructure/P04_current_schema_persistence_WRAPUP.md`.
...
Commit packet-local changes on `codex/corex-clean-architecture-restructure/p04-current-schema-persistence`, update the shared status ledger only if you are running this prompt standalone, and stop after this packet.
