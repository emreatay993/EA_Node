...
Notes:
- Target branch: `codex/title-icons-for-non-passive-nodes/p03-qml-header-icon-rendering`.
- Review Gate: `$env:QT_QPA_PLATFORM='offscreen'; .\venv\Scripts\python.exe -m pytest tests/graph_surface/passive_host_interaction_suite.py -k title_icon --ignore=venv -q`
- Expected artifacts:
  - `docs/specs/work_packets/title_icons_for_non_passive_nodes/P03_qml_header_icon_rendering_WRAPUP.md`
- Use `nodeData.icon_source` and the shared effective icon-size property from P02. Do not alter the existing collapsed comment-backdrop `uiIcons` icon path.
