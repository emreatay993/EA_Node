...
Notes:
- Target branch: `codex/title-icons-for-non-passive-nodes/p04-builtin-node-icon-assets-and-migration`.
- Review Gate: `.\venv\Scripts\python.exe -m pytest tests/test_node_title_icon_assets.py -k title_icon --ignore=venv -q`
- Expected artifacts:
...
- Migrate only active and `compile_only` built-ins. Keep passive built-ins out of title-icon rendering.
