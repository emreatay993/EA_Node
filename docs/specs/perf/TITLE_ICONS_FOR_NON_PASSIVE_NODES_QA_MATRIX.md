...
- Scope: final closeout matrix for the shipped path-backed title-leading icons on active and `compile_only` nodes, theme-aware repo-managed SVG title-icon rendering, passive title-icon exclusion, local-file-only resolver behavior, built-in node-title icon asset packaging, collapsed Group glyph preservation, app-global nullable icon-size override, and traceability/docs evidence.
...
- `icon_source` and `icon_theme_aware` remain derived live graph payload fields, are populated only for eligible title icons when local resolution succeeds, and are not persisted into `.cxproj` project files.
- Passive nodes remain title-iconless even when their specs carry authored `icon` metadata, and the collapsed Group glyph stays on the existing `uiIcons` / `comment.svg` path instead of converting to this image-path contract.
- `graphics.typography.graph_node_icon_pixel_size_override` remains a nullable app-global integer preference: `null` follows `graph_label_pixel_size`, explicit values clamp to `8..50`, repo-managed SVG title icons theme-tint from `headerTextColor`, and raster plus plugin/add-on image colors remain authored/untinted by default.
...
| Collapsed Group glyph exception and unchanged `uiIcons` / `comment.svg` behavior alongside the new header contract | `P03` | `REQ-UI-038`, `REQ-PERF-013`, `AC-REQ-PERF-013-01` | `$env:QT_QPA_PLATFORM='offscreen'; .\venv\Scripts\python.exe -m pytest tests/test_group_backdrop_collapse.py tests/main_window_shell/shell_runtime_contracts.py tests/test_icon_registry.py tests/test_group_backdrop_contracts.py -k title_icon --ignore=venv -q` | Accepted `P03` packet commit `a0cbb845271b2b8d8be0150d5fcb81609e305b2f`; T25 replaced the stale shell path with the direct Group collapse owner. |
...
| Registry asset validation, passive-node title-icon ineligibility, and retained built-in contract coverage after asset rollout | `P04` | `REQ-NODE-028`, `AC-REQ-NODE-028-01` | `.\venv\Scripts\python.exe -m pytest tests/test_registry_validation.py tests/test_passive_node_contracts.py tests/test_passive_flowchart_catalog.py -k title_icon --ignore=venv -q` | Current focused regression. |
| Theme-aware repo-managed SVG title-icon payload derivation and QML header tinting through `headerTextColor` | `2026-05-23 follow-up` | `REQ-UI-038`, `REQ-NODE-028`, `REQ-PERF-013` | `.\venv\Scripts\python.exe -m pytest tests/test_node_title_icon_sources.py -k title_icon --ignore=venv -q`; `$env:QT_QPA_PLATFORM='offscreen'; .\venv\Scripts\python.exe -m pytest tests/graph_surface/inline_editor_suite.py tests/graph_surface/passive_host_interaction_suite.py tests/test_group_backdrop_contracts.py -k title_icon --ignore=venv -q` | Local follow-up proof for `icon_theme_aware` and retained title-icon contracts |
...
## 2026-05-23 Theme-Aware SVG Follow-Up

| Command | Result | Notes |
|---|---|---|
| `.\venv\Scripts\python.exe -m pytest tests/test_node_title_icon_sources.py -k title_icon --ignore=venv -q` | PASS | Confirms repo-managed SVG title icons derive `icon_theme_aware`, plugin/provenance icons stay untinted by default, and live payload fields remain derived |
| `$env:QT_QPA_PLATFORM='offscreen'; .\venv\Scripts\python.exe -m pytest tests/graph_surface/inline_editor_suite.py tests/graph_surface/passive_host_interaction_suite.py tests/test_group_backdrop_contracts.py -k title_icon --ignore=venv -q` | PASS | Confirms QML title icons expose the theme-aware flag, tint from `headerTextColor`, and preserve edit hit regions plus Group precedence |

## Remaining Manual Smoke Checks

1. Windows built-in SVG check: start a desktop Qt session on this branch, add built-in active and `compile_only` nodes such as `Process Run`, `Excel Write`, `Subnode Input`, and `Model Viewer`, switch between dark and light graph themes, and confirm each shipped non-passive SVG title icon follows the header text color while passive note/flowchart/Group nodes remain iconless in the title row.
2. Local PNG/JPG/JPEG fixture check: on the same desktop session, use a file/package plugin or local test node spec that points `icon` at a reachable local PNG file such as `ea_node_editor/assets/app_icon/corex_app_minimal_32.png`, then repeat with JPG/JPEG fixtures where available locally; confirm each supported suffix renders in the title row with authored colors, while unsupported or missing local files still show no icon.
3. Passive exclusion and Group exception check: inspect a passive node that still carries authored `icon` metadata and confirm the header remains iconless, then collapse a Group and confirm its existing `uiIcons` / `comment.svg` glyph remains unchanged.
...
- Automated coverage validates the contract offscreen, but a real Windows desktop compositor is still needed to judge final rasterization quality, theme-tinted SVG anti-aliasing, and perceived balance between the leading icon and centered title text.
