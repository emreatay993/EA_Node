...
- Updated: `2026-07-11`
- Packet set: retained `ANSYS_DPF_FULL_PLUGIN_ROLLOUT` (`P00` through `P07`), refreshed by `COREX_INTERNAL_PERFORMANCE_IMPROVEMENT` `P02` / `P15`
...
- Built-in DPF registration is descriptor-first and version-aware. The committed schema-1 catalog is generated for the package-build DPF version; an exact runtime match loads it without operator construction or live discovery. Other supported `0.16.x` patches, invalid JSON, or unsupported schemas warn once and discover live in memory. No automatic writable user cache is created.
...
## 2026-07-11 Deterministic Catalog Performance Follow-up

- Declared dependency range remains `ansys-dpf-core>=0.16,<0.17`; the committed catalog was generated with `0.16.1`.
- `ea_node_editor/addons/ansys_dpf/operator_catalog.json` contains schema version `1` and `767` deterministically ordered generated operator descriptors. The complete registered DPF surface contains `805` descriptors after curated, helper, foundational, plot, and viewer families are included.
- Exact catalog/runtime matches deserialize without constructing DPF operators or running live discovery. Missing DPF keeps the existing optional-plugin behavior.
- Another supported `0.16.x` patch, corrupt JSON, or unsupported schema warns once and discovers live in memory. If live discovery fails, generated raw-operator mirrors are omitted while curated, helper, foundational, and plot nodes remain.
- Viewer/full package preflight fails when the build-environment DPF version and committed catalog differ. Base packaging retains its optional-dependency behavior.
- Exact-match registry construction measured `898.565 ms` median, `76%` faster than the fresh `P00` baseline and below the `1.5 s` gate.

```powershell
.\venv\Scripts\python.exe .\scripts\generate_ansys_dpf_operator_catalog.py --check
.\venv\Scripts\python.exe -m pytest tests/test_dpf_operator_catalog_asset.py tests/test_dpf_node_catalog.py tests/test_dpf_generated_operator_catalog.py tests/test_packaging_configuration.py --ignore=venv -q
```

The historical `P01` cache-refresh wording below is retained as packet provenance only. The committed deterministic catalog plus guarded live fallback above is the current runtime and packaging contract.
...
| Workflow-first DPF family taxonomy, catalog ordering, and library-filter coverage | `P03` | `REQ-INT-008`, `REQ-QA-038` | `.\venv\Scripts\python.exe -m pytest tests/test_dpf_library_taxonomy.py tests/test_dpf_node_catalog.py tests/test_library_projection.py --ignore=venv -q` | PASS in `PLANS_TO_IMPLEMENT/in_progress/ansys_dpf_full_plugin_rollout_packets/P03_family_taxonomy_WRAPUP.md` (`0dce2c51fb7de3067db53b90043a87a42af806a5`) |
...
2. Installed-dependency catalog check: start with exact DPF `0.16.1` and confirm the committed catalog loads without live discovery; on another supported `0.16.x` patch confirm one warning and in-memory live discovery; verify the library exposes `Ansys DPF > Inputs`, `Ansys DPF > Workflow`, `Ansys DPF > Helpers`, `Ansys DPF > Operators`, and `Ansys DPF > Viewer` without creating a writable user cache.
