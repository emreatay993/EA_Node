...
| T05 Tabular query | `ACCEPTED` | `t05_tabular_writer` | Tabular coordinator, source backends, normalized query/evaluators, direct tests/maps | Final integrated acceptance: 191 tests and 75 subtests passed with seven existing PyArrow warnings; Ruff, compile, maps, traceability, links, and diff hygiene passed | 409.6 MiB fixture hashes, cache keys, scan/conversion counts, output hashes, caps/totals unchanged; direct evidence neutral/inconclusive; offscreen shell harness inconclusive | `t05_tabular_reviewer`: null/missing parity and lost native-Parquet/independent-node coverage fixed; source/test re-review `CLEAR` | `57d4b84d` |
...
| T07 Residual tests | `READY TO COMMIT` | `t07_test_writer` | One graph-host preset test plus canonical style normalization imports | Standalone import passed; final owner/dialog/architecture/serializer acceptance 131 tests and 97 subtests passed; seven catalog guards and full dry-run passed; Ruff, compile, and diff hygiene passed | Product runtime unaffected; one unnecessary `ShellWindow` test construction removed | `t07_test_reviewer`: weakened final A/B payload assertion fixed; re-review `CLEAR` | Pending |
...
| T04 | Offscreen shell create; cursor and reset-edge action paths | `727.949 ms`; cursor `1426.923 ns`; host reset `291.733 ns`; full action `3042.979 ns` | `712.441 ms`; cursor `1311.855 ns`; host reset `229.580 ns`; full action `2894.418 ns` | Same 3 shell runs and 3x100k micro-path batches; 156 QObjects, 3 timers, 1 graph host, 1 shell host, 5 image providers unchanged | `NEUTRAL-TO-BETTER` |
| T05 | Deterministic 429,484,247-byte/3.8M-row fixture; managed cold/warm and source-direct queries | Baseline JSON plus cache key `ad0b23...`; 1 scan, cold 1 conversion, warm 0, direct capped at 200k; stable hashes | Corrected candidate JSON retains identical key, cache bytes, scans/conversions/calls, row counts/totals/truncation and comparable hashes | Managed/direct p50 deltas were mixed/noisy; no consistent material p95 regression; UI-thread guard retained; real-shell offscreen runs stalled before checkpoint in baseline and were not repeated | `NEUTRAL / INCONCLUSIVE`; no correctness or IO regression |
| T06 | Synthetic 5 s H.264 fixture SHA-256 `8E57FAE7...F6F9B4`; HEAD/candidate QQuickWindow runs | Ready `12.564 ms`, position `10.591`, paused stability `10.351`, unload `10.527`, reload `22.734`; HEAD resume failed within 1 s | Ready `12.911 ms`, position `10.943`, paused `10.664`, unload `10.522`, reload `22.387`; retained/recreated resume works | Three interleaved runs; comparable timing within `0.35 ms`; candidate same-node source unload and different-node playback proven; CPU/RSS incomparable because HEAD stayed idle | `NEUTRAL` load/lease timing; correctness improved. GIF `animated_media` evidence is unrelated/inconclusive |
