...
- The node persists semantic project state only, primarily `current_path`; navigation history, search text, sort column, selection, context-menu position, maximized state, tree expansion, and browse defaults remain transient UI/session state and are not written to `.cxproj`.
...
| Final closeout proof, canonical QA matrix, requirement traceability, markdown links, and fast verification gate | `P07` | `REQ-QA-044`, `AC-REQ-QA-044-01` | `.\venv\Scripts\python.exe scripts\run_verification.py --mode fast`; `.\venv\Scripts\python.exe scripts\check_traceability.py`; `.\venv\Scripts\python.exe scripts\check_markdown_links.py` | PASS in this packet: non-budget fast pytest passes, traceability and markdown checks pass; context-budget checks are retained only as optional historical diagnostics |
...
|---|---|---|
| `.\venv\Scripts\python.exe scripts\run_verification.py --mode fast` | HISTORICAL WAIVER | At the time of this packet, fast stopped in the since-retired `fast.context_budgets` phase; current fast verification no longer requires context budgets |
| Direct non-budget fast pytest slice from this historical packet | PASS | `989 passed, 5 skipped, 56 warnings` |
...
| `.\venv\Scripts\python.exe scripts\check_context_budgets.py` | OPTIONAL | Historical diagnostic only; context-budget pass/fail is no longer a required future coding or closeout gate |
