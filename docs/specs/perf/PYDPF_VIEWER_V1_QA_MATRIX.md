...
- The shipped DPF node family covers `dpf.load_result`, `dpf.result_field`, `dpf.mesh`, `dpf.export`, and `dpf.viewer`; live viewer state stays session-owned instead of flowing as ordinary graph payloads.
...
|---|---|---|---|
| Final packet regression | `REQ-ARCH-016`, `REQ-NODE-025`, `REQ-NODE-026`, `REQ-EXEC-013`, `REQ-QA-023` | `./venv/Scripts/python.exe -m pytest tests/test_dpf_viewer_node.py tests/test_dpf_node_catalog.py tests/test_viewer_session_bridge.py tests/test_viewer_surface_host.py tests/test_packaging_configuration.py tests/test_traceability_checker.py --ignore=venv -q` | Revalidates the shipped PyDPF viewer scope end to end: viewer-node catalog and execution, bridge and overlay live-policy behavior, packaging configuration, and packet-owned docs/traceability coverage |
...
|---|---|---|
| DPF viewer live policy | `./venv/Scripts/python.exe -m pytest tests/test_dpf_viewer_node.py tests/test_dpf_node_catalog.py tests/test_viewer_session_bridge.py tests/test_viewer_surface_host.py --ignore=venv -q` | Use for `dpf.viewer`, bridge, overlay-manager, or viewer-surface behavior changes |
