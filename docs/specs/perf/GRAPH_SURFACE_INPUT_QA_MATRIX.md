...
| Media-surface direct ownership and crop-mode whole-surface lock | `REQ-UI-022`, `REQ-UI-023`, `REQ-QA-013` | `QT_QPA_PLATFORM=offscreen ./venv/Scripts/python.exe -m unittest tests.test_graph_surface_input_contract tests.test_graph_surface_input_inline tests.test_passive_graph_surface_host tests.test_passive_image_nodes -v` | Crop button, crop handles, and apply/cancel actions publish direct ownership while crop mode still locks host interactions through `blocksHostInteraction`; the `editSource` toolbar action opens the native file dialog directly |
