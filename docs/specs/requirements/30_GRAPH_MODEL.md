...
- `REQ-GRAPH-012`: passive visual nodes shall remain in `WorkspaceData.nodes` and passive `flow` connections shall remain in `WorkspaceData.edges` so selection, copy/paste, duplicate, view state, and save/load stay on the existing graph-document path.
