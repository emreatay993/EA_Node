...
- `REQ-UI-034`: executable graph-node chrome shall expose run-scoped node execution visualization from the existing worker event stream: running nodes use a blue pulse halo, completed nodes emit a green completed flash and then keep a static green border until the run ends, failure red remains the highest-priority visual state, inactive workspaces stay unhighlighted, and the QML-local elapsed timer remains a shell-only presentation detail rather than adding Python timestamp bridges or `.sfe` persistence.
...
- `AC-REQ-UI-034-01`: shell/controller, host-chrome, and QML-preference regressions confirm the blue pulse halo, green completed flash plus retained green border, failure red priority, active-workspace-safe canvas projection, and QML-local elapsed timer, with retained proof summarized in `docs/specs/perf/NODE_EXECUTION_VISUALIZATION_QA_MATRIX.md`.
