...
- `AC-REQ-UI-020-01`: standard executable nodes and passive flowchart/planning/annotation/media nodes all render through the shared host, and inline title editing on standard, passive, collapsed, and scope-capable shells reuses the shared header editor without breaking existing `graphNodeCard` / `graphCanvas` discoverability contracts.
