...
- `REQ-NODE-019`: graph validation and connection authoring shall use registry-aware multiplicity rules so passive `flow` inputs can opt into multiple incoming edges without weakening executable-port validation.
...
- `AC-REQ-NODE-019-01`: graph-scene and runtime-wiring regressions verify registry-declared multi-connection `flow` inputs are accepted without regressing exec/data edge validation.
