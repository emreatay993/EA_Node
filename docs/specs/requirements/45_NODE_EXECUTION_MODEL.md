...
- `REQ-NODE-027`: node execution visualization shall stay run-scoped and protocol-neutral: `node_started` and `node_completed` remain the only event sources for running/completed highlights, the lookups stay ephemeral shell/QML state, `run_started`, `run_completed`, `run_stopped`, and fatal worker-reset failures clear the highlights, non-fatal `run_failed` preserves the last execution context, and the feature shall add no `.sfe` persistence fields or worker protocol payload expansion.
...
- `AC-REQ-NODE-027-01`: run-controller, bridge-contract, and main-window canvas regressions confirm `node_started` / `node_completed` projection, active-workspace bridge filtering, `run_started` / `run_completed` / `run_stopped` cleanup, and non-fatal `run_failed` preservation without protocol or persistence expansion.
