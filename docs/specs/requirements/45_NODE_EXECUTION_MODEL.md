...
- `REQ-NODE-027`: node execution visualization shall stay run-scoped and protocol-neutral across node chrome and authored control edges: `node_started` and `node_completed` remain the running/completed node inputs, `node_completed` progresses authored `exec` / `completed` control edges, `node_failed_handled` progresses authored `failed` control edges when execution continues, the lookups stay ephemeral active-workspace-filtered shell/QML state frozen from the run-start authored workspace snapshot, `run_started`, `run_completed`, `run_stopped`, and fatal worker-reset failures clear the visualization, non-fatal `run_failed` preserves the last execution context, and the feature shall add no `.sfe` persistence fields, project-file schema changes, or second execution-visualization revision channel.
...
- `AC-REQ-NODE-027-01`: worker, run-controller, bridge-contract, and canvas regressions confirm `node_started` / `node_completed` projection, `node_failed_handled` failure-edge progression, active-workspace bridge filtering, shared revision-path invalidation, `run_started` / `run_completed` / `run_stopped` cleanup, non-fatal `run_failed` preservation, and no persistence expansion.
