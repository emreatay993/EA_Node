...
- `REQ-NODE-026`: PyDPF viewer execution shall keep playback and live/proxy state session-owned: `dpf.viewer` may publish proxy summaries into graph state, but live promotion, demotion, one-live arbitration, and reopen-from-proxy behavior shall occur through the worker viewer session service instead of multi-set field payload propagation.
...
- `AC-REQ-NODE-026-01`: viewer-node, bridge, and viewer-surface regressions confirm session-owned live/proxy transitions, default one-live behavior, and reopen-from-proxy restoration without reintroducing multi-set viewer payloads into ordinary graph execution.
