...
- `REQ-NODE-027`: node execution visualization shall stay run-scoped and protocol-neutral across node chrome and authored control edges: `node_started` and `node_completed` remain the running/completed node inputs and may carry additive `started_at_epoch_ms` / `elapsed_ms` timing metadata, `node_completed` progresses authored `exec` / `completed` control edges, `node_failed_handled` progresses authored `failed` control edges when execution continues, shell fallback timing computes elapsed values from transient started-at state when those additive fields are absent, the lookups stay ephemeral active-workspace-filtered shell/QML state frozen from the run-start authored workspace snapshot, per-workspace cached elapsed values remain session-only and separate from running/completed chrome, `run_started` keeps cached elapsed values for an otherwise unchanged workspace while clearing transient running/completed visualization and transient started-at state, `node_completed` overwrites the cached elapsed value for that node only, stopped or failed runs update only nodes that actually completed, execution-affecting edits plus project/session replacement invalidate cached elapsed data, `run_completed`, `run_stopped`, and fatal worker-reset failures clear only the transient visualization, non-fatal `run_failed` preserves the last execution context, and the feature shall add no `.sfe` persistence fields, project-file schema changes, session-restore payloads, or second execution-visualization revision channel.
...
- `AC-REQ-NODE-027-01`: worker/client, run-controller, project-session, bridge-contract/QML-boundary, history/invalidation, and canvas/footer regressions confirm additive `started_at_epoch_ms` / `elapsed_ms` timing metadata, shell fallback timing, `node_failed_handled` failure-edge progression, active-workspace bridge filtering, shared revision-path invalidation, execution-affecting cache clears, `run_started` / `run_completed` / `run_stopped` cleanup, non-fatal `run_failed` preservation, persistent cached footer behavior, and no `.sfe` persistence expansion.
