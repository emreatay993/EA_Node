...
- `AC-REQ-PERSIST-023-01`: plugin-loader, serializer, schema-migration, and registry-validation regressions confirm add-on enabled-state plus pending-restart persistence and missing-add-on placeholder round-trip or rebind behavior, with retained proof summarized in `docs/specs/perf/ADDON_MANAGER_BACKEND_PREPARATION_QA_MATRIX.md`.
