...
- `REQ-PERF-010`: node-execution visualization shall stay on the packet-owned host-chrome path: the QML-local elapsed timer keeps no Python monotonic timestamp bridge or completed-duration persistence, running/completed halos may only add render-active and stacking exceptions, and the feature shall not mutate graph geometry, hit-testing contracts, selection semantics, or cached scene payload structure.
...
- `AC-REQ-PERF-010-01`: `tests/test_passive_graph_surface_host.py`, `tests/graph_track_b/qml_preference_bindings.py`, and `docs/specs/perf/NODE_EXECUTION_VISUALIZATION_QA_MATRIX.md` record cache-key updates, render-active/z-priority behavior, the QML-local timer constraint, and preserved geometry/hit-testing contracts for the shipped execution chrome.
