...
- `REQ-ARCH-016`: the PyDPF viewer subsystem shall layer `RuntimeHandleRef` transport, worker-local handle and viewer services, `DpfRuntimeService` materialization, and shell-owned `ViewerSessionBridge` plus `EmbeddedViewerOverlayManager` orchestration so DPF-backed live views stay out of ordinary graph payloads.
...
- `AC-REQ-ARCH-016-01`: runtime-handle, DPF service, viewer-session bridge, overlay-manager, and live-policy regressions confirm the PyDPF viewer stack stays split across transport, worker services, and shell-owned live-view orchestration, and `docs/specs/perf/PYDPF_VIEWER_V1_QA_MATRIX.md` remains the published closeout audit.
