...
Overall status: **IN_PROGRESS — T01–T04 ACCEPTED; NEXT T05**.
...
- Current milestone: T04 is independently accepted; create its dedicated commit, then fresh Sol T05 owns native model opening/discovery, bulk transfer and owner entrypoint integration. T05–T18 remain required with separate commits and independent reviews; final T18 review uses Astra. No current decision is unanswered.
...
| T04 | Shared canvas/inspector metadata selectors | T02–T03 | ACCEPTED | t04_selectors (Sol) / t04_shared_ui_review (Sol) | `T04 Share metadata-driven node selectors` | 103 tests +212 subtests and QML7 PASS; independent 44-case/real-bridge review approved locator/cache/focus/Signal migration/50 cap. Hygiene PASS. Synthetic Mechanical specs exercise production bridge/registration; declarations remain T05. |
| T05 | Open standalone and Workbench models | T01–T04 | NOT_STARTED | — / — | — | Native family open/copy, registration/discovery/Info. Bulk route: private session spool with strict relative descriptors, regular/reparse/path/size/hash checks, existing scientific/PNG decode, combined budgets and eager deletion; values survive owner close. Frozen route: private allowlisted bootstrap role before Qt/app, source/frozen command selection and main(argv), no general launcher/extra EXE. |
