...
Overall status: **IN_PROGRESS — T01–T03 ACCEPTED; NEXT T04**.
...
- Current milestone: T03 is independently accepted; create its dedicated commit, then assign fresh Sol T04 metadata-selector work. T04–T18 remain required in dependency order with separate local commits and independent reviews; final T18 review uses Astra. No current decision is unanswered.
...
| T03 | Fresh-run owner process/session lifecycle | T01–T02 | ACCEPTED | t03_sessions (Sol) / t03_lifecycle_review (Sol) | `T03 Own Mechanical sessions within execution runs` | 197 tests +160 subtests PASS; independent 9-case safety/real-worker check and natural parent-exit proof PASS. Retry-safe retained cleanup, daemon worker + job-owned subprocess, exact correlation/EOF and workspace retirement approved. T05 owns frozen/native-file/bulk routes. |
| T04 | Shared canvas/inspector metadata selectors | T02–T03 | NOT_STARTED | — / — | — | Read-only route lookup: reuse PropertyEditAdapterContext/build_property_items_with_adapters; replace factory's direct plot.signal branch, generalize Inspector/canvas refresh trackers, preserve focus deferral, and cap filtered shared combo suggestions at 50. Exact whole-DataTree locator matching; no backend/source I/O. |
| T05 | Open standalone and Workbench models | T01–T04 | NOT_STARTED | — / — | — | Implement native family open/copy, registration/discovery/Info and bulk transport beyond T03 controls. Read-only launch lookup found no frozen module helper: add one private allowlisted owner role at bootstrap before Qt/app imports, source/frozen command selection and main(argv); verify command/dispatch, no generic launcher or extra production EXE. |
