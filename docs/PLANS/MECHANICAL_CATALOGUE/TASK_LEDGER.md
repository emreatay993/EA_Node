...
Overall status: **IN_PROGRESS — T01–T02 ACCEPTED; NEXT T03**.
...
- Current milestone: T02 is independently accepted. Create its dedicated commit, then start fresh Sol T03 ownership for run-scoped session/process lifecycle. T03–T18 remain required in dependency order with separate commits and independent reviews; final T18 review uses Astra. No current decision is unanswered.
...
| T02 | Mechanical semantic value and catalogue contracts | T01 | ACCEPTED | t02_contracts (Sol) / t02_boundary_review (Sol) | `T02 Add Mechanical runtime value contracts` | 48 focused/add-on and 131 typed/scientific checks PASS; independent boundary review approved nullable cameras, row applicability, typed selectors, discovery-only Info and no-Ansys-import discovery. Ruff/maps/index/links PASS. Runtime core unchanged. |
| T03 | Fresh-run owner process/session lifecycle | T01–T02 | NOT_STARTED | — / — | — | Reuse existing workspace_id transport and WorkerServices cleanup_run/reset. Read-only route lookup identifies missing workspace retirement fan-out across all live concrete clients. Account for active workspace closure and overlapping headless clients without global reset or affecting workspace B; prove lease-independent cleanup and same-workspace retirement. |
