...
- A Group's `held_member_ids` (stored member list of a collapsed Group or of a Group hidden inside one) is authored node state on the normal document path, added without a schema bump: `record_payloads.node_instance_to_mapping` emits it only when it is not `None`, and `node_instance_from_mapping` reads it leniently even for strict payloads (non-empty strings, deduped and sorted; anything else becomes `None`; the node is never rejected, so a paste or a custom workflow never fails on it). Load runs `ea_node_editor/graph/hierarchy.py` `sanitize_workspace_held_member_ids` after `sanitize_workspace_parent_links`: dangling, self, and cross-parent ids are dropped, Group-to-Group list cycles are broken, and lists survive only on collapsed or hidden Groups. It is not one of the stripped runtime membership keys; documents without it load with `None`, and the scene later derives a list for an older collapsed Group from geometry. `tests/test_serializer.py::test_held_member_ids_round_trip_and_are_sanitized_on_load` owns the round trip and the sanitize rules.
