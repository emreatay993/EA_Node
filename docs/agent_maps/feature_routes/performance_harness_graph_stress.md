...
- The `animated_media` scenario uses `--nodes` with `--edges 0` and deterministic fixtures under `tests/fixtures/media/`. It reads the generic `GraphNodeHost.inVisibleViewport` fact, then reports confirmed animator instances, visible/playing/idle/offscreen counts, preview states, policy violations, process CPU, and RSS. Offscreen/software runs are regression evidence; retain three-run 20- and 100-panel CPU/RSS results as release baselines rather than absolute resource gates.
