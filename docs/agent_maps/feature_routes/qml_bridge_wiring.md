...
- `ea_node_editor/ui_qml/graph_canvas_state/` — state bridge package. The composition root (`__init__.py`) owns construction + source resolution ONLY; production composition supplies `ShellWorkspacePresenter` as the persisted-graphics source and `GraphCanvasPresenter` as the canvas/session source. Persisted graphics never fall back through the canvas source; only minimap-expanded, selected-run preview, snap enabled, and snap size come from it. A new QML-visible canvas fact is one `@pyqtProperty` in the matching projection mixin (`graphics_preferences_props`, `execution_state_props`, `scene_models_props`, `view_state_props`) — each property must live with its notify signal's mixin. Viewport-virtualization logic (hysteresis/bucketing/delta sync and the `visible_*` projections) lives in `visible_scene_service.py`.
...
- `ea_node_editor/ui_qml/graph_canvas_command/` — command bridge package. The composition root (`__init__.py`) owns construction + source resolution ONLY; persisted graphics slots target the explicit `ShellWorkspacePresenter`, snap/minimap/selected-run-preview slots target `GraphCanvasPresenter`, and Media Image/Video slots target the concrete `MediaPanelActionService` without a new protocol. A new canvas command is one `@pyqtSlot` in the matching per-domain ops module (`graphics_settings_ops`, `viewport_ops`, `scene_mutation_ops`, `media_video_ops`, `node_creation_ops`, `annotation_style_ops`, `canvas_host_ops`, `folder_explorer_ops`) plus its existing concrete/protocol owner where applicable. `tests/test_graph_canvas_command_ops_modules.py` guards mixin slot registration and cross-domain name collisions.
