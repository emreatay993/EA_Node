...
- Passive node and flow-edge style actions are declared as graph action contracts, routed by `GraphCanvasActionRouter.qml`, and dispatched through `GraphActionController` directly to `GraphCanvasHostPresenter`. Passive node style actions surface in the node context menu; flow-edge Edit Label, Copy/Paste/Reset Style, and More settings (the style dialog) surface only on the selected flow-edge floating toolbar (`GraphActionPresentation.edgeToolbarActions`), and the edge context menu deliberately omits them. That presenter owns dialog/preset/clipboard/label policy and calls the graph scene mutation owner; `ShellHostPresenter` and `ShellWindow` are not on this route.
