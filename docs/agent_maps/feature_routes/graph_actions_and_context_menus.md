...
- `ea_node_editor/ui_qml/graph_scene_mutation/alignment_and_distribution_ops.py` (scene move and resize commits, align, distribute, Set Same Width/Height, and straighten; the pure builders are in `ea_node_editor/graph/transform_layout_ops.py`)
...
- Passive node and flow-edge style actions are declared as graph action contracts, routed by `GraphCanvasActionRouter.qml`, and dispatched through `GraphActionController` directly to `GraphCanvasHostPresenter`. Passive node style actions surface in the node context menu; flow-edge Edit Label, Copy/Paste/Reset Style, and More settings (the style dialog) surface only on the selected flow-edge floating toolbar (`GraphActionPresentation.edgeToolbarActions`), and the edge context menu deliberately omits them. Reverse Direction (`reverse_flow_edge`) is on both the toolbar and the edge context menu, shown for flow edges whose payload is `reversible`. That presenter owns dialog/preset/clipboard/label policy and calls the graph scene mutation owner; `ShellHostPresenter` and `ShellWindow` are not on this route.
...
- Measurement: align and distribute place nodes by the rectangles the canvas draws, read for the whole selection in one call to `ea_node_editor/ui_qml/graph_scene_mutation/group_scope.py::scene_layout_bounds` (the cached payload, else the payload builder's measure) from `GraphSceneMutationHistory._selected_layout_metrics`. Canvas smart guides snap to the same drawn rectangles: `GraphCanvasStateBridge.smart_guide_snapshot` reads the cached payload rects through the viewport indexes, and `tests/test_graph_canvas_smart_guide_snapshot.py` pins them to `scene_layout_bounds`. The plain surface measure (`resolved_node_surface_size`) leaves out the settings band (`plot.signal` is drawn 128 px tall but measures 56; with every settings group open the gap passes 1000 px) and counts the rows a view that hides optional ports leaves out. Set Same Width/Height deliberately keeps that plain measure (`_effective_node_size`): it writes custom sizes, which count those hidden rows, and passive nodes have no settings band because settings groups are active-only. Resize commits take the drawn size instead: `set_node_geometry` / `resize_node` (the canvas resize handles and fits through `ea_node_editor/ui_qml/components/graph_canvas/GraphCanvasNodeDelegate.qml` `onResizeFinished`, and automation `node.update`) clamp to the drawn minimum (`_minimum_node_size` reads the payload's `surface_metrics`), then add `GraphScenePayloadBuilder.hidden_port_rows_height`, so a node resized in a view that hides optional ports keeps the released size (a Web Page Viewer released at 418 px used to draw at 382). Group wrap/grow and Tidy write Group backdrops, which hide no rows. `tests/test_graph_scene_layout_measure.py` pins these rules, the resize one over every type that keeps a custom height; `tests/test_passive_graph_surface_host.py` (`WebPageGenericSurfaceQmlTests`) drags the real canvas handle.
...
- Semantics owned by `tidy_layout_ops.py`, with the scope geometry (drawn rectangles, membership, owner growth, guard) from `ea_node_editor/ui_qml/graph_scene_mutation/group_scope.py`: one undo step, or none when nothing changes. Selection ids are normalised by the scene selection rules, and hidden collapsed-Group members are excluded. A selected Group backdrop pulls in its contents, but becomes a fixed obstacle if it holds a node the user cannot select. Expanded groups are laid out recursively and refitted; collapsed groups move rigidly with their hidden members and are laid out at pill size at every level (`GroupScope.rects`), and because a collapsed Group belongs to its parent by its pill's top-left corner, the parent is refitted around the pill rather than the expanded area. An older collapsed Group without a stored member list still reserves its expanded size. Tidy on the members of a peeked collapsed Group grows that Group's expanded size and stops there (`grow_owner_chain`). Selected nodes inside an unselected backdrop form their own block (laid out before the top-level block), and that owner chain grows only around members that moved. Each block then steps off what nothing will move for it: inside a Group every other member; at the top level locked content and backdrops holding tidied nodes. Unwired annotations are never re-arranged; inside a laid-out group they are moved clear of the members. Other top-level neighbours are pushed off each tidied rectangle in one pass (`transform_layout_ops.build_collision_avoidance_position_updates`), only when expand collision avoidance is on, whatever its Make room or Nearest strategy. Snap-to-grid is ignored. A membership guard (`GroupScope.owner_changes`) rejects the whole command when any node would enter or leave a Group; nodes over a collapsed Group's hidden area are not its members, so they are no conflict.
- Swimlane pools are laid out as one unit (`TidyItem.swimlane_pool` / `swimlane_lane`, `_resolve_swimlane_pool` in `ea_node_editor/graph/transform_tidy_layout.py`): layer columns along the pool's flow shared by every lane, one row per lane, lanes sized to what they hold and restacked; a selected lane pulls its pool in, and a settle pass (`swimlane_ops.settle_swimlanes`) runs in the same undo step. The pool and lane context-menu rows (Add Lane, Insert/Move/Remove Lane, Tidy Lanes, Switch to Vertical/Horizontal Lanes; a standalone lane has no move rows) are node-local rows (`node_context::swimlane_*`), not graph action IDs; see [Swimlane Pools And Lanes](swimlane_pools_lanes.md). A standalone lane is tidied as a pool without lanes.
