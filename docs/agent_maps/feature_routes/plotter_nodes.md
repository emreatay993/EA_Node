...
## Purpose
Use this for plotter node planning, backend registry/static export work, generic and DPF-typed node family routing, direct and extraction-ref tabular auto-plotting, display surfaces, packet docs, and retained plan paths.
...
- Plot inspector controls project JSON-backed plot properties such as `axis_limits`, `log_scales`, `tabular_mapping`, `plot_options`, and DPF `frame_selector` into structured rows such as numeric fields, toggles, searchable combos, and chip lists. Do not expose these plot fields as raw JSON textareas in the inspection pane.
- 2D live plot investigation options (`plot_theme`, `hover_readout`, `vertical_guide`, `crosshair`) persist inside `plot_options`, project through `PlotPropertyEditAdapter`, and are mirrored by fullscreen quick controls through `ContentFullscreenBridge.set_active_plot_option`; keep the live interaction behavior in pyqtgraph-only paths unless another backend explicitly implements it.
...
- Generic and DPF typed plot nodes share a required List-access `series` input. Plot sinks/side effects consume whole inputs through the audited data-access contract and expose no execution/completed/failed ports; keep catalog contract tests aligned when changing plot definitions.
...
## Update Triggers
Update when plot backend registry contracts, backend capability flags, export behavior, plot List/Tree access declarations, optional plot dependencies, generic or DPF-typed node definitions, structured plot-inspector control fields, tabular-to-plot mapping behavior, tabular auto-preview scene payloads, DPF plot adapters/frame metadata, plot app preferences, legacy plot-session key stripping, sink-mode scene payloads, graph surface support, inline preview/view-state cache behavior, detached-window hosting, packet paths/prompts, or plotter verification changes.
