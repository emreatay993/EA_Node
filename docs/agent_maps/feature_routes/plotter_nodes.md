...
## Start Here
- `ea_node_editor/nodes/builtins/plot/specs.py` — inert seven-node declarations, fresh defaults, immutable editor roles and separate runtime validation order
- `ea_node_editor/nodes/builtins/plot/generic.py` — one render-request builder shared by execution and previews, plus plugin orchestration; package initializer has no re-exports
- `ea_node_editor/nodes/builtins/plot/exports.py` — full-fidelity source exports and paired artifact publication with validated failure cleanup
- `ea_node_editor/nodes/builtins/plot/data_series.py` — values, shared table/window selection and reading, numeric preparation, bounded full-array sampling, explicit slices, source provenance and runtime column parsing
- `ea_node_editor/nodes/builtins/plot/property_edit_adapter.py` — inspector-specific coercion; metadata imports never load generic execution
- `scripts/verify_plot_visual_parity.py` — production Signal Plot/XY reduced-versus-full PNG proof and canonical sample equality; fullscreen interaction is separately covered
- `tests/test_plot_specs.py` — import boundaries, fresh defaults and immutable role metadata
- `tests/test_plot_node_contracts.py` — catalogue and declaration contracts
- `tests/test_generic_plot_exports.py` — full-source table/array exports, backend fallback and artifact cleanup
- `tests/test_plot_auto_preview_service.py` — asynchronous preparation and shared-cache lifecycle
- `tests/test_plot_visual_parity_tool.py` — line/scatter PNG parity and a real reduction/threshold failure
...
- `scripts/benchmark_generic_plot.py` — source-selected Bar/Heatmap preparation and full-export baselines; timing and loader counters run separately
- `tests/test_plot_graph_ownership_benchmarks.py` — benchmark provenance, deterministic outputs, bounded reads and graph-copy counters
- `tests/test_generic_plot_preparation.py` — preparation, direct/window export provenance, runtime-versus-inspector mapping policy, caller parity, source-read counts and diagnostics
- `tests/test_plot_render_request.py` — JSON-safe render-request transport
- `tests/generic_plot_fixtures.py` — QML-free contexts and request construction shared by preparation and export/integration tests
...
```powershell
.\venv\Scripts\python.exe -m pytest -n 0 tests/test_generic_plot_preparation.py tests/test_plot_render_request.py tests/test_plot_series_decimation.py tests/test_tabular_perf_guards.py -q
.\venv\Scripts\python.exe -m pytest -n 0 tests/test_plot_node_contracts.py tests/test_generic_plot_exports.py tests/test_plot_headless_export.py tests/test_registry_validation.py tests/test_passive_runtime_wiring.py --ignore=venv -q
...
$env:QT_QPA_PLATFORM='offscreen'; .\venv\Scripts\python.exe -m pytest -n 0 tests/test_graphics_settings_preferences.py tests/test_graphics_settings_dialog.py tests/graph_track_b/qml_preference_bindings.py tests/test_plot_surface_integration.py --ignore=venv -q; $exitCode = $LASTEXITCODE; Remove-Item Env:QT_QPA_PLATFORM -ErrorAction SilentlyContinue; exit $exitCode
...
- Signal Plot is the sole function-SDK plot conversion: its full grouped-control contract is statically parsed from the internal inert source and executed through the verified function bundle, while rendering remains execution-owned. The seven specialized generic plots remain trusted descriptors.
- The generic family contains seven specialized nodes. `plot.line` and `plot.scatter` are removed without aliases or migration; registry-backed loading rejects their saved documents unchanged. Signal Plot owns line/scatter authoring: marker-only uses Line styles None and a nonzero Marker shape, with Media Panel/Image Export consumers. Shared generic fixtures use Bar.
...
- `data_series.py` resolves direct tables and explicit windows into one frozen selection and performs one column-array read. Direct-table provenance omits a column restriction; window provenance preserves the complete admitted columns, offset and minimum positive limit independently of plotted columns. Full-array sampling and authored-slice loading remain different strategies. `generic.py` copies request mappings once; execution adds fresh defaults once before invoking the same builder used by previews.
- Generic plot `series` compatibility is declared by runtime shape instead of the abstract Graph Data root: bar/histogram accept numeric, array/dictionary, and tabular/array-reference sources; heatmap/contour/surface accept array/dictionary and reference sources; point-cloud/streamline accept dictionary and reference sources. Keep Path and unrelated graph-root descendants out of these accepted-type unions while retaining runtime checks for genuinely abstract producers.
...
- Tabular/array plot series are decimated to `TABULAR_PLOT_MAX_POINTS_PER_SERIES` (4000) through `execution/plot_series_decimation.py` — min-max envelope for x/y series (global extremes always survive), stride sampling for values/grids/points. Each series carries `decimation` metadata and a `source_ref`; `nodes/builtins/plot/exports.py` streams the FULL source table/array through the shared loader service, so decimation never reduces export fidelity. One uniform-source CSV preflight retains table, array, then backend fallback order. Paired export publication records attempted IDs before registration and independently attempts validated entry cleanup, path cleanup and persistence on any `BaseException`, preserving the original failure. An explicit `tabular_mapping.row_limit` is a hard source-row cap applied before decimation. The legacy `preview_series` plot property is deleted; `project_codec._copy_node_mapping` strips it from legacy documents on load and save.
