...
- Signal Plot consumes immutable scientific values, numeric trees/matrices, numeric-text plain sequences/Panel Data branches, and table/array/window/slice refs through `signal_plot_inputs.py`. Numeric text is declared explicitly at the Values port and coerced only in plain sequences; typed tabular text columns remain non-numeric. Shared column IO remains tabular-owned. The default 4000-point reduction is per rendered trace; zero is full resolution, gaps remain gaps, and source values are never reduced. Generic plot defaults remain independent.
