...
- Python Script declaration controls use the same resolved-spec projection: its AST parser supplies only ordinary ports, properties, and groups before this builder runs. Keep Python Script out of dynamic-port payloads and add no script-specific renderer facts.
...
- Preview helpers derive from the existing output cache only: Item safe value; List count plus up to eight indexed values; Tree branch/item counts plus up to eight paths and eight items per shown branch; each text sample is at most 160 characters. Structured preview `rows` plus a `truncated` flag reuse those bounds. The Panel requests its complete current non-stale tree and Copy/Copy as tree text through dedicated bridge slots only when displayed or invoked; every displayed/copied item still passes through the same callback-free projector, so full DataTrees, arrays, tables, handles, or plugin values never enter scene payloads.
...
- `visual_style` is passive-only in the active scene: style mutations on active and `compile_only` nodes are ignored without a history entry, and their scene payloads omit the field. Passive and unresolved node styles keep the existing projection path.
...
## Update Triggers
Update when payload fields, compact data-type projection/catalog generation, compatible-endpoint snapshots, dynamic-port groups, geometry, authored/display/condition/sensitive presentation, Interval 1D or searchable-enum projection, access/modifier/Principal or edge-enabled projection, bounded node/port rich previews, compact-pill title/geometry mutations, durable node link/comment projection, data-wire presentation, projection ownership, scene mutation routing, plot embedded-render suppression metadata, or scene bridge tests change.
