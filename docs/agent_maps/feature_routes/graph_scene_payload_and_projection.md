...
- `tests/test_type_forwarding_ui.py`
- `tests/test_rewire_validation.py`
...
- Each scene payload build and library-registry refresh creates one compact data-type presentation projection from one catalog `snapshot()` and one `fingerprint()`, then reuses it across all ports. Per-port QML facts include the canonical type ID, bounded display/family/color/icon/access/generation fields, and at most eight accepted or inferred display labels of 160 characters each; assignability, conversions, compatibility edges, and the catalog graph never enter a port payload. Library rows retain authored primary/accepted IDs only for Python-owned Quick Insert compatibility.
- Invocation-scoped `GraphTypeResolver` facts feed ports and edge warnings. Output `source_type_ids` and `source_types_unresolved` remain distinct from input `accepted_data_types`; all semantic IDs survive the display-label cap. `publish_edge_topology_delta` and `publish_node_payload_delta` compare old/new contracts and topology to refresh affected downstream nodes and publish every pruned edge, including Undo/Redo. Catalog identity remains separate from graph-contract revision.
- `GraphSceneMutationPolicy.compatible_endpoint_snapshot(...)` resolves current in-scope inferred sources and target accepted unions, returning `catalog_generation`, `candidate_role`, and `compatible_endpoint_ids`. QML interaction state separately tracks the graph-contract revision and refreshes active drag/rewire/click snapshots once per revision, failing closed on malformed or generation-mismatched data. Rewire candidates share one graph-owned prepared proposal session; final mutation still validates fresh topology.
...
- Group membership fields (`GROUP_BACKDROP_MEMBERSHIP_FIELDS` from `ea_node_editor/graph/group_backdrop_geometry.py`, plus a Group's `expanded_occupied_bounds`) come only from a full build or the cached-candidate addition path. `replace_cached_full_node_payloads` rebuilds one node at a time, so while Groups are drawn it carries them over from the payload it replaces (`_GraphSceneBackdropPartitioner.carry_group_backdrop_membership_payload`), or returns `None` (counter `payload_cache_group_membership_fallback`) when the node is redrawn at another place or size; `publish_node_payload_update` then counts a slot mismatch only when the cache saw one. `publish_edge_topology_delta` takes the full rebuild when a removal changes membership or what is drawn (`ScenePayloadCacheSync.removal_changes_group_membership`). The Group route map owns the rules.
- `_GraphSceneNodePayloadFactory.presentation_bounds` measures the rectangle a payload build draws a node at, or would draw it at expanded, from the presentation layout alone (settings band and view-filtered ports included; no payload, port anchors or icon). Presentation facts use it for `expanded_bounds`, and `GraphScenePayloadBuilder.measure_node_layout_bounds` serves Group layout for nodes without a drawn payload.
- A custom height is stored counting every port row: `_GraphSceneNodePayloadFactory._filtered_layout_node` draws a custom-sized node shorter by the rows a hide-optional-ports view leaves out (`_hidden_port_rows`, never below the filtered minimum). `hidden_port_rows_height` returns that difference for the node drawn expanded, so a drawn height plus it is the height to store; `GraphScenePayloadBuilder.hidden_port_rows_height` serves it for the workspace's active view, and the resize commit (`ea_node_editor/ui_qml/graph_scene_mutation/alignment_and_distribution_ops.py::set_node_geometry`, owned by the graph-actions route) adds it.
...
- Group membership candidates in the full build (`builder.py`, `backdrop_partitioner.py`) and in the cached-candidate delta path (`ea_node_editor/ui_qml/graph_scene/payload_cache_sync.py`) carry the honoured stored list (`group_backdrop_geometry.honoured_held_member_ids` over `hidden_node_ids`) and take their size from `membership_rect_kind`: a collapsed Group with a stored `held_member_ids` list is a `CORNER_CANDIDATE_SIZE` square at its top-left and owns exactly its listed ids, so only those are hidden and proxied while nodes over its hidden area stay drawn; an older collapsed Group without a list keeps its expanded size (`factory.py` `membership_candidate_size`); every other node uses its drawn size. The cached path still falls back to a full rebuild whenever a backdrop is collapsed.
