...
- `GraphCanvas.nativeOverlaySuppressionActive` is the shared presentation fact for viewport interaction, live node drag, live resize geometry, and active wire drag. Explicit press/hold viewport gestures use `viewportInteractionHeld`; wheel events keep timer-based recovery. Native Model Viewer overlays switch to their lightweight proxy while suppression is true; generic node press/resize handlers must not clear viewer focus or own viewer-session lifetime.
