...
## Purpose
Use this for graphics settings, shell themes and the shell-derived graph palette, app preferences, and app-wide visual preferences.
...
- The canvas options menu is a compact QML entry point for graphics preferences, including `graphics.canvas.background_variant` for dark/white canvas fills; persisted changes route through `GraphCanvasCommandBridge.graphics_source` to `ShellWorkspacePresenter`, not project persistence. The top-right gear visibility is app-wide at `graphics.canvas.show_canvas_options_button` and projects through the graph-canvas state bridge.
...
- Expand collision avoidance is app-wide at `graphics.interaction.expand_collision_avoidance` (Graphics Settings > Interaction > Expand Collision Avoidance, "Avoid overlaps when expanding collapsed items"). `EXPAND_COLLISION_AVOIDANCE_STRATEGY_CHOICES` in `ea_node_editor/settings.py` offers "Make room" (`make_room`, the default: the row after a grown item and the column below it shift just enough, level by level through parent Groups) and "Nearest" (`nearest`: each overlapping item moves to its nearest free spot). `DEFAULT_EXPAND_COLLISION_AVOIDANCE_SETTINGS` carries `strategy_revision` 2 (`EXPAND_COLLISION_AVOIDANCE_STRATEGY_REVISION`). `normalize_expand_collision_avoidance_settings` in `ea_node_editor/app_preferences.py` upgrades a saved `nearest` without a `strategy_revision` of at least 2 to `make_room` once, at read time only (nothing is written until the user saves Graphics Settings), and always outputs `strategy_revision: 2`; the dialog's values dict writes `strategy_revision: 2`, so an explicit Nearest sticks. With the option off, overlaps stay, but non-members that would end up inside an expanding Group are still moved out and parent Groups still grow. Tidy's own neighbour push ignores the strategy. The scene reads the setting in `ea_node_editor/ui_qml/graph_scene_mutation/collision_avoidance_ops.py`; the tooltips live in `ea_node_editor/ui/tooltips/settings.json`. Focused proof is in `tests/test_graphics_settings_preferences.py` and `tests/test_graphics_settings_dialog.py`.
...
- Tooltip appearance delay is app-wide at `graphics.shell.tooltip_delay_ms`, defaults to 400 ms, and accepts 0-5000 ms. Keep the Graphics Settings > Layout > Tooltips spin box, Canvas Options tooltip-delay submenu, workspace presenter, graph-canvas state/command bridges, `GraphCanvasPreferenceFacts.qml`, and `ManagedToolTip.qml` aligned. Individual hover callers do not own their own delays; `delayOverride` is reserved for immediate non-hover feedback.
...
- Graph-theme node tokens are `Passive Node Defaults` only. They keep passive body gradients, title/icon foreground, and `card_selected_border` outline/glow; authored passive `visual_style.border_color` remains idle-only. Active and `compile_only` bodies, outlines, title/port/inline-control foregrounds, and selected fill use the fixed shell-brightness palette in `GraphNodeHostTheme.qml` and ignore graph-theme node tokens. Keep `ui/graph_theme/tokens.py`, `GraphThemeBridge.node_palette`, the Graphics Settings shadow preview (`NodePreviewWidget`), and passive QML rendering in sync. Add-on Manager category marks use the shell accent rather than a graph-theme category palette.
- Graph themes are not user-selectable. `ui/graph_theme/registry.py` holds only `graph_stitch_dark` and `graph_stitch_light`, and `ShellWorkspacePresenter.apply_graphics_preferences` applies `default_graph_theme_id_for_shell_theme(active_theme_id)` to `GraphThemeBridge`. There is no graph-theme picker, follow-shell toggle (Graphics Settings or Canvas Options), manager/editor, custom theme, or `graphics.graph_theme` preference; normalization drops a legacy `graph_theme` payload. Proof lives in `tests/test_graph_theme_shell.py`, `tests/test_graphics_settings_dialog.py`, and `tests/test_graphics_settings_preferences.py`.
- `GraphPortStateTokens` (`port_state_tokens` on `GraphThemeDefinition`, `GraphThemeBridge.port_state_palette`) owns valid-green, waiting, idle, and invalid grip colors. These state colors are shared per brightness (`GRAPH_STITCH_DARK/LIGHT_PORT_STATE_TOKENS_V1`); rest-state grips no longer use per-kind colors.
...
.\venv\Scripts\python.exe -m pytest tests/test_graph_theme_shell.py tests/test_graphics_settings_dialog.py --ignore=venv -q
...
## Update Triggers
Update when preferences storage/migration, graphics/canvas source ownership, scene graphics binding/fingerprint, routing of non-visual app preferences away from Graphics Settings, passive-default graph theme tokens, the fixed active-node palette boundary, theme bridges, graphics dialogs, node-library display or learned-usage preferences, shell pane collapse preferences, tooltip delay, node elapsed-time units, notched ports, node comment editor defaults, node floating-toolbar reveal behavior, text toolbar recents, expand collision-avoidance strategy or `strategy_revision`, plot/lightweight canvas preferences, or theme tests change.
