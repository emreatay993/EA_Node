...
- `GraphSceneBridge` reads the explicitly bound workspace graphics source, never a parent/window/controller lookup. Its rebuild fingerprint is `(show_port_labels, graph_label_pixel_size, node_title_icon_pixel_size, lightweight_canvas)`; thread those four values through full, targeted, and library-drop payload paths and do not add an imperative refresh for unrelated preferences.
