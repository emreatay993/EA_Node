...
## Common Changes
- Keep the public family fixed at Secret, SSH/SFTP Host, Run SSH Command, Run SSH Script, SFTP Upload, and SFTP Download. Their static port order, labels, types, access, descriptions, properties, category, keywords, and Help metadata live in `integrations_ssh_sftp.py`; registration stays on the normal `bootstrap.py` descriptor path.
- D042 records exactly two existing production types under `corex.ssh_sftp`: `SSH_SFTP_Connector.Nodes.Control.SSH_SFTP.SecretData` and `SSH_SFTP_Connector.Nodes.Control.SSH_SFTP.SshSftpHostData`. Both are native opaque tagged values, use `persistence="never"`, and are `transport_ready` in the crosswalk. Their production declarations in `integrations_ssh_sftp.py`, runtime handling in `ssh_sftp_runtime.py`, shared carrier codec in `runtime_contracts/runtime_values.py`, and `bootstrap.py` registration remain unchanged.
...
- Runtime `secret_data` and `ssh_sftp_host_data` values are immutable-by-convention tagged mappings, not live sessions. `runtime_values.py` preserves these opaque mappings through worker serialization; operation nodes decrypt credentials only while creating a fresh Paramiko client.
...
## Update Triggers
Update when SSH/SFTP node metadata, protected-value envelopes, tagged runtime values, Paramiko connection/command/script/transfer behavior, fixed public error/redaction behavior, sensitive-property projection or commands, retained HPC migration handling, dependencies, packaging, icons, or focused tests change.
