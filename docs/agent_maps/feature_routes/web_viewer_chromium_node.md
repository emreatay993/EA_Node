...
- Clipboard paste fallback creates `web.page_viewer` nodes by setting `start_location` for local HTML files, non-media remote URLs, remote URLs without a clear image/PDF/video suffix, and rich clipboard links with exactly one remote `href`. General browser-selected HTML snippets stay text annotations.
