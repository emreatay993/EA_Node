...
## Dataflow Port Notes
- `GraphNodePortsLayer.qml` keeps key-only models of visited input/output rows, reconciling topology and publishing changed row payloads without rebuilding surviving controls. Settings-group visibility hides and disables retained rows; unopened groups remain lazy and hidden geometry does not follow live layout. `GraphNodePortRow.qml` separates retained editor state from presented grip geometry, preserving full editor layout under clips and animated wire anchors. Host armed/running presentation prevents a transient collapse hide/show cycle. Hidden controls publish no hit rectangles, lose focus, and dismiss searchable popups; true port removal and node replacement discard retained rows.
