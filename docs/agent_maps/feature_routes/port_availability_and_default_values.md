...
- Normal active-data drop validates first and atomically replaces every incoming wire at that input. Shift-drop appends in persisted order. Ctrl-drag routes `GraphCanvasBridge.request_move_edge_endpoint` through the shell and `GraphInteractions` into `GraphSceneMutationHistory.move_edge_endpoint` and `ValidatedGraphMutation.move_edge_endpoint`; Ctrl+Shift appends, and blank release disconnects only that edge.
...
- Registry edge pruning reuses a per-pass `RegistryValidationPassMemo`; duplicate connections, input capacity, mutually exclusive DPF inputs, exposed/hidden ports, and subnode pin ports still resolve through graph-owned invariant checks.
...
## Update Triggers
Update when dynamic group declarations/resolution/stable-key behavior, explicit default/readiness/condition declarations, registry validation, typed-inline or native runtime fallback conversion, authored/display presentation facts, effective-port compatibility snapshots, availability/type-warning separation, optional filtering, default/waiting grip payload or chrome, endpoint reassignment, or focused tests change.
