...
- Hover badges/cards live in the graph canvas overlay path through `GraphNodeLinkHoverLayer.qml`; keep card visuals theme-aware, rooted in the mockup references above, and re-synced on scene node signals so removed nodes or links cannot leave stale cards behind.
- Both overlay layers resolve a card's node through `nodeResolver` (wired to `GraphCanvasRootLayers.nodePayloadForId`, i.e. the canvas `_sceneNodePayload` lookup covering regular nodes and Groups). Do not scan a `QAbstractListModel` such as `visible_nodes_model` from JS: it has no index access in QML, so lookups silently return nothing and cards close on the next scene change. Badge delegates read `visible_badge_nodes_model`, which is built from the merged visible regular-node and Group payloads.
...
.\venv\Scripts\python.exe -m unittest tests.test_group_backdrop_interactions.GroupBackdropInteractionTests.test_graph_canvas_comment_and_link_cards_resolve_regular_and_group_nodes -v
