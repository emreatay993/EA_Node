...
## Common Changes
- Execution property, port-flow and preview maps are cached until a source notification. The bridge clears them synchronously and coalesces their QML notification on the next event-loop turn. Selector metadata refresh is likewise queued through the scene bridge. QML facts read each map once per binding evaluation.
