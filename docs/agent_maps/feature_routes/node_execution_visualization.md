...
| --- | --- | --- | --- |
| Node lifecycle | `node_started` / typed `node_settled` update ShellRunState running, completed, warning, start-time, elapsed, and freshness fields through RunController. | ExecutionStateProps running/completed/warning/start-time lookups -> GraphCanvasExecutionFacts -> GraphNodeHost chrome. | Workspace-scoped and transient; terminal run cleanup clears lifecycle highlights while retained cache freshness stays session-only. |
| Node run count | Retained output records already store one entry per node and run. | `node_run_count_lookup` -> GraphCanvasExecutionFacts.nodeRunCountLookup -> shared node-help tooltip footer. | Session-only and workspace-scoped; it counts retained run records and does not add a second cache. |
| Settled result | `node_settled` carries completed/empty/failed/blocked node status plus per-output value(`DataTree`)/empty/failed results. | RunController caches only the typed bounded state needed by projections; full values remain in the existing output cache. | Failed dependents retain original root errors without cascade failures; independent branches may continue. |
...
| DPF summary | RunController stores a projected DPF summary beside each completed output record. | `dpf_workflow_summary_lookup` -> GraphCanvasExecutionFacts -> GraphDpfWorkflowSurface. | Chooses the latest observed record, exposes ready/stale state, and caps displayed facts at six; it does not recompute results. |
...
`node_settled` -> `RunController.handle_execution_event` -> `_cache_node_outputs` -> `ShellRunState.cached_node_output_records_by_workspace_id` -> `ExecutionStateProps.port_flow_state_lookup` -> `resolve_runtime_port_flow_states` in the [port-flow semantic resolver](../../../ea_node_editor/ui/support/port_flow_state.py) -> `GraphCanvasExecutionFacts.portFlowStateLookup` -> `GraphNodeHost.resolvedPortFlowState`.
...
- `ShellRunState.fresh_run_node_ids_by_workspace_id` and its `fresh_run_node_lookup` projection remain session-only runtime facts for cache/status consumers, but no longer change active-node body or glow. `RunController` still clears freshness only for execution-affected nodes plus downstream nodes through `graph.reachability.downstream_node_ids` when history makes cached run state stale.
