...
| Settled result | Runtime-enriched `node_settled` names the exact retained record ID/key/digest/disposition only when the store accepted it. | `solution_output_cache.retained_output_record(...)` selects that record for port/DPF/property/Panel/media/fullscreen/command/presenter/deck consumers. | Maximum two records per node, 4,096/512 MiB per workspace, and 16,384/2 GiB globally; oldest unpinned eviction is deterministic and unfit payloads become metadata-only. |
...
| DPF summary | RunController stores a projected DPF summary beside each accepted output record. | Retained-record selector -> `dpf_workflow_summary_lookup` -> GraphCanvasExecutionFacts -> GraphDpfWorkflowSurface. | Uses the fact-retained record rather than newest timestamp, exposes derived ready/stale state, and caps displayed facts at six. |
...
`generation callback` -> `CorexRuntime` store handling/enrichment -> execution event stream -> `ShellWindow.execution_event` -> `RunController.handle_execution_event` -> retained-record cache/fact projection -> `node_execution_state_changed` -> `GraphCanvasStateBridge` -> `ExecutionStateProps` / `GraphCanvasExecutionFacts`.
