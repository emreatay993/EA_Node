...
- Copy and duplicate expand a selected collapsed Group through its stored `held_member_ids` list (`expand_group_backdrop_fragment_node_ids` in `ea_node_editor/graph/transform_fragment_ops.py`), not through payload `contained_*` ids. Fragment insertion remaps each stored list through the new ids (pasted, same-parent members only), keeps a list only on a pasted collapsed Group or a pasted Group that a pasted collapsed Group lists, and then runs `sanitize_workspace_held_member_ids`. Inside the same history group, `GraphSceneMutationHistory._insert_fragment` fills lists for collapsed Groups from older fragments without capturing bystanders (`fill_missing_held_member_ids(workspace, inserted_node_ids)`) and adds nodes pasted during Peek to the peeked Group (`hold_new_nodes_in_peeked_group`).
...
- A settings-group toggle is one `toggle-settings-group` history entry containing declaration-ordered expansion state, exact custom-height correction, and the make-room moves and parent Group growth of an expand. A refused expand (a locked Group would have to grow, or no room without changing Group membership) restores the pre-toggle `WorkspaceSnapshot` and its `commit_if` gate records no entry. Toggling while the whole node is collapsed measures the hidden band with an isolated globally-expanded presentation probe so reopening preserves specialized body height.
- A collapse/expand toggle is one `toggle-collapsed` history entry containing the flag, the frozen or cleared Group member lists, stray growth, and the make-room moves and Group growth; a refused expand is decided before any write and records nothing. Moves (`move_node`, `move_nodes_by_delta`, and the align/distribute/straighten layout path) carry a collapsed Group's held members inside the same `move-node` entry.
...
- Undo/redo shell aftermath routes through `MutationUiEffects.after_history_replayed(...)`, which refreshes the scene and tabs and uses `after_graph_change` for the same viewer presentation and execution comparison as committed edits. Undo supplies reversed snapshots. `execution/graph_changes.py` compares compiled computation, independently of history labels; graph-scene code does not compute a downstream closure. Presentation changes stay saved and undoable without expiring results.
