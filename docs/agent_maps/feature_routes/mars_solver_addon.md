...
## Contracts
- COREX owns descriptors, installation, scratch paths, subprocess lifecycle, and artifact registration.
- MARS owns the `mars-modal-response-solver` wheel, `MARSBatch`, schema-v1 jobs, JSONL events/results, and numerical execution.
- Desktop and worker processes do not import MARS; nodes launch the absolute console script beside the selected COREX Python.
...
- All MARS descriptors use the add-on-owned `icons/mars_icon_64.png`, extracted from the 64 px frame embedded in MARS's official `mars_icon.ico`. The raster frame avoids Qt SVG text/clip limitations while matching the executable icon exactly.
...
```powershell
.\venv\Scripts\python.exe -m pytest tests/test_mars_nodes.py tests/test_managed_runtime.py tests/test_addon_manager_install.py tests/test_packaging_configuration.py --ignore=venv -q
```
