...
## Routing Notes
- `GraphInlinePropertiesLayer.qml` synchronously loads only each row's effective editor; ordinary authored/runtime presentation updates retain that editor and its draft. Editor-kind or searchable-enum changes replace the control. Lightweight component wrappers preserve intrinsic control geometry and the existing text-fit/interactive-rectangle contract. Settings toggles keep the shared animation timing; `selection_and_scope_ops.py` builds before/after band-height payloads only for custom-height compensation.
- `GraphNodePortsLayer.qml` reconciles visited port identities in place and publishes only changed row payloads, retaining controls across settings toggles without constructing unopened groups. Hidden geometry-only changes are ignored until reveal; keyed models contain identities only, avoiding nested descriptor conversion through QML model roles. `GraphNodePortRow.qml` separates retained editor state from current grip geometry, keeps full editor layout beneath animation clips, and disables hidden rows. Raw topology removal and node replacement release retained rows; the host supplies one coherent armed/running presentation so collapse grips never briefly hide and reappear.
- Port rich-help text is shared by the grip/label and formatted only on actual hover or keyboard focus; accessibility descriptions stay eager. `GraphSurfaceSearchableComboBox.qml` closes its detached popup when hidden or disabled, and rejects hidden/disabled commits, so retaining a row cannot leave its options popup active.
...
- `data.panel` renders through `GraphPanelSurface.qml` and opens `GraphPanelEditorPopover.qml` through the shared surface-editor overlay owner after `GraphNodeHost.dispatchSurfaceAction("panel_edit")`; setting only the surface-local editor flag does not open the canvas overlay. Display mode publishes no embedded interactive rectangle, preserving node select/drag gestures. The compact editor keeps Text/Data, Data interpretation (Text exact / Automatic / Number strict), multiline value, and Auto-resize changes in a draft; Cancel or Escape discards them and OK or Enter commits the complete property map once. Shift+Enter inserts a line break. The Data surface reserves a header strip for Text/Auto/Number badges and shows Input for connected passthrough; interpretation is disabled while connected. `tests/test_panel_surface.py` covers actual dropdown keyboard activation, Cancel, atomic undo, compact badge geometry, and readable light/dark QML rendering. Runtime and migration details are mapped under [Passive surfaces](../subsystems/passive_media_tabular_surfaces.md).
