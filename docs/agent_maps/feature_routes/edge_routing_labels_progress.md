...
- `EdgePaintPolicy.js` is the pure renderer-neutral owner for flow/standard stroke, dash, structure, display-mode, marker, drag-preview, and gradient-stop facts, including `flowEdgePaintState` (stroke, width in screen and scene units, dash, both end markers and their line trims) and the hidden-wire arc and disabled-X sizes/colours. Canvas and retained renderers call it directly and publish its state as paint diagnostics. `EdgeMath.js` owns shared edge anchors and `edgeBodyPieces`: the stroked body with end-marker trims and crossing/label gaps cut out, as connected sub-paths that both renderers trace. Retained rendering must not reference Canvas implementation methods.
- Default flow-edge label backing stays transparent; `EdgeCanvasLayer.applyCrossingMetadata` adds a label-sized range to each snapshot's `crossingBreaks` (with any `gap_break` crossings), and both renderers cut it through `edgeBodyPieces`, so the edge is visually discontinuous under the label. Explicit `label_background_color` styles still draw their chosen backing color.
...
- Selection keeps retained wire delegates alive and paints only highlighted wires through the Canvas overlay, preserving shared selection stacking. Drawing-order changes update a separate model role instead of replacing stroke data. Both renderers use pen-width dash units from `EdgePaintPolicy.js`; each body piece is one connected path, so dash phase survives corners and restarts after a gap. Parallel strokes (stroke count, empty wires) sit at signed offsets on both sides of the path in both renderers. `tests/test_edge_snapshot_spatial_index.py` verifies rendered dash placement, unrelated pixels, and delegate/update counts across selection changes.
...
- Live bezier handles: Python sizes standard forward-bezier handles for the payload chord (`route_pipe.py::edge_control_points`). While sockets are displaced from the payload by a `liveNodeGeometry` overlay (settings-group animation, resize preview) or a drag offset, `EdgeLayer._edgeGeometry` adds `EdgeMath.forwardBezierLeadDelta` so the handles fit the live chord. That keeps the wire from reshaping in place at animation start or on drop. The correction is exactly 0 at rest, so settled geometry is unchanged. Flow and backdrop-hidden (side-aware) edges and pipes stay translate-only. Keep `EdgeMath.EDGE_FORWARD_LEAD_MIN` equal to the Python constant; `tests/test_flow_edge_labels.py` guards it. Retained delegates carry no drag deltas of their own: each drag flush rebuilds the incident snapshots and their retained entries before paint. `tests/graph_surface/passive_host_interaction_suite.py` owns the per-frame settings-animation and drag/drop handle checks.
...
## Retained Renderer And Per-Edge Routing

- The renderer is chosen per edge, not per visible set. `EdgeRetainedLayer.canvasOverlayReason(snapshot)` returns `""` (retained delegate only), `selection_overlay` (selected or previewed edges, and active-data wires with a selected endpoint node: the delegate stays alive but hidden while the Canvas overlay paints the highlight), or a Canvas-only reason: `invalid_type_gradient` (ShapePath has no stroke gradients) or `unsupported_route`. `EdgeLayer.edgeRendererCanvasEdgeReasonById` lists the Canvas-only edges, and `activeEdgePaintDiagnosticsByEdgeId` still covers every visible edge. The Canvas overlay also draws wire-drag previews, so a drag no longer moves every edge onto the Canvas. `edgeRendererFallbackReason` now reports only a whole-layer fallback for an unavailable renderer (the native-scenegraph placeholder).
- Each retained edge is one `Shape` with fixed ShapePath slots: the body (every piece and parallel stroke in one `PathSvg`), start and end markers (flow arrowheads, or a hidden wire's endpoint arcs), and the disabled X. `preferredRendererType: Shape.CurveRenderer` antialiases on GPU backends; the default GeometryRenderer is aliased on Windows/D3D11 without MSAA, while the software backend draws both renderers through QPainter.
- Pans do not rebuild delegates. Entries are built in screen pixels of a sticky paint transform, kept while the zoom holds and the pan stays within `paintOriginMaxDriftPx`, and the always-on `viewportTransformCompensation*` maps it onto the live viewport; a zoom change rebases it. A per-edge entry cache (edge payload and geometry identity, output previews, theme revision, paint transform, selection/hidden state, crossing breaks) lets unchanged edges skip entry building during pans and drags, and an edge scrolling out hands its row and live delegate to one scrolling in. Counters: `profileRetainedEntryCacheHitCount`, `profileRetainedEntryBuildCount`, and the model entry update/skip counts.
- Pixel parity: `tests/test_edge_snapshot_spatial_index.py` renders the same scene through both renderers offscreen and compares pixels (flow styles and arrowheads, label and crossing gaps, hidden arcs, disabled X, parallel strokes, and a panned view). Dashed unbroken curves can drift by a sub-pixel dash phase at non-unit zoom, because QPainter flattens the Canvas' scaled scene-unit curves differently; straight and broken pieces match.
- Performance (2026-09-26, Windows/D3D11, QQuickWidget, `scripts/generate_flowchart_perf_fixture.py` flowchart: 576 nodes, 920 flow edges; HEAD fell back to the Canvas for it). `performance_harness --project-path ... --interaction-samples 10 --interaction-warmup-samples 3`, medians of per-run p50s over alternated before/after runs: at the default zoom range (12 visible edges, 3 pairs) pan+zoom 82.2 → 71.3 ms, zoom 49.3 → 40.5 ms, selected drag 52.3 → 43.8 ms, and no-readback frame 17.1 → 15.0 ms, with the single-node drag gesture 20.3 → 23.5 ms; at zoom 0.30–0.35 (84 visible edges, 36 labels, 4 pairs) pan+zoom 195 → 191 ms (within the run-to-run noise), selected drag 105 → 76 ms, drag end 60 → 52 ms, and no-readback frame 23.6 → 21.6 ms, with the single-node drag gesture 30 → 36 ms. The trade: the Canvas repaints and rasterizes every visible edge on each changed frame, while retained Shapes cost GPU draw calls on each rendered frame, and the harness renders each step twice (frame plus `widget.grab()` readback), so steps that change only a few edges come out slightly slower there. Timed to `afterRendering` without readback after a settle (two alternated rounds, drag start/steady/cancel frames), retained drags are faster at both zooms: 103.5/75.5/99.1 → 80.1/52.3/75.0 ms with 312 visible edges and 26.8/24.7/17.8 → 17.8/15.6/13.6 ms with 32. See the [performance route](performance_harness_graph_stress.md) for the readback caveat.
