...
- Excel-style table paste creates the same `tabular.input` node with a staged `.tsv` source path; keep clipboard-owned parsing and prompting in `clipboard_paste_nodes.py` / `WorkspaceEditController` rather than adding a tabular add-on paste path.
