...
- Body-region placement is not content padding: the tabular surface insets its status/grid/review content inside the panel stroke and rounded corners. The shared table viewport clips scrolling headers/cells/scrollbars to its own bounds. The passive-host regression covers resizing, zoom, wide scrolled columns and migration notices. Composer QML tests also exercise CSV, TSV and semicolon-delimited text through table mapping, saved filtering/ranges, Apply, fresh-loader reopening and full export without changing the source.
