...
- `tests/test_execution_graph_changes.py` covers grouping equivalence, resolver-dependent interfaces, unavailable-node recovery, and property/identity separation. The isolated `tests/test_shell_run_controller.py::ShellRunControllerTests::test_media_toolbar_history_and_bulk_edits_preserve_real_workflow` runs a real table/plot/media workflow and QML toolbar actions, asserting no extra dispatch, invalidation, node starts, or result/timing changes.
