...
- Prepared dispatch reserves the execute-viewer subset of `PreparedExecution.recompute_node_ids` without visible mutation. `RunController` projects run-required only from the ordered one-shot `viewer_invalidation_committed` event after worker preflight acceptance; failed dispatch/preflight performs no viewer reset or invalidation. The shell has no live-viewer preflight/reset/rollback path.
