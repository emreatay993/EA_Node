...
- `RunController` retains Manual/Selected/Trigger/Auto commands, preview, dirty-script Apply, backend/dispatch policy, pause/resume/stop, history invalidation, and the existing event-intake order until T12. Projection callers use `RunProjectionController` directly; the retired ShellWindow projection/status/cache/failure forwarders must not return.
