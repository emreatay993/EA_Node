...
## Start Here
- `ea_node_editor/execution/submission_service.py`
- `ea_node_editor/ui_qml/components/common/PresentationModelKeys.js`
- `tests/test_shell_submission.py`
- `tests/test_presentation_model_keys.py`
...
- Manual, Run Selected, Auto, and Trigger build one `ExecutionRequest` and call `CorexRuntime.submit_execution(...)`. The shell projects Preparing and adopts the run identity only from ordered `submission_admitted`; Pause requires an executing run. Manual targets all active nodes, Selected uses explicit targets, Auto uses the exact invalidation result, and Trigger preserves clicked/capture fields. Synchronous headless preparation/dispatch remains execution-owned.
...
- Shared connected-input semantics skip invalidation and Auto work for unused dynamic slots. `tests/test_shell_submission.py` covers changed empty-slot layouts across queued/preparing/active work; `tests/test_shell_run_controller.py::ShellRunControllerTests::test_unused_viewer_inputs_preserve_real_cad_workflow` proves actual CAD imports stay current while populated connection edits run only the viewer and its consumer.
- `ShellRunState.active_execution_node_ids` is UI-only remaining-work metadata: request targets during preparation, admitted recompute nodes during execution, and removal on settlement/cancel/terminal. Together with pending Auto and running nodes, it scopes the Media Panel Updating indicator to its own source route; unrelated work must not animate it. The canvas also observes `run_controls_changed`, including queued Stop, so idle stale previews become Out of date without a geometry refresh.
...
- Idle Auto work starts on the next event-loop turn and reuses the graph edit's captured snapshot. A new computational edit supersedes preparing Auto work and unions its targets with the new exact invalidation targets; explicit stale actions cancel without replay. An admitted run finishes against its captured snapshot, and successful completion schedules pending Auto. Stop, Manual, project replacement, navigation and close cancel the relevant queued/preparing work. Failed/protocol/infrastructure outcomes clear pending Auto.
- Property presentations bind values separately from QML repeater identities, retaining controls for value/label/option updates and rebuilding on structural changes. The shared helper covers inline properties, settings groups/items, and input/output ports, and remains usable by standalone Qt Quick tests.
- `execution/graph_changes.py` compares complete directional snapshots using `NodeRegistry.execution_properties`, compiled interfaces, enabled incoming connections, and hidden dependencies. Presentation-equivalent edits skip execution preparation; changed roots and removed executable IDs feed the existing runtime closure. `runtime_history.py` retains snapshots and labels only. `MutationUiEffects.after_graph_change` applies viewer presentation and calls `RunController.invalidate_solution_for_graph_change` for commits and undo/redo.
- `tests/test_execution_graph_changes.py` covers grouping equivalence, resolver-dependent interfaces, unavailable-node recovery, and property/identity separation. The isolated `tests/test_shell_run_controller.py::ShellRunControllerTests::test_media_toolbar_history_and_bulk_edits_preserve_real_workflow` runs a real table/plot/media workflow with a separate title Panel. Editor, drag, Fit Width, wire, undo/redo and queued-Auto actions assert exact node counts and retained facts/timings. Width/Font-size updates keep the prior frame and correctly positioned icon during queued/preparing work, then display the newest plot. Runtime facts remain stale during recomputation; Manual/Stop, unrelated execution and presentation-only edits retain their distinct semantics.
