...
- Clipboard/media payloads from `WorkspaceEditController` and `GraphCanvasPresenter` call `ProjectSessionController.stage_node_artifact_bytes(...)` directly. The composition-injected Jupyter bridge calls `create_blank_notebook_artifact(...)` directly. Each resulting node stores a `temp://` ref; Save/Save As rewrites only candidate-document refs during copy-on-write publication.
