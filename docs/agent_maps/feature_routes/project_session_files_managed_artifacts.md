...
- `ProjectFilesService.replace_project_artifact_store(...)` owns the in-memory artifact-store metadata merge and returns the one `ProjectData.replace_metadata(...)` result. Cross-owner callers use `ProjectSessionController.replace_project_artifact_store(...)`, which emits `project_meta_changed` exactly once only when that result is true; there is no private setter alias.
