...
- `tests/test_viewer_preview_state_cache.py`
- `tests/test_viewer_preview_warmup.py`
...
## Routing Notes
- Runtime composition binds the initial registry before a fresh unsaved workspace can emit viewer settlements. The bridge normalizes typed/serialized settled port results through the active catalog before enforcing its existing singleton handle and identity guards.
- Stable-ID scene labels synchronize through existing presentation updates, including history, late events and retained refocus. They do not replace source leases or advance geometry revision. The binder's metadata-only path preserves selected/isolated actors; preview and camera identity ignore engineering names only.
...
- A viewer that has never been activated gets one warm-up preview: `ViewerPreviewWarmupQueue` paces the renders at one node per event-loop turn and the engineering binder's `render_preview_image` builds the scene into a throwaway offscreen `pyvista.Plotter`. It never binds or reparents a widget, and it is dropped if the node goes live first.
- `ViewerPreviewStateCache` owns the cached proxy frame and the camera that outlive a live session; `ViewerHostService` delegates to it. Every live-to-proxy transition refreshes the frame - inline exit, fullscreen close and detached close all reach `capture_live_state` before the binding is parked in the retained-inline slot - deduplicated by a live-frame dirty mark taken when a live presentation episode begins. Frames keep their capture aspect and the proxy surface crops them to the mapped `graphNodeViewerViewport` rect, which is exact because VTK fixes a camera's vertical extent and widens only with the window aspect.
...
- Model Viewer mouse handling is binder-owned, and a binder takeover aborts the press instead of disabling the VTK trackball style: a disabled style still rotates, and once a press reaches it the style grabs mouse focus, which hides move/release events from non-passive observers at any priority until release. `EngineeringViewerWidgetBinder._install_orientation_triad_drag` (priority 1.0) aborts presses inside the triad viewport and turns the drag into camera Azimuth/Elevation. `_install_camera_orbit` (priority 0.9) takes over plain left-drags (not Shift/Ctrl, triad, or view cube), picks the pivot from one depth-buffer pixel under the cursor (visible-prop bounds center over background), and aborts. Both re-invoke `StartInteractionEvent`/`EndInteractionEvent` so interaction LOD keeps VTK-style timing. The rigid pivot rotation, VTK-parity angles, depth pick, and screen-space pivot dot live in `engineering_viewer_orbit.py`; install/detach follows the selection-picking lifecycle (populate, attach refresh, reparent, release).
- Model Viewer click selection (`_install_selection_picking`) uses VTK passive observers (`GetCommand(tag).SetPassiveObserver(1)`). They run before focus holders and aborted observers, so Ctrl+click toggle (and a stationary Shift+click, which selects like a plain click) completes while the style spins or pans; the move threshold and interaction events keep drags from selecting. Pick sources draw at opacity 0 unless an edge/vertex cue shows them, and VTK pickers skip zero-opacity actors, so `_pick_selection` raises them to opaque only around the non-rendering `Pick` call. `_FakeInteractor` in `tests/test_engineering_viewer_widget_binder.py` models passive dispatch, the style focus grab (`attach_trackball_style`), and the zero-opacity pick rule, but binder input changes still need a live probe: a shown `QtInteractor` (`QT_QPA_PLATFORM=windows`) driven by real `QMouseEvent`s through `QApplication.sendEvent`.

## Focused Verification
- Live/preview parity uses the shared QML viewport frame and mode-independent badge width. The native host injects a pre-navigation publication guard into `ViewportBridge`; deferred relative requests preserve cursor anchors and are released only after the old native rectangle is hidden. `tests/test_viewport_navigation_handoff.py` covers accumulation and cancellation, with host/surface tests proving the integrated ordering.
- The optional `ViewerWidgetViewportPreparer` contract carries explicit presentation mode and hidden viewport preparation. Engineering inline camera controls use 16% size and 2% corner padding in physical viewport pixels; fullscreen/detached retain their existing cap. The manager prepares current Qt geometry, the binder synchronizes VTK dimensions/DPI and renders, and only matching prepared geometry may be revealed. Hidden QWidget geometry is delivered to the existing native Qt windows before rendering. When an existing viewport rectangle changes, a separate manager handoff waits for the destination canvas to paint (including after a minimized/slow host resumes); initial attachment keeps its existing recovery path. VTK buffer dimensions use Qt device-pixel rounding without resizing Qt-owned native windows; the reveal render rebinds the existing GL context, with session/geometry/suppression checks before and after callbacks. This applies to the same-widget retained fast path as well as normal attachment.

```powershell
.\venv\Scripts\python.exe -m pytest tests/test_native_presentation_handoff.py tests/test_viewer_session_bridge.py tests/test_execution_viewer_service.py tests/test_viewer_control_bridge.py tests/test_viewer_host_service.py tests/test_viewer_preview_cache_provider.py tests/test_viewer_preview_state_cache.py tests/test_viewer_preview_warmup.py tests/test_embedded_viewer_overlay_manager.py tests/test_viewer_surface_contract.py tests/test_viewer_surface_host.py tests/test_execution_viewer_protocol.py --ignore=venv -q
...
## Update Triggers
Update when viewer bridge, worker-side session-handle lease ownership, host service, preview/camera state cache, engineering viewer binder, viewer preview cache provider, overlay manager, fullscreen bridge, Python Script fullscreen guide, fullscreen tabular persistence, viewer protocol, or Help reference fullscreen coverage changes.
