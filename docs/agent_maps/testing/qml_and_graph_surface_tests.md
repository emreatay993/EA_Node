...
- Use `tests/test_data_tree_ui.py`, `tests/test_graph_surface_input_controls.py`, and graph-track scene suites for read-only Item/List/Tree access, modifier/Principal checks, normal replacement versus Shift append, edge Enable/Ctrl+E, Stream Gate dynamic outputs, one/two/three-stroke wires, and bounded preview/status projection. Assert the complete absence of control rows, compact control endpoints, and execution-wire animation.
...
- Keep Ctrl-drag coverage on the existing edge/socket interaction path: sole or sole-selected edge choice, replacement versus Ctrl+Shift append, blank disconnect without Quick Insert, metadata/order preservation, invalid/cancel no-op, and one-step undo.
...
durable node link hover/inline-link coverage, data-access/default/modifier/Principal/edge-enabled presentation,
standard-node port notch/shadow coverage, graph-track suites, passive host tests, viewer control/host/surface tests,
default-grip editors, Ctrl-drag reassignment, flat node chrome, detached-window tests, or graph input gates change.
