...
- `tests/test_graph_node_chrome_geometry.py`
...
- `tests/qml_quick/tst_edge_paint_policy.qml` owns direct pure-JS structure/display/marker/drag-preview, `EdgeMath.edgeAnchor(...)`, and `EdgeMath.forwardBezierLead*` live-handle checks. `tests/test_flow_edge_labels.py` retains real Canvas/retained parity, hit geometry, label/crossing, and renderer-fallback integration.
...
- T08 type UI proof is split deliberately. `tests/test_data_type_ui_projection.py` owns one bounded catalog projection, current-effective-port endpoint snapshots, availability/type-warning separation, and rich-preview redaction/callback/I/O bounds. `GraphSurfaceCanvasInteractionTests.test_ctrl_drag_reassigns_selected_or_sole_edge_endpoint_without_quick_insert` proves one snapshot across repeated moves and release plus malformed/generation fail-closed behavior. `PortTypePresentationQmlTests.test_ports_use_projected_input_state_and_keep_edge_reason_out_of_port_text` and `test_family_accent_binding_uses_graph_theme_port_kind_palette`, together with `ShellThemeServiceTests.test_graph_theme_bridge_resolves_family_tokens_to_the_active_data_role`, own the existing-ring/tooltips/accessibility/non-color cue and graph-palette fallback.
- Keep direct dynamic-port and sparse-state mutation coverage in `tests/test_graph_node_reconciliation.py`; persistence and format-migration coverage stays in `tests/test_dataflow_graph_persistence.py`, with real UI/history coverage in `tests/test_data_tree_ui.py` and serializer/graph-track suites.
...
- Canvas tests that need frame-scheduler work (queued live-drag offsets, edge-snapshot rebuilds after a selection or move) call `frameSchedulerRef.flushPendingRedraws()` instead of waiting on its 16 ms QML Timer, as `tests/test_group_backdrop_interactions.py` (`_flush_frame_scheduler`, `_wait_for_flushed`) and `tests/graph_surface/passive_host_interaction_suite.py` do. While any QML animation runs, QML Timers follow the render loop's animation clock, which on a native window advances only when a frame is presented, so with the display off the Timer never fires. Plain-`unittest` modules default `QT_QPA_PLATFORM` to offscreen the way `tests/conftest.py` does for pytest.
...
- `tests/qml_quick/tst_graph_node_port_context_menu.qml` owns the direct port Menu root/order/check state, open/close, modifier/Principal/dynamic layer calls, data visibility, and read-only guards. `tests/test_graph_surface_input_controls.py` and `tests/test_data_tree_ui.py` retain real command/history and Principal checks; `tests/test_graph_node_reconciliation.py` owns dynamic mutation/sparse-state cleanup and `tests/test_dataflow_graph_persistence.py` owns persistence behavior.
...
- Keep notched-port render coverage in the graph-surface suites: default-on/explicit-off state, ordinary and aggregate vector grips, exact anchors, true cutout transparency over contrasting content, and intact complete cards during settings animation and fractional resize. Test inward borders, gradients, hatch clipping, selected/warning/error states, horizontal-only halo suppression, shadow geometry, and collapsed/chrome-free/flowchart exclusions. Geometry retention checks must compare object identity and change counts, not only equal SVG strings. `tests/test_graph_node_chrome_geometry.py` executes the actual JS contour builder and checks intersections, overlapping/tangent cutouts, pills, degeneracy, and independent point-membership expectations, including straight-edge socket stacks on the edge-walk route and corner/cross-card layouts on the general pass. Preference projection remains in `tests/graph_track_b/qml_preference_bindings.py`; `tests/test_track_h_perf_harness.py` owns benchmark parity.
...
.\venv\Scripts\python.exe -m pytest tests/test_graph_surface_input_controls.py tests/test_passive_graph_surface_host.py tests/test_shell_theme.py -k "ctrl_drag_reassigns_selected_or_sole_edge_endpoint_without_quick_insert or ports_use_projected_input_state_and_keep_edge_reason_out_of_port_text or family_accent_binding_uses_graph_theme_port_kind_palette or graph_theme_bridge_resolves_family_tokens_to_the_active_data_role" --ignore=venv -q
.\venv\Scripts\python.exe -m pytest tests/test_annotation_catalog.py tests/test_flowchart_surfaces.py tests/test_group_backdrop_contracts.py --ignore=venv -q -n 0
