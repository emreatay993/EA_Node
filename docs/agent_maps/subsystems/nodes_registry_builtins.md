...
- Preserve the exact 144-row repo-owned catalog in `tests/fixtures/node_catalog/current_repo_owned_catalog.json`. The Model Viewer representation default is `surface_with_edges`, and the earlier migration inventory tracks 78 conversions, eight native functions, and 53 internal exceptions.
