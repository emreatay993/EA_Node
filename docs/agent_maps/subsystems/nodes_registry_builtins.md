...
- `builtins/core.py` registers Python Script as `code.python_script` in the Library's `Code` category. Jupyter Notebook shares the `Code` category; persistence migrates the former `core.python_script` ID when opening a project.
...
- `nodes/solution_provenance.py` is the registry-owned typed overlay for the seven accepted shipped session readers, including Mesh Import. It adds exact `path` file provenance only for the trusted built-in/tabular owners; declarations, public packages, and trusted factories cannot supply or imitate this metadata.
...
## Boundaries
- `DynamicPortGroupSpec.execution_policy` defaults to `all_ports`; `connected_inputs` admits only optional, list-backed active input groups without property defaults or conflicting readiness/provenance/sensitive dependencies. Registration and instance resolution both enforce it. Model Viewer opts in; Python Script source-backed groups and Stream Gate outputs remain computational. The backing property remains computational, and `tests/test_dynamic_input_execution.py` proves the shared custom-node contract. This is trusted metadata, not a public `corex` export.
...
- Preserve the exact 145-row repo-owned catalog in `tests/fixtures/node_catalog/current_repo_owned_catalog.json`. The Model Viewer representation default is `surface_with_edges`, and the earlier migration inventory tracks 79 conversions, eight native functions, and 50 internal exceptions (the swimlane pool and lane joined the trusted passive annotations; `PASSIVE_ANNOTATION_BACKDROP_TYPE_IDS` in `ea_node_editor/nodes/builtins/passive_annotation.py` lists every Group-backdrop type, and `LIBRARY_HIDDEN_NODE_TYPE_IDS` there lists the types the node Library does not offer: the swimlane pool, which forms from lanes).
