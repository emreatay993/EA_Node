...
- `ea_node_editor/graph/validated_mutation.py` for registry-backed node, edge, endpoint reassignment, dynamic-port insert/remove/rename, property, parent, exposed-port, view-filter, and PDF page-policy mutations.
...
- Endpoint reassignment is graph-owned and atomic in `ValidatedGraphMutation.move_edge_endpoint`: move or disconnect one existing edge while preserving its identity, state, metadata, and unaffected ordering. `GraphSceneMutationHistory.move_edge_endpoint` owns the one-step history boundary; replacement/append does not compose public remove-plus-add calls.
...
- `NodeRegistry.data_types.compatibility(...)` is the single semantic data-port authority. Data edges accept `assignable`, `convertible`, and `runtime_check`; they reject `incompatible` and `unresolved`. The graph never performs conversion. Compatibility is target-directional: a target input accepts its primary `data_type` plus every ordered `accepted_data_types` alternative, while `flow` ports bypass semantic typing after their structural kind check. Derived subnode/grouping ports must preserve that union and exact type-ID casing.
- Registry-backed add, endpoint reassignment, edge re-enable, dynamic-output edits, semantic property changes, and fragment insertion preflight resolved catalog IDs before writing. Fragment preflight covers every resolved data port, including edge-free dynamic ports; flow ports are not catalog-validated as data. Bulk edge-enable preflights the whole requested batch against a temporary edge view before changing records or history. Failed mutations leave graph state, fan-in order, revision, and history unchanged. Semantic port changes prune only newly invalid incident edges; registry normalization and registry-backed compilation prune incompatible or unresolved edges.
