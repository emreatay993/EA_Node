...
- `ea_node_editor/graph/hierarchy.py`, including `sanitize_workspace_held_member_ids` for stored Group member lists on load and paste.
- `ea_node_editor/graph/transforms.py`
- `ea_node_editor/graph/transform_layout_ops.py`, including `build_make_room_position_updates` (the "Make room" expand push).
- `ea_node_editor/graph/transform_tidy_layout.py` for the pure Tidy layout (layered auto-layout from the wires, in-place clean-up, group refit) over plain `TidyItem` / `TidyWire` data, where a collapsed Group is one rigid item at its pill size; `tests/test_transform_tidy_layout.py` owns it.
- `ea_node_editor/graph/transform_fragment_ops.py` (a selected swimlane pool takes its lanes into a fragment or a deletion)
- `ea_node_editor/graph/swimlane_layout.py` for the pure swimlane rule (stack lanes, keep what they hold inside them, edge-aware lane and pool resizes, lane removal, a pool's lanes from stored frames); `tests/test_swimlane_layout.py` owns it. See [Swimlane Pools And Lanes](../feature_routes/swimlane_pools_lanes.md).
- `ea_node_editor/graph/transform_grouping_ops.py`
- `ea_node_editor/graph/group_backdrop_geometry.py` for pure Group membership: area ownership for expanded Groups, identity ownership for Groups with an honoured `held_member_ids` list, collapsed-body top-left corner containment (`membership_rect_kind`, `CORNER_CANDIDATE_SIZE`), the freeze/clear list rules, the held-member carry, and wrap bounds; `tests/test_group_backdrop_membership.py` owns it.
...
- `ValidatedGraphMutation.reverse_edges` swaps each edge's source and target in place (`_move_edge_endpoint_record`), so ids, labels, styles, and enabled state survive. It validates every swapped pair as a fresh connection against the workspace minus the requested edges first; a directed or hidden port, a same-node flow loop, or a duplicate rejects the whole batch, and the new target port gets the next input order. The scene `reverse_edges` op owns the one-step history and label-position mirroring (`tests/test_flow_edge_labels.py::FlowEdgeReverseTests`).
...
- `ValidatedGraphMutation.apply_python_script(...)` owns one candidate-first Apply action for `code.python_script`. Resolve the source before writing, then reconcile ordinary settings, resolved ports, per-port state, wires, and `expanded_settings_group_ids` together. A failed parse or validation leaves the existing applied node untouched; one successful Apply is one undo/redo entry.
...
- `NodeInstance.held_member_ids` is authored state on Groups (`passive.annotation.group_backdrop`) while they are collapsed or hidden inside a collapsed Group: the contained subtree frozen at collapse (nodes and Groups, all depths), and `None` elsewhere, including on a collapsed Group from an older file or fragment until the scene fills it in. Node mappings emit it only when not `None` and read it leniently even for strict payloads; project load, registry normalization, and fragment insertion run `hierarchy.sanitize_workspace_held_member_ids` (existing same-parent ids only, Group-to-Group list cycles broken, lists only on collapsed or hidden Groups); node removal prunes the removed id from every list. Execution ignores it (`tests/test_execution_graph_changes.py`).
...
- `transform_tidy_layout.py` stays UI-free: callers pass port sides and anchor offsets as plain data. It lays out one level at a time (groups deepest first, then the top level) in a left-to-right frame; top-to-bottom transposes each level in and out, so keep the two directions one code path. Rows are assigned per column by desired row first (then drawn position), and the cell directly below a node is reserved for its first same-column branch child, so a decision's second branch never has another node between it and the decision. Straightening reuses `transform_layout_ops.build_port_alignment_offsets` (also behind Straighten Connections). Group blocks move as units, so a wire into a nested member is straightened with that member's real port offset. The level is then shifted back to its anchored top-left, so a repeated Tidy is a no-op, and the straightening is discarded when it would create an overlap. A swimlane pool item is resolved by `_resolve_swimlane_pool` instead of the group refit (its lanes are skipped as groups): layer columns across all its lanes along the pool's own flow, one row per lane, lanes restacked. In-place clean-up groups rows and columns by overlap with their first item (at least half the smaller size) rather than by centre distance, so mixed-height shapes stay in one row. It measures gaps between cells (median centre ± half the largest size), so straightening cannot make reruns creep. Results are deterministic under shuffled input.
- `transform_layout_ops.build_collision_avoidance_position_updates` is the several-blocker collision solver (Tidy's neighbour push and obstacle clearing, and in the scene's expand cascade the Nearest pushes, the Make room follow-up passes, and moving intruders out of a growing Group); When the nearest-blocker steps bounce between blockers, `_separate_from_bounds` falls back to the smallest single-axis move that clears all of them.
- `transform_layout_ops.build_make_room_position_updates` is the pure "Make room" push: boxes in the grown box's row band after it shift right, and boxes in its column band below it shift down, each band by one amount so rows stay straight; the first box keeps its original gap capped at `max(gap, keep_gap_x)` / `max(gap, keep_gap_y)`, no box moves further than the growth (no drift on repeated expand/collapse), and growth to the left or up mirrors it. Callers pass the Tidy gaps as `keep_gap_x` / `keep_gap_y` because importing them here would create an import cycle. `tests/test_transform_layout_ops.py` owns it.
