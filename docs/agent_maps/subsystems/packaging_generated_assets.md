...
- Video Panel trim-save requires `imageio-ffmpeg` and its bundled ffmpeg executable in every package profile; keep `pyproject.toml`, `ea_node_editor.spec`, `scripts/build_windows_package.ps1`, and `tests/test_packaging_configuration.py` aligned.
