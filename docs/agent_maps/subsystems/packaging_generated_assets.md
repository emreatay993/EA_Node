...
## Common Changes
- `ea_node_editor/common/build_info.py` stamps the full Git commit, reachable commit count, dirty state, UTC build time, profile, and package version into WORKPATH/build_info.json on every `ea_node_editor.spec` execution. PyInstaller includes it at ea_node_editor/build_info.json (normally under the distribution's _internal/ directory). Missing Git provenance fails packaging; source launches can display `commit unknown` without Git. Shallow clones omit the incomplete count.
- The splash code `CX-<count>-<token>` uses the first 12 uppercase SHA-256 hex characters of `COREX:<full commit>`; this obscures the Git hash but is not access control. Resolve a report with `.\venv\Scripts\python.exe scripts\resolve_build_id.py CX-1234-7A92D6E1B830`. The command searches local refs and reflogs and reports ambiguity. The count is informational and is not a global build counter; repeated builds from the same commit share a code. A `dirty` suffix means the commit alone cannot reproduce the build. Distribute clean commits when exact source reproduction is required. `tests/test_build_info.py` covers source identity, dirty/shallow checkouts, resolution, spec collection, and frozen operation without Git.
...
- Windows packages include a schema-v2 `runtime\runtime_manifest.json` plus a contained COREX wheel and, by default, a MARS wheel; `build_windows_package.ps1 -ExcludeMars` produces a valid COREX-only bundle. Installer validation requires COREX and validates every package declared by the manifest. The app-managed venv installs COREX first and MARS with `--no-deps` when MARS is present. Source manifests point to editable local repositories, but add-on setup installs only MARS into the active COREX interpreter. Keep runtime preparation, package, and installer validation in sync.
