...
- Production composition supplies `ShellWorkspacePresenter` as the explicit persisted-graphics source for both graph-canvas bridges and `GraphSceneBridge`; it supplies `GraphCanvasPresenter` only for minimap-expanded, selected-run preview, snap enabled, snap size, and remaining canvas operations; it supplies the concrete `MediaPanelActionService` only to the command bridge's Media Image/Video mixins. Persisted graphics must not fall back through either source.
