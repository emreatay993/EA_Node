...
- `GraphSearchOverlay.qml` hands keyboard focus back to `graphCanvasRef` when it hides while owning focus (jump, result click, or Esc). Hidden QML items keep active focus, and a focused text field accepts `ShortcutOverride`, which silently swallows single-key window shortcuts such as F, Shift+F, and A. `tests/main_window_shell/shell_basics_and_search.py` owns the regression.
...
- The Properties pane contains node definition, links, comments, and property editors; the obsolete Port Management section and its QML editing state are removed. Port actions remain with the graph/canvas and subnode-pin workflows.
