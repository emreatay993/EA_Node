...
- `ea_node_editor/addons/mechanical/cdb_export.py`
- `ea_node_editor/addons/mechanical/cdb_save.py`
...
- Mechanical's eight declarations expose 55 typed inputs and 22 outputs (77 ports) through the shared named-settings-group controls. Each declaration expands its primary group on fresh creation and leaves advanced groups collapsed. Save's `cdb_options` group starts collapsed and owns `cdb_content`, scalar `cdb_analysis`, and Full-only `cdb_load_step`; CDB inputs are inactive/unconsumed for other formats and archive inputs are inactive/unconsumed for CDB. `tests/mechanical_catalogue/test_controls.py` owns the exact section/order/default/condition inventory; `test_visuals.py` owns production GraphCanvas interactions and font/theme/scale captures without launching Ansys. CDB-specific GUI qualification is separate from these metadata contracts.
...
- Save CDB uses `cdb_export.py` for bounded native command preparation and CDB proof, and `cdb_save.py` for the existing source snapshot/save routes, a fresh CDB owner, and final Report helpers. `backend.py` allowlists `cdb_source_preflight` and `export_cdb_snapshot`; source preflight selects the original source analysis before conversion. `runtime.py`/`saving.py` validate the receipt and publish one CDB; the returned Model stays on the original source owner at a fresh revision. Only 2026 R1 Static Structural with a current mesh is supported, without solving/remeshing. Mesh retains physical elements, solver-exported named selections, and needed type/real/section definitions; Full captures one supported load step without results/history. `owner_process.py`/`session.py` distinguish typed expected operation rejection from protocol failure and preserve sticky working-copy retention with recovery paths. `tests/mechanical_catalogue/test_cdb_export.py` and `tests/mechanical_catalogue/test_cdb_save.py` own focused helper/save checks; `docs/MECHANICAL_CATALOGUE.md` records bounded native helper/contact and registered standalone Mesh/Workbench Full step 3 Unicode qualification, with live GUI qualification unclaimed.
...
.\venv\Scripts\python.exe -m pytest tests/mechanical_catalogue/test_cdb_export.py tests/mechanical_catalogue/test_cdb_save.py --ignore=venv -q
