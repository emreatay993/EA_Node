...
- Hot-apply enablement is staged until runtime services and the graph scene accept the rebuilt registry. Publish the registry to shell consumers before persisting the enabled-state document; a failed scene rebuild must preserve the active scene/shell registry, serializers, and stored preference.
