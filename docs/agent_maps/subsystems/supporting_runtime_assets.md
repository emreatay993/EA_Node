...
- `runtime_contracts/data_types.py` owns catalog registration, parent assignability, strongest declared-union compatibility, carrier compatibility, type-owned input normalizers (`DataTypeSpec.normalize_input`, applied through `DataTypeCatalog.normalize_input`), and catalog fingerprints. `DataTypeCompatibility` remains the sole structured relation record; `connection_fallback` is a type capability consumed only by recommendation policy.
- `ea_node_editor/nodes/core_data_types.py` registers bounded exact JSON-domain `COREX.DataTypes.JsonValue` and native `COREX.Plot.ExportBundle`; the latter is an exact four-key dictionary with two artifact refs and two JSON metadata dictionaries, not a JSON-derived type. Any, Json, GraphArray, and GraphDictionary carry the fallback capability. `COREX.DataTypes.Path` declares the only input normalizer: it strips surrounding whitespace and one matched quote pair (Windows "Copy as path") via `common/coercions.py`; path-typed properties use the same helper in `nodes/property_coercion.py`, so nodes never re-implement quote handling.
