...
- Selected-node property edits and `inspector_projection.py` route add-on-specific policy through live add-on property edit adapters. Do not add direct tabular constant imports or tabular branches to `workspace_edit_ops.py` or the generic Inspector owner.
