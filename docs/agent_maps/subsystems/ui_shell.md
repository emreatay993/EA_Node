...
- `ProjectDocumentIOService._install_project()` resets the attached `CorexRuntime` project session before installing the replacement model, so new/open/import cannot retain the previous project's session records, preparations, Trigger reservations, or unsaved namespace.
