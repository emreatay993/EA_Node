...
- `ui/shell/property_edit_adapters.py` explicitly composes the core Plot adapter before live add-on adapters. Selected-node property edits and `inspector_projection.py` use that shell owner; the add-on catalog returns add-on adapters only. Do not add direct tabular constant imports or tabular branches to `workspace_edit_ops.py` or the generic Inspector owner.
