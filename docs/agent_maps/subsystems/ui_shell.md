...
- `composition/bridges.py` constructs and attaches `GraphCanvasStateBridge` and `GraphCanvasCommandBridge` directly with their existing host parent and scene/view/presenter collaborators. It passes `ShellWorkspacePresenter` as each bridge's persisted-graphics source and `GraphCanvasPresenter` as each bridge's canvas/session source, then explicitly binds the workspace presenter to `GraphSceneBridge`. `ViewportBridge` remains primitive-owned; the shell has no aggregate `graph_canvas_bridge` field.
...
- Selected-node property edits and inspector property item projection route add-on-specific policy through live add-on property edit adapters. Do not add direct tabular constant imports or tabular branches to `workspace_edit_ops.py` or `window_library_inspector.py`.
...
- Sensitive Inspector properties project only `{has_value, scope}` state. Replace, Clear, and scope reprotection route through `ShellInspectorBridge` to the graph-scene secret commands; `window_library_inspector.py` must never send the persisted DPAPI envelope to QML.
...
- Persisted graphics changes flow `AppPreferencesController` -> shell host state -> `ShellWorkspacePresenter.graphics_preferences_changed`; that presenter is the only runtime graphics source for the state/command bridges and scene. Do not add a second refresh path through `GraphCanvasPresenter` or `ShellHostPresenter`.
...
- `ProjectSessionController.replace_project_artifact_store(...)` is the public cross-owner store boundary. The lower project-files service merges store metadata through one `ProjectData.replace_metadata(...)`; the controller emits one metadata signal only when changed. Same-owner staging/save services call the lower method directly.
...
- The Ctrl+B Node Browser action belongs in `window_actions.py`; its cached library/help rows and learned-shortcut ranking stay in `LibraryPresenter`/`window_library_inspector.py` and are exposed through `ShellLibraryBridge` rather than rebuilt in QML.
...
- Canvas view export is shell-presenter owned: `window_actions.py` and view-tab bridge calls route to `GraphCanvasPresenter.export_canvas_views(...)`, `CanvasViewExportDialog` gathers options including crop-to-visible-content opt-out, and `ShellHostPresenter.choose_output_folder_dialog(...)` owns folder picking. Crop-to-content capture temporarily frames workspace scene bounds, restores the live view per capture, and post-crops the composited PNG including native overlay extents. Project review deck export is also shell-presenter owned: `ProjectReviewDeckPresenter` coordinates the global-order tree draft dialog, workspace snapshot slides, saved workspace-view slides, project-artifact slide planning, optional `.pptx` template selection, and generic PowerPoint writing. Review-deck workspace views supply saved zoom/pan directly to the canvas capture transaction and must restore saved `WorkspaceData.views`, `active_view_id`, and final live pan/zoom.
...
- `RunController` projects execution-owned `NodeSolutionFact` values and strict `solution_state_changed` revisions; it does not own another freshness set or closure. Store-accepted output observations use `ui/support/solution_output_cache.py`, whose retained-record selector and bounded eviction are shared by shell presenters and QML projections.
...
- `WorkspaceGraphEditController` owns `MutationUiEffects` and injects it into workspace edit/drop-connect operations.
