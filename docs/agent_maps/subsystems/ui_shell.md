...
- A selected swimlane pool or lane gets the Lanes section: `build_selected_node_swimlane_lane_items` rows and the `*_selected_swimlane_lane*` actions (the scene's undoable lane commands) on `ShellInspectorPresenter` / `ShellInspectorBridge`; `on_scene_nodes_changed` (wired in `ShellWindow._wire_signals`) refreshes it when lanes change while it is shown. See [Swimlane Pools And Lanes](../feature_routes/swimlane_pools_lanes.md).
