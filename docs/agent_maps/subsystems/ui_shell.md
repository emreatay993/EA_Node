...
- `composition/controllers.py` constructs the one `CorexRuntime` with `SolutionRepositoryFactory`; execution code sees only its backend port. `ProjectDocumentIOService._install_project()` resets the runtime, then binds decoded `metadata.solution_store` before installing the replacement model. Cache failure is session-only and cannot discard authored graph/metadata. Save/Save As do not switch durable bindings in T07.
