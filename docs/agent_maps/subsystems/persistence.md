...
- `migration.py` rewrites `core.python_script` to `code.python_script` in workspace nodes and embedded custom-workflow fragments during `.cxproj` v4/v5 open. The migration report marks the project dirty; the next save writes only the new ID while preserving script properties and wires.
...
- Persist a Group's stored member list `held_member_ids` (collapsed Groups and Groups hidden inside one) as authored node state through the normal node document path: `ea_node_editor/graph/record_payloads.py` writes it only when it is not `None`, and `project_codec.py` runs `ea_node_editor/graph/hierarchy.py` `sanitize_workspace_held_member_ids` right after `sanitize_workspace_parent_links` on load. It is never one of the stripped runtime membership fields (`GROUP_BACKDROP_MEMBERSHIP_FIELDS` from `ea_node_editor/graph/group_backdrop_geometry.py`, the fields scene payloads project), and `.cxproj` stays v5. Expanded-Group ownership is still recomputed from geometry; a collapsed Group saved without a list gets one derived from geometry when the scene first shows its workspace (`ea_node_editor/ui_qml/graph_scene_mutation/group_backdrop_ops.py`, `fill_missing_held_member_ids`), which is not an edit.
...
## Focused Tests
- `tests/test_serializer.py` (including `test_held_member_ids_round_trip_and_are_sanitized_on_load`)
- `tests/test_serializer_schema_migration.py`
- `tests/test_python_script_persistence.py` for decorator settings, section expansion, and old Python Script ID migration on project open/save.
