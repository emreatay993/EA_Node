...
- Inject viewer camera/preview capture callables at runtime composition with a late-bound host-service reference; the session bridge must not discover `ViewerHostService` through raw shell attributes.
...
- Native inline viewer overlays short-circuit polish/geometry/move/resize work and disable QWidget updates while `GraphCanvas.nativeOverlaySuppressionActive` is true. `ViewerHostService` owns one automatic attached-but-hidden retained-inline slot, separate from explicit fullscreen/detached/keep-live holds; unchanged session/transport reactivation skips the binder, while identity or lifecycle termination still calls its release path.
