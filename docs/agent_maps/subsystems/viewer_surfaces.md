...
- `NativePresentationHandoff` is a plain per-host state machine, instantiated independently by `ViewerHostService`, `PlotHostService`, and `EmbeddedViewerOverlayManager`; the instances never share pending state. It owns preview-source records/serials, expected-source validation, at most one `afterRendering` connection, timeout, cancel/flush/shutdown, and queued completion. Host-specific capture and final viewer-demotion/plot-release callbacks stay in the hosts; the manager uses `wait_for_render` to gate native reveal on the painted destination canvas, and the render callback schedules rather than mutates host/native state.
- `EmbeddedViewerOverlayManager.export_overlay_snapshots()` is the native-overlay export handoff: it returns visible geometry-ready overlays in canvas-relative coordinates with an owner string. Prepared viewers retain the full content rectangle and clip their container at canvas edges, preserving camera aspect and control placement. Export keeps that full destination rectangle; the compositor clips pixels to the canvas. Native reveal calls the binder's `present_viewport` after Qt maps final geometry and before publishing readiness: Engineering releases the current GL binding before hidden preparation and reveal so WGL cannot reuse the old drawable origin/clip. Hidden QWidget geometry is delivered to its existing Qt native windows before rendering; reveal waits for the destination canvas frame, and then renders once at the final native rectangle. This retains the GL context and scene resources. Keep owner ids centralized in `native_overlay_owners.py`; unknown visible native overlays should fail export rather than disappear silently.
- Native inline viewer overlays short-circuit polish/geometry/move/resize work while `GraphCanvas.nativeOverlaySuppressionActive` is true. Before hiding and disabling QWidget updates, the overlay manager asks the registered owner to prepare a fresh preview. `ViewerHostService` captures once per gesture and reuses `NativePresentationHandoff`; its queued GUI completion repaints the preview backing store before hide. Holding pan/zoom/drag keeps the new frame without worker live-mode updates, and release restores the same widget. Inline live mode begins only after a proxy-viewport double-click; selection, hover, and single-click remain proxy. `ViewerHostService` owns one attached-but-hidden retained-inline slot only for a widget previously created by explicit inline activation, while fullscreen/detached use explicit presentation holds. Unchanged session/transport reactivation skips the binder; selection/background loss demotes, and identity or lifecycle termination still calls the release path.
...
- Inline visual exit remains host-owned even when deselection/background focus loss has already projected the session to proxy. Pending exits keep the same applied binding and visible overlay through queued syncs. QML renders the replacement beneath that overlay and acknowledges its actual `Image.Ready` source before the render gate permits parking. Failed capture/load uses an acknowledged unavailable placeholder; the 300 ms recovery timeout clears unconfirmed pixels. Reactivation and identity/lifecycle replacement cancel pending work, and queued render completions validate the captured handoff serial. Label-only edits preserve cached-image identity and binder-applied snapshot semantics.
- Inline `vtkCameraOrientationRepresentation` size and padding are proportional to the physical render window (16% and 2% of its shorter edge), with no fixed-pixel clamps, matching scaled cached images across canvas zoom and DPR. The host passes explicit inline/fullscreen/detached mode through the optional binder preparation contract; fullscreen/detached retain the 44-120 px size bounds and 10 px padding. The separate lower-right axes retain normalized viewport sizing.
- `GraphViewerSurfaceBody.qml` owns one always-shared viewport outline, outside the inner `graphNodeViewerViewport` rectangle used by the native widget, cached image, capture sizing and input geometry. Its reserved corner clearance prevents the native child from covering the rounded outline. The mode badge reserves Preview's width in live mode too, so narrow-node status wrapping cannot change the viewport height.
- `model.viewer` is the sole active-node manual-resize exception. `GraphNodeHost.qml` exposes the existing four hover corner handles for that exact type, and the viewer surface opts into the shared drag-start aspect-ratio lock. Collapsed, locked, and read-only viewers remain non-resizable; other active and `compile_only` nodes remain excluded. Resize previews continue through `liveNodeGeometry`, so native inline content is temporarily suppressed and restored without changing viewer-session lifetime.
- `ViewportBridge` defers publication of navigation behind a composition-injected viewer handoff guard. Relative pan/zoom builds on the latest requested state while QML continues displaying committed geometry; the host releases it only after the native old rectangle is hidden. Navigation holds survive an early gesture release and a genuine reactivation, but workspace/session lifecycle replacement cancels obsolete work. `tests/test_viewport_navigation_handoff.py` owns the pure accumulation/publication checks.
- Native geometry preparation and reveal are separate. The manager applies hidden widget geometry and the binder synchronizes VTK size/DPI plus control metrics before rendering; prepared geometry/DPR and the latest mapped rectangle must agree before reveal. Qt visibility is not a prerequisite for prepared readiness. Retained reactivation uses this path without reparenting or actor/data reloads, and fullscreen remains exempt from canvas suppression.
