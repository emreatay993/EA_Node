...
- Passive node `visual_style` gradients are normalized in `passive_style_normalization.py` and the passive style dialog; flowchart shapes consume the resolved body gradient through `FlowchartShapeCanvas.qml`. Surfaces that paint their own fill (annotation notes, group backdrop, mail panel, Excalidraw board, web page frame) nest `GraphNodeGradientFill.qml` inside that fill, gated on `GraphNodeHost.passiveBodyGradientActive` (the node's own style turns the gradient on); graph-theme card gradients stay host-chrome only. Proof: `tests/graph_surface/passive_host_interaction_suite.py::test_self_filling_passive_surfaces_paint_the_style_body_gradient`.
...
- Panel Data interpretation is authored as `text`, `auto`, or `number`: `GraphPanelEditorPopover.qml` provides the theme-aware selector, explanation, and atomic draft acceptance; `GraphPanelSurface.qml` reserves a header strip for Text/Auto/Number badges or Input when connected. Text preserves item whitespace, Automatic infers finite numbers, and strict Number rejects nonnumeric lines with branch/line context while blank lines become missing values. Runtime behavior is pinned by `tests/test_panel_node.py` and the Panel-to-plot-to-media process case in `tests/test_signal_plot_scientific_integration.py`.
- Tabular input authoring is a canonical `data_view` with a fullscreen draft session, a virtual member catalogue, Mapping/Output rules/Source options and one undoable Apply. Inline/fullscreen cell and column selection stays transient; presentation widths remain separate. Old input hints are converted at document/fragment boundaries into a review notice. See the Tabular feature route for current owners and tests.
- Cold tabular previews and catalogue discovery are asynchronous. The inline presenter returns cached bounded payloads; the fullscreen composer rejects outdated generations and clears old rows when mapping or source changes. ND raw preview planes are explicit and separate from output slices. Don't reintroduce synchronous data reads in UI projection.
...
- `pdf_preview_provider.render_pdf_page_image(...)` is the single PDF page render path: `LocalPdfPreviewImageProvider` serves the inline canvas preview through it, and non-QML consumers such as Project Review Deck export call it directly. Pass the Media Panel's authored `page_number` and let preview metadata clamp the resolved page.
