...
- Standard input/output port rows are directly shared by `GraphNodePortRow.qml`, and `GraphNodePortContextMenu.qml` directly owns the access/modifier/Principal/dynamic root Menu. `GraphNodePortsLayer.qml` retains models, dynamic scheduling/mutations, notch cache, aggregate rectangles, context/edit state, menu placement/opening, add controls, and the minimal direction-only children without adding per-port/menu objects or loaders.
- Action presentation is pure and QML-owned by `GraphActionPresentation.js`; seven files across the original six canvas/menu/toolbar/host presentation surfaces use it for lookup, normalization, field shaping, menu/popover extraction, and stable ordering after the node-toolbar popover extraction. `GraphCanvasActionRouter.qml` remains the sole dispatch owner and Python/QML feature owners retain policy/lifecycle decisions.
