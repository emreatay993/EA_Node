...
- [Swimlane Pools And Lanes](feature_routes/swimlane_pools_lanes.md) - pool/lane node types in the Group backdrop family, the stacked-lane rule and settle pass, lane commands and menu rows, lane-aware Tidy, pool/lane surface and title band, `swimlane.*` automation ops
...
- Swimlane pools and lanes are Group backdrops (`group_backdrop` family, variants `swimlane_pool` / `swimlane_lane`), so lane membership is the Group area rule and a lane drag carries its nodes. The stacked-lane rule is pure (`ea_node_editor/graph/swimlane_layout.py`); `ea_node_editor/ui_qml/graph_scene_mutation/swimlane_ops.py` owns the commands and the capture/settle pass that the move, resize, create, paste, delete, collapse/expand, align, wrap and Tidy paths run in their own undo step. The lane-aware Tidy is `_resolve_swimlane_pool` in `ea_node_editor/graph/transform_tidy_layout.py`. Backdrop hosts now stack by `backdrop_depth` (`GraphNodeHost.qml`), so a nested Group or lane is drawn and clicked above the backdrop around it. The [swimlane route](feature_routes/swimlane_pools_lanes.md) owns the proof: `tests/test_swimlane_layout.py`, `tests/test_swimlane_scene_ops.py`, `tests/test_swimlane_tidy_layout.py`, `tests/test_swimlane_persistence.py`, `tests/test_swimlane_shell.py`; the automation route owns `tests/automation/test_handlers_swimlanes.py`.
