...
- Mechanical's dependency-gated semantic values remain in `addons/mechanical/contracts.py`; `runtime.py` and `inspection.py` own Open, accepted discovery, and eleven-mode data-only Search execution, `tables.py` owns full-fidelity definition/result/probe/worksheet extraction, `graphics.py` owns saved/current cameras plus deterministic viewport image batches and rollback-protected optional PNG publication, and `commands.py` owns scoped scripts plus deterministic marker-owned APDL snippets with verified rollback. Its run/workspace service and dedicated bounded owner process keep native imports out of the shell and shared execution worker. Native working copies use source-family APIs; catalogue, typed Search, table, camera, and image payloads use bounded hashed owner spools over existing runtime codecs.
...
- The exact current catalog is `tests/fixtures/node_catalog/current_repo_owned_catalog.json`;
  `tests/repo_owned_catalog_fixture.py` loads its 146 rows for catalog, migration,
