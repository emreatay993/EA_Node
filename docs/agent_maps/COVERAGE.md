...
- Standard input/output port rows are directly shared by `GraphNodePortRow.qml`; `GraphNodePortsLayer.qml` retains models, dynamic scheduling/mutations, notch cache, aggregate rectangles, context/menu/edit state, add controls, and the minimal direction-only children without adding per-port objects or loaders.
