...
- Shell projection ownership is direct: `library_projection.py` owns Library rows, filters, category trees, and display projections; `inspector_projection.py` owns selected-node projection; `quick_insert_projection.py` owns canvas/connection search and ranking. `LibraryPresenter` caches one category tree and projects grouped/display rows lazily.
