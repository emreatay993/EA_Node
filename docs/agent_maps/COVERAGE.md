...
- Action presentation is pure and QML-owned by `GraphActionPresentation.js`; six canvas/menu/toolbar/host consumers use it for lookup, normalization, field shaping, menu/popover extraction, and stable ordering while `GraphCanvasActionRouter.qml` remains the sole dispatch owner and Python/QML feature owners retain policy/lifecycle decisions.
