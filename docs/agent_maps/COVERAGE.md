...
- Media Panel video state is shared without a singleton player: `ui/media_video_state.py` owns Python normalization and `GraphMediaVideoPlaybackCore.qml` owns per-renderer playback/seek/clip/bookmark/primer behavior. Inline and fullscreen renderers retain their distinct host actions and handoff policy.
...
- Shell execution projection is direct: `RunProjectionController` alone owns node execution sets, elapsed/warning state, accepted-output observations, solution facts, port availability, failure focus, run-control status, and their signals over the one shared `ShellRunState`. `RunController` retains commands, preview, dispatch, Auto, Trigger, pause/resume/stop, history invalidation, and—until T12—the existing event-intake order.
...
- Graph-canvas QML composition is direct: `GraphCanvasStateBridge` reads persisted graphics only from the explicit `ShellWorkspacePresenter` graphics source and canvas/session facts only from `GraphCanvasPresenter`; `GraphCanvasCommandBridge` sends the corresponding command families to those same sources; `ViewportBridge` owns view facts and commands. Composition explicitly binds `ShellWorkspacePresenter` to `GraphSceneBridge`, whose payload fingerprint carries port-label visibility, graph-label size, effective title-icon size, and lightweight-canvas state. `GraphCanvas.qml` binds these focused owners without an aggregate facade or compatibility aliases.
- Content-fullscreen composition is also direct: shell composition supplies live owner providers/callbacks to one `ContentFullscreenBridge`; Viewer/Plot hosts observe it directly, ProjectSession owns artifact-store metadata publication, and no `ShellWindow` locator, policy facade, compatibility alias, dependency bag, or QML API change remains.
- `MainWindowShellTestBase` owns shared full-shell QML traversal and Inspector lookup; image/PDF subclasses own QML reference retention. `SharedMainWindowShellTestBase` owns shell reuse within each child, while manifest child-process boundaries own process isolation and bound the reuse scope.
