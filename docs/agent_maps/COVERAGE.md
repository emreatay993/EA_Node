...
- Mechanical's dependency-gated semantic values remain in `addons/mechanical/contracts.py`; its accepted-metadata selector adapter lives in `addons/mechanical/property_edit.py`, while its run/workspace session service and dedicated bounded owner process keep native imports out of the shell and shared execution worker.
