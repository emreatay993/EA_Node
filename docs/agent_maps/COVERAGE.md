...
- Durable solution selection/publication remains inside that execution-owned store through strict backend/factory ports. Persistence implements only immutable schema-1 sidecar I/O and isolated generation/reachability/prune primitives; project install injects/binds it, while `.cxproj` commit, Save/Save As binding switch, and lifecycle prune remain outside T07.
