...
- Mechanical's dependency-gated semantic values remain in `addons/mechanical/contracts.py`; `runtime.py` and `inspection.py` own Open, accepted discovery, and eleven-mode data-only Search execution, `tables.py` owns full-fidelity definition/result/probe/worksheet extraction, and `graphics.py` owns saved/current camera extraction with exact native presentation restoration. Its run/workspace service and dedicated bounded owner process keep native imports out of the shell and shared execution worker. Native working copies use source-family APIs; catalogue, typed Search, table, and camera payloads use bounded hashed owner spools over existing runtime codecs.
