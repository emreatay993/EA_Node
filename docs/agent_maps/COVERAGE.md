...
- Mechanical's dependency-gated semantic values remain in `addons/mechanical/contracts.py`; `runtime.py` and `inspection.py` own Open, accepted discovery, and eleven-mode data-only Search execution, `tables.py` owns full-fidelity definition/result/probe/worksheet extraction, `graphics.py` owns saved/current cameras plus deterministic viewport image batches and rollback-protected optional PNG publication, `commands.py` owns scoped scripts plus deterministic marker-owned APDL snippets with verified rollback, and `saving.py` owns standalone native save staging, reopen validation, and primary/companion publication rollback. Its run/workspace service and dedicated bounded owner process keep native imports out of the shell and shared execution worker. Native working copies use source-family APIs; catalogue, typed Search, table, camera, image, and save receipts use bounded hashed owner spools over existing runtime codecs.
