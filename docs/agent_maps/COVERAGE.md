...
- Mechanical's dependency-gated semantic values remain in `addons/mechanical/contracts.py`; `runtime.py` and `inspection.py` own Open, accepted discovery, and eleven-mode data-only Search execution, while its run/workspace service and dedicated bounded owner process keep native imports out of the shell and shared execution worker. Native working copies use source-family APIs; catalogue and typed Search payloads use bounded hashed owner spools over existing runtime codecs.
