...
- Mechanical's dependency-gated semantic values remain in `addons/mechanical/contracts.py`; `runtime.py` and `inspection.py` own Open, accepted discovery, and eleven-mode data-only Search execution, while `tables.py` owns full-fidelity definition, native ITable, configured-result, PlotData, ForceReaction, and qualified worksheet extraction into existing immutable TableValue/Definitions outputs. Its run/workspace service and dedicated bounded owner process keep native imports out of the shell and shared execution worker. Native working copies use source-family APIs; catalogue, typed Search, and table payloads use bounded hashed owner spools over existing runtime codecs.
...
- The exact current catalog is `tests/fixtures/node_catalog/current_repo_owned_catalog.json`;
  `tests/repo_owned_catalog_fixture.py` loads its 140 rows for catalog, migration,
