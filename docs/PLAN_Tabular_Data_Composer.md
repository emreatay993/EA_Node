...
### T13 — Preview containment and delimited-file verification (2026-09-15)

The user reported side-border overlap in the inline preview and requested CSV/TSV Configure Data verification. The reproduction showed zero content inset inside the body-region panel: the table could paint over its border. The tabular surface now uses a stroke/corner-aware inset, and content/viewport clipping contains scrolled cells, headers and scrollbars. CSV, TSV and semicolon-delimited text pass real composer mapping, filter/range entry and Apply, then JSON property round-trip, fresh-loader reads and full CSV export with unchanged original bytes. **Complete: 198 checks and 35 subtests passed**, including the reproduced-then-fixed containment regression, toolbar behavior, composer/export/loader tests and shared surface tests. The scrolled node was rendered and inspected at 1.5x scale with its migration notice. Agent-map, traceability, Markdown-link and whitespace checks passed. The user's subsequent 2026-09-15 instruction authorizes committing/pushing this fix to main. Only the tabular source/tests and corresponding documentation/index changes belong to that publication; concurrent media/shared-surface/execution work remains excluded. The task response and Git remote identify the published revision.
