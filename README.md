...
The shipped DPF backend remains intentionally narrow and optional: existing
handwritten compute nodes plus `dpf.viewer` stay as the only built-in DPF
surfaces on this branch, `ansys-dpf-core` is optional at startup, saved DPF
nodes reopen as read-only missing-plugin placeholders when the backend is not
available, and broad autogenerated operator exposure plus non-operator
reflection remain deferred. See the
...
for the frozen preparation contract and retained proof surface.
