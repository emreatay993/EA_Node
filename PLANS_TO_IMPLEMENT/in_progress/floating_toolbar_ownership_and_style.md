...
### 3. Setting plumbing (follows the existing canvas-preference pattern)
...
On the Canvas page under a new "Floating toolbar" sub-section (beneath the existing "Overlay" card), render a `QComboBox` labelled "Floating toolbar style" with the three choices. Keep it a compact combo. One-line helper label: *"Chrome style for the hover toolbar above nodes."*
...
- `ea_node_editor/ui_qml/graph_canvas_state_bridge.py` — add `graphics_floating_toolbar_style` beside the existing canvas preference properties.
