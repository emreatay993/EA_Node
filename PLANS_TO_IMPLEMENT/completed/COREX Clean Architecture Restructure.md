...
I would not do a greenfield rewrite. I would layer a new architecture program on top of the existing no-legacy cleanup contracts, preserving current `.cxproj` behavior, launch paths, QML affordances, plugin descriptors, runtime snapshots, and viewer transport. The main goal is to break the current 14-domain import cycle into one-way layers with explicit service ports.
...
- Graph/workspace: mutation, Track-B, graph surface, Group backdrop, passive runtime wiring.
...
- Current `.cxproj` files, descriptor-first plugins, runtime snapshots, and typed viewer transport remain canonical.
