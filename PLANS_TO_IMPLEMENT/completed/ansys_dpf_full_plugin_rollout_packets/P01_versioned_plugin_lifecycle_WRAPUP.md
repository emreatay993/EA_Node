...
Yes: DPF startup now records the exact installed version, refreshes the builtin descriptor cache on version change, and preserves the missing-dependency path without touching `.cxproj` documents.
