...
- Ship `Peek Inside` in the same milestone as a Group-only, temporary isolated view for collapsed Groups.
...
- Expand auto-push:
  ...
  - Compute a non-mutating post-expand footprint before commit. For Groups, the fixed footprint is the backdrop plus its current direct members; for subnodes and other nodes, use the post-expand projected visible bounds. Internal contents keep their relative layout and are never rearranged.
...
- Comment `Peek Inside`:
  ...
  - Show the Group backdrop plus its current direct members in the current canvas scope; do not recursively open nested subnode contents. Show only edges whose endpoints are still visible.
...
- Peek tests: action appears only on collapsed Groups; filtered view shows direct members only; edits inside peek persist; exit works by button and empty-canvas click; normal subnode scope navigation is unchanged.
