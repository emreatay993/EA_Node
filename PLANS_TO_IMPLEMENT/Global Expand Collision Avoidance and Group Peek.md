# Global Expand Collision Avoidance and Group Peek
...
- Ship a companion `Peek Inside` action for collapsed Groups as a temporary, isolated, fully editable view of the Group's direct current members, with explicit exit control plus click-away dismissal.
...
- New Group context-menu action: `Peek Inside`.
...
- Non-goals: no expand collision solver, no geometry translation, no Group Peek mode, no Group context-menu changes.
...
- Goal: Add the expand-time collision solver and grouped mutation/history wiring for collapsed item expansion, including group-backdrop occupied bounds and configurable solver behavior.
- Preconditions: `T01`
- Conservative write scope: `ea_node_editor/ui_qml/graph_scene/context.py`, `ea_node_editor/ui_qml/graph_scene_bridge.py`, `ea_node_editor/ui_qml/graph_scene_mutation_history.py`, `ea_node_editor/ui_qml/graph_scene_mutation/selection_and_scope_ops.py`, `ea_node_editor/ui_qml/graph_scene_mutation/alignment_and_distribution_ops.py`, `ea_node_editor/ui_qml/graph_scene_mutation/collision_avoidance_ops.py`, `ea_node_editor/ui_qml/graph_scene_payload_builder.py`, `ea_node_editor/graph/group_backdrop_geometry.py`, `ea_node_editor/graph/transform_layout_ops.py`, `ea_node_editor/graph/mutation_service.py`, `tests/test_graph_scene_bridge_bind_regression.py`, `tests/test_group_backdrop_membership.py`, `tests/test_group_backdrop_collapse.py`, `tests/test_graph_surface_input_contract.py`
...
- Verification: `.\venv\Scripts\python.exe -m pytest tests/test_graph_scene_bridge_bind_regression.py tests/test_group_backdrop_membership.py tests/test_group_backdrop_collapse.py tests/test_graph_surface_input_contract.py --ignore=venv -q`
- Non-goals: no Group Peek mode, no new lock or pin behavior, no restore-on-collapse behavior, no batch expand orchestration, no future stylus model implementation beyond keeping the solver adapter-friendly.
...
- Verification: `.\venv\Scripts\python.exe -m pytest tests/main_window_shell/bridge_contracts_graph_canvas.py tests/main_window_shell/group_backdrop_workflows.py tests/main_window_shell/passive_style_context_menus.py tests/test_main_window_shell.py tests/graph_surface_pointer_regression.py --ignore=venv -q`
- Non-goals: no reuse of subnode `scope_path` for Group Peek, no recursive reveal of nested subnode internals, no new docs or QA-matrix closeout packet.
...
4. `P03 Group Peek Mode and Canvas Entry Points`: derived primarily from `T03`.
...
- Expand solver and history regression: `tests/test_graph_scene_bridge_bind_regression.py`, `tests/test_group_backdrop_membership.py`, `tests/test_group_backdrop_collapse.py`, `tests/test_graph_surface_input_contract.py`
- Group Peek and canvas regression: `tests/main_window_shell/bridge_contracts_graph_canvas.py`, `tests/main_window_shell/group_backdrop_workflows.py`, `tests/main_window_shell/passive_style_context_menus.py`, `tests/test_main_window_shell.py`, `tests/graph_surface_pointer_regression.py`
...
- `Peek Inside` is group-backdrop-only, shows direct current members only, stays fully editable, and does not reuse the subnode scope-path architecture.
